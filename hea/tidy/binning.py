"""ggplot2-style continuous-to-factor binning: ``cut_width`` /
``cut_interval`` / ``cut_number``.

Algorithms mirror ggplot2's ``R/bin.R``; the actual binning delegates to
:func:`hea.R.cut` so labels follow R's ``"(a,b]"``-style formatting.
Each accepts either eager input (Series / ndarray / list) or a column
