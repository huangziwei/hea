...
def _xwx(xi, wxj):
    ...
    if _rs_gamlss_xwx is not None:
        return np.asarray(
            _rs_gamlss_xwx(np.ascontiguousarray(xi), np.ascontiguousarray(wxj))
        )
...
def _dbinom_raw_disp(x, n, p, q, give_log=True):
    ...
    a = [
        np.ascontiguousarray(np.broadcast_to(v, shape), dtype=float).ravel()
        for v in (x, n, p, q)
    ]
...
class LogLink(Link):
    ...
    def linkinv(self, eta):
        ...
        return np.maximum(np.exp(np.asarray(eta, dtype=float)), np.finfo(float).eps)
    ...
    def d2link(self, mu):
        return -1.0 / _rpow_int(np.asarray(mu, dtype=float), 2)

    def d3link(self, mu):
        return 2.0 / _rpow_int(np.asarray(mu, dtype=float), 3)

    def d4link(self, mu):
        return -6.0 / _rpow_int(np.asarray(mu, dtype=float), 4)
    ...
    def g3g(self, mu):
        return 2.0 * np.ones_like(np.asarray(mu, dtype=float))

    def g4g(self, mu):
        return -6.0 * np.ones_like(np.asarray(mu, dtype=float))
...
class InverseLink(Link):
    ...
    def mu_eta(self, eta):
        return -1.0 / _rpow_int(np.asarray(eta, dtype=float), 2)
    ...
    def d4link(self, mu):
        return 24.0 / _rpow(np.asarray(mu, dtype=float), 5.0)
    ...
    def g3g(self, mu):
        return 6.0 * _rpow_int(np.asarray(mu, dtype=float), 2)

    def g4g(self, mu):
        return 24.0 * _rpow_int(np.asarray(mu, dtype=float), 3)
...
class SqrtLink(Link):
    ...
    def d2link(self, mu):
        return -0.25 * np.asarray(mu, dtype=float) ** -1.5

    def d3link(self, mu):
        return 0.375 * np.asarray(mu, dtype=float) ** -2.5

    def d4link(self, mu):
        return -0.9375 * np.asarray(mu, dtype=float) ** -3.5
...
class PowerLink(Link):
    ...
    def linkinv(self, eta):
        ...
        return np.maximum(
            np.asarray(eta, dtype=float) ** (1.0 / self.lam),
            eps,
        )
    ...
    def mu_eta(self, eta):
        eps = np.finfo(float).eps
        return np.maximum(
            np.asarray(eta, dtype=float) ** (1.0 / self.lam - 1.0) / self.lam,
            eps,
        )
    ...
    def d3link(self, mu):
        lam = self.lam
        return (
            lam * (lam - 1.0) * (lam - 2.0) * np.asarray(mu, dtype=float) ** (lam - 3.0)
        )
...
class LogitLink(Link):
    ...
    def linkinv(self, eta):
        ...
        tmp = np.where(eta < -30.0, eps, np.where(eta > 30.0, 1.0 / eps, tmp))
        return tmp / (1.0 + tmp)
...
class CloglogLink(Link):
    ...
    def d4link(self, mu):
        ...
        l1m = np.log1p(-mu)
        return (
            (-12.0 - 11.0 * l1m - 6.0 * l1m**2 - 6.0 / l1m) / (1.0 - mu) ** 4 / l1m**3
        )
...
class InverseSquareLink(Link):
    ...
    def d2link(self, mu):
        return 6.0 * np.asarray(mu, dtype=float) ** -4

    def d3link(self, mu):
        return -24.0 * np.asarray(mu, dtype=float) ** -5

    def d4link(self, mu):
        return 120.0 * np.asarray(mu, dtype=float) ** -6
...
class SoftplusLink(Link):
    ...
    * ``g2g = g″/g′²  = −u``     ``g3g = g‴/g′³ = u(1+u)`` ``g4g = g⁗/g′⁴ = −u(1+4u+u²)``
    ...
    def linkinv(self, eta):
        ...
        return np.maximum(
            np.logaddexp(0.0, np.asarray(eta, dtype=float)), np.finfo(float).eps
        )

    def mu_eta(self, eta):
        # dμ/dη = σ(η); expit is the stable logistic. Lower-clamp like mgcv.
        return np.maximum(expit(np.asarray(eta, dtype=float)), np.finfo(float).eps)
...
def find_null_dev(family: Family, y, eta, offset, weights) -> float:
    ...
    eta = np.asarray(eta, dtype=float)
    offset = np.zeros_like(eta) if offset is None else np.asarray(offset, dtype=float)
    ...
    def dDeta(self, y, mu, wt, theta, level: int = 0, dd: dict | None = None) -> dict:
        ...
        if level > 0:
            ...
            with np.errstate(invalid="ignore"):
                ...
                d["Deta2th"] = (
                    _cb(r["Dmu2th"], ig12) - _cb(_cb(r["Dmuth"], g2g), ig1)
                    if has_th
                    else None
                )
        ...
        if level > 1:
            ...
            with np.errstate(invalid="ignore"):
                d["Deta4"] = (
                    ...
                    - r["Dmu"] * (15.0 * g2g**3 - 10.0 * g2g * g3g + g4g) * ig1
                )
                d["Detath2"] = _cb(r["Dmuth2"], ig1) if has_th2 else None
                d["Deta2th2"] = (
                    _cb(r["Dmu2th2"], ig12) - _cb(_cb(r["Dmuth2"], g2g), ig1)
                    if has_th2
                    else None
                )
                d["Deta3th"] = (
                    (
                        _cb(r["Dmu3th"], ig13)
                        - _cb(
                            _cb(3.0 * np.asarray(r["Dmu2th"], dtype=float), g2g), ig12
                        )
                        + _cb(_cb(r["Dmuth"], 3.0 * g2g * g2g - g3g), ig1)
                    )
                    if has_th2
                    else None
                )
...
class Gaussian(Family):
    ...
    def ls(self, y, wt, scale):
        ...
        ls0 = -0.5 * nobs * np.log(2.0 * np.pi * scale) + 0.5 * float(
            np.sum(np.log(wt[good]))
        )
    ...
    def aic(self, y, mu, dev, wt, n, theta=None):
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            logp = _nmath._disp(
                "dgamma",
                _nmath.dgamma,
                [y, 1.0 / disp, np.asarray(mu, dtype=float) * disp],
                (True,),
            )
...
class Poisson(Family):
    ...
    def variance(self, mu):
        return np.asarray(mu, dtype=float).copy()
    ...
    def dev_resids(self, y, mu, wt, theta=None):
        ...
        contrib = np.where(positive, wt * (y * np.log(ratio) - (y - mu)), wt * mu)
        return 2.0 * contrib
    ...
    def qf(self, p, mu, wt, scale):
        return _nmath._disp(
            "qpois", _nmath.qpois, [p, np.asarray(mu, dtype=float)], (True, False)
        )
...
class Binomial(Family):
    ...
    def initialize(self, y, wt, n=None, warn_non_integer=True):
        ...
        if warn_non_integer:
            ...
            if np.any(np.abs(m - np.rint(m)) > 0.001):
                ...
                _w.warn("non-integer #successes in a binomial glm!", stacklevel=2)
    ...
    def aic(self, y, mu, dev, wt, n, theta=None):
        ...
        if (
            n_arr is not None
            and n_arr.ndim == 1
            and n_arr.shape == y.shape
            and np.any(n_arr > 1.0)
        ):
            good = n_arr > 0
    ...
    def qf(self, p, mu, wt, scale):
        ...
        if not np.allclose(wt, np.ceil(wt)):
            ...
            _w.warn(
                "non-integer binomial denominator: quantiles incorrect", stacklevel=2
            )
        q = _nmath._disp(
            "qbinom", _nmath.qbinom, [p, wt, np.asarray(mu, dtype=float)], (True, False)
        )
        return q / (wt + (wt == 0))

    def rd(self, rng, mu, wt, scale):
        wt = np.asarray(wt, dtype=float)
        d = rng.binomial(np.rint(wt).astype(np.int64), np.asarray(mu, dtype=float))
        return d / (wt + (wt == 0))
...
class InverseGaussian(Family):
    ...
    def aic(self, y, mu, dev, wt, n, theta=None):
        ...
        sw = float(wt.sum())
        return (
            sw * (1.0 + np.log(dev / sw * 2.0 * np.pi))
            + 3.0 * float(np.sum(np.log(y) * wt))
            + 2.0
        )
    ...
    def ls(self, y, wt, scale):
        ...
        nobs = int(np.sum(good))
        ls0 = -0.5 * float(
            np.sum(np.log(2.0 * np.pi * scale * y[good] ** 3))
    ...
    def rd(self, rng, mu, wt, scale):
        ...
        x[big] = mu[big] * (
            1.0 + 0.5 * (mys[big] - np.sqrt(4.0 * mys[big] + mys[big] ** 2))
        )
...
class Quasi(Family):
    ...
    def variance(self, mu):
        return self._shadow.variance(mu)

    def dvar(self, mu):
        return self._shadow.dvar(mu)

    def d2var(self, mu):
        return self._shadow.d2var(mu)

    def d3var(self, mu):
        return self._shadow.d3var(mu)
    ...
    def ls(self, y, wt, scale, n=None):
        ...
        good = wt > 0
        nobs = int(np.sum(good))
        ls0 = -0.5 * nobs * np.log(scale) + 0.5 * float(np.sum(np.log(wt[good])))
        return np.array([ls0, -0.5 * nobs, 0.0], dtype=float)
...
def _tweedie_log_a_one(y_i: float, phi_i: float, p: float):
    ...
    log_z = -alpha * ly + alpha * np.log(p - 1.0) - one_minus_alpha * rho - np.log(tm)
    log_neg = np.log(-om1) + rho
    wp_base = log_neg / onep2 - alpha / om1 + 1.0 / tm
    wp2_base = 2.0 * log_neg / onep3 - (3.0 * alpha - 2.0) / onep2 + 1.0 / (tm * tm)
...
def _tweedie_series_rs(ly, j_int, alpha, w_base, wp_base, wp2_base, onep, J):
    ...
    while J < _LD_J_MAX:
        ...
        lw_peak = (
            j_int * log_z
            - _nmath._lgammafn_arr(j_int + 1.0)
...
def _tweedie_log_a_vec(y, phi, p, _chunk_bytes: int = 256 * 1024 * 1024):
    ...
    """
    y = np.asarray(y, dtype=float)
    phi_arr = np.broadcast_to(np.asarray(phi, dtype=float), y.shape).astype(
        float, copy=True
    )
    ...
    wp2_base = 2.0 * log_neg / onep3 - (3.0 * alpha - 2.0) / onep2 + 1.0 / (tm * tm)
    j_star = np.maximum(
        np.exp((log_z + alpha * np.log(-alpha)) / one_minus_alpha),
        1.0,
    )
    ...
    peak_var = 1.0 / (
        polygamma(1, j_int_max + 1.0) + alpha * alpha * polygamma(1, -j_int_max * alpha)
    )
    ...
    if _rs_tweedie_series is not None:
        ...
        (out_la, out_jb, out_jv, out_jpb, out_m1, out_mc, out_md) = _tweedie_series_rs(
        ...
        while True:
            ...
            for s in range(0, n_active, chunk):
                ...
                out_jb[s:e] = jb_c
                out_jv[s:e] = np.sum(p_w * j_grid[None, :] ** 2, axis=1) - jb_c * jb_c
                out_jpb[s:e] = np.sum(p_w * j_grid[None, :] * psi_arr[None, :], axis=1)
                ...
                wp1 = (
                    j_grid[None, :] * wp_base[s:e, None]
                    + xj
                    - j_grid[None, :] * (ly[s:e] / onep2)[:, None]
                )
                wp2 = (
                    j_grid[None, :] * wp2_base[s:e, None]
                    + 2.0 * xj / om1
                ...
                out_md[s:e] = np.sum(
                    p_w * (wp1 * j_grid[None, :] / om1 + (j_grid / onep2)[None, :]),
                    axis=1,
                )
            if not grow:
                break
...
def _tweedious_work_scalar_py(y, rho, p, dpth1, dpth2):
    ...
    for i in range(n):
        ...
        wmax = j_max * w_base - lg(j_max + 1.0) - lg(-j_max * alpha) - j_max * alogy_i
        wmin = wmax + log_eps
        wi = 0.0
        w1i = 0.0
        w2i = 0.0
        ...
        while True:
            ...
            wp2j = (
                j * wp2_base + 2.0 * xdig / onep - trig * xx * xx
...
def _tweedious_work_pv_py(y, rho, p, dpth1, dpth2):
    ...
    for i in range(n):
        ...
        wmax = j_max * w_base - lg(j_max + 1.0) - lg(-j_max * alpha) - j_max * alogy
        wmin = wmax + log_eps
        wi = w1i = w2i = 0.0
    ...
    if _rs_tweedious_work_pv is not None:
        return _rs_tweedious_work_pv(
            ...
            np.ascontiguousarray(
                np.broadcast_to(np.asarray(rho, dtype=float), np.shape(y)).astype(float)
            ),
            np.ascontiguousarray(
                np.broadcast_to(np.asarray(p, dtype=float), np.shape(y)).astype(float)
            ),
            np.ascontiguousarray(
                np.broadcast_to(np.asarray(dpth1, dtype=float), np.shape(y)).astype(
                    float
                )
            ),
            np.ascontiguousarray(
                np.broadcast_to(np.asarray(dpth2, dtype=float), np.shape(y)).astype(
                    float
                )
            ),
            _TWEEDIOUS_LOG_EPS,
...
def _ld_tweedie_work(
    y, mu, theta, rho, a: float = 1.001, b: float = 1.999
) -> np.ndarray:
    ...
    y = np.asarray(y, dtype=float)
    n = y.shape[0]
    mu = np.ascontiguousarray(np.broadcast_to(np.asarray(mu, dtype=float), y.shape))
    theta = np.ascontiguousarray(
        np.broadcast_to(np.asarray(theta, dtype=float), y.shape)
    )
    rho = np.ascontiguousarray(np.broadcast_to(np.asarray(rho, dtype=float), y.shape))
    if not (1.0 < a < b < 2.0):
    ...
    p = np.where(pos, (b + a * eth) / (1.0 + eth), (b * eth + a) / (eth + 1.0))
    ...
    if np.any(zm):
        ...
        ld[zm, 4] = 2.0 * ld[zm, 3] / (2.0 - p_z) + ld[zm, 0] * lmu_z**2
    ...
    if any_pos:
        ...
        x_ = (
            theta_s * y_i * (1.0 / onep - log_mu) / phii
            + k_theta * (log_mu - 1.0 / twop) / phii
        )
        ld[ind, 3] = x_
        ld[ind, 4] = (
            theta_s * y_i * (log_mu**2 - 2.0 * log_mu / onep + 2.0 / onep**2) / phii
    ...
    if any_pos:
        ...
        if buffer:
            sw = _tweedious_work_scalar(
                y_i,
                float(rho[ind].flat[0]),
                float(p_i.flat[0]),
                float(dpth1[ind].flat[0]),
                float(dpth2[ind].flat[0]),
            )
        else:
            sw = _tweedious_work_pv(y_i, rho[ind], p_i, dpth1[ind], dpth2[ind])
        ld[ind, 0] += sw[:, 0]
        ld[ind, 1] += sw[:, 1]
        ld[ind, 2] += sw[:, 2]
        ld[ind, 3] += sw[:, 3]
...
def _tw_null_fit(y, a: float = 1.001, b: float = 1.999):
    ...
    def _ld_sums(t):
        ld = _ld_tweedie_work(
            y, np.exp(t[0]) * ones, t[1] * ones, t[2] * ones, a=a, b=b
        )
...
def _ax2m1_div_x2m2_sq(x, m1, m2, a=1.0):
    ...
    if np.any(nk):
        out[nk] = (
            (_sqrt_x2pm(np.sqrt(a) * x[nk], m1) / _sqrt_x2pm(x[nk], m2))
            / _sqrt_x2pm(x[nk], m2)
        ) ** 2
    return out
...
def _shash_derivs(y, mu, tau, eps, phi, phi_pen, deriv):
    ...
    l0 = (
        ...
        + np.log(CC)
        - 0.5 * _shash_log1pexp(log_abs_z2)
        - 0.5 * SS**2
        - phi_pen * phi**2
    )
    L1 = L2 = L3 = L4 = None
    if deriv >= 1:
        ...
        Dmm = (
            ...
            + _ax2m1_div_x2m2_sq(z, -1.0, 1.0) / (delta * sig * delta * sig)
        )
        Dmt = zsd * Dmm - Dm
    ...
    if deriv > 1:
        ...
        Dmmm = (
            2 * z * Dme / (sig * sig * delta * sSp1**3)
            + Dmme / (sig * sSp1)
            + _ax2m1_div_x2m2_sq(z, -1, 1, 2) * De / (sig**3 * delta**2 * sSp1)
            + 2
            * (z / sSp1)
            * _ax2m1_div_x2m2_sq(z, -3, 1)
            / ((sig * delta) ** 3 * sSp1)
        )
        Dmmt = zsd * Dmmm - 2 * Dmm
        Dtee = zsd * Dmee
    ...
    if deriv > 3:
        ...
        bgg4 = aee5 - aaa2 / (exp1**t * np.sqrt(exp1 ** (2 * abb5) * aaa2**2 + 1))
        ...
        j2 = (
            ...
            - (12 * bjj24 * aaa2 * app16 * abb9**3) / abb8**3
            + 8 * bjj14 * app10 * aqq27 * aff17
            + 4 * app08 * app10 * aqq27 * aff17
            - 15 * bjj08 * abb3 * bjj11 * aqq27 * aff17
            - 4 * bjj14 * app10 * aff17
            + 4 * app08 * app10 * aff17
            - 15 * bjj08 * abb3 * bjj11 * aff17
            - 9 * bjj26 * aaa2 * app16 * abb8 * abb9
            + 24 * bjj24 * aaa2 * app16 * abb8 * abb9
            + 15 * bjj18 * ajj15 * bjj21 * abb8 * abb9
            + 9 * bjj26 * aaa2 * app16 * agg17 * abb9
            + 12 * bjj24 * aaa2 * app16 * agg17 * abb9
            - 15 * bjj18 * ajj15 * bjj21 * agg17 * abb9
            - 4 * bjj14 * app10 * aff13
            + 4 * app08 * app10 * aff13
            - 15 * bjj08 * abb3 * bjj11 * aff13
            - 2 * bjj14 * app10
            - 4 * app08 * app10
            + 15 * bjj08 * abb3 * bjj11
            + (6 * exp1 ** ((-4 * t) - 4 * p)) / app04**2
        ...
        bkk34 = abb9**3
        k2 = (
            (-(6 * bjj14 * aaa2 * app10 * abb9**4) / abb8**4)
            + 6 * app21 * aff15 * bkk33 * bkk34
            - 12 * bjj24 * abb3 * app16 * bkk33 * bkk34
            + 8 * bjj14 * aaa2 * app10 * aqq27 * aff17
            + 13 * app08 * aaa2 * app10 * aqq27 * aff17
            - 15 * bjj08 * ajj15 * bjj11 * aqq27 * aff17
            - 4 * bjj14 * aaa2 * app10 * aff17
            + 13 * app08 * aaa2 * app10 * aff17
            - 15 * bjj08 * ajj15 * bjj11 * aff17
            - 12 * app21 * aff15 * abb8 * abb9
            + 3 * aff14 * aff15 * abb8 * abb9
            - 18 * bjj26 * abb3 * app16 * abb8 * abb9
            + 24 * bjj24 * abb3 * app16 * abb8 * abb9
            + 15 * bjj18 * att19 * bjj21 * abb8 * abb9
            - 6 * app21 * aff15 * agg17 * abb9
            - 3 * aff14 * aff15 * agg17 * abb9
            + 18 * bjj26 * abb3 * app16 * agg17 * abb9
            + 12 * bjj24 * abb3 * app16 * agg17 * abb9
            - 15 * bjj18 * att19 * bjj21 * agg17 * abb9
            - 4 * bjj14 * aaa2 * app10 * aff13
            + 13 * app08 * aaa2 * app10 * aff13
            - 15 * bjj08 * ajj15 * bjj11 * aff13
            - 2 * bjj14 * aaa2 * app10
            - 13 * app08 * aaa2 * app10
            + 15 * bjj08 * ajj15 * bjj11
        ...
        l2 = (
            ...
            - (6 * bll16 * aaa2 * app10 * abb9**3) / abb8**3
            + 8 * app21 * aff15 * aqq27 * aff17
            + aff14 * aff15 * aqq27 * aff17
            - 3 * app14 * abb3 * app16 * aqq27 * aff17
            - 4 * app21 * aff15 * aff17
            + aff14 * aff15 * aff17
            - 3 * app14 * abb3 * app16 * aff17
            + 12 * bll16 * aaa2 * app10 * abb8 * abb9
            + (6 * bll16 * aaa2 * app10 * abb9) / abb8
            - 4 * app21 * aff15 * aff13
            + aff14 * aff15 * aff13
            - 3 * app14 * abb3 * app16 * aff13
            - 2 * app21 * aff15
            - aff14 * aff15
            + 3 * app14 * abb3 * app16
        )
        ...
        bmm35 = abb9**3
        m2 = (
            (6 * app21 * aff15 * ass16 * abb9**4) / abb8**4
            + 6 * app08 * aaa2 * app10 * ass16 * bmm34 * bmm35
            - 6 * bjj24 * abb3 * app16 * bmm34 * bmm35
            - 8 * app21 * aff15 * ass16 * ass28 * aff17
            - aff14 * aff15 * ass16 * ass28 * aff17
            + 3 * bjj26 * abb3 * app16 * ass16 * ass28 * aff17
            + 6 * app08 * aaa2 * app10 * ass28 * aff17
            - 12 * bjj08 * ajj15 * bjj11 * ass28 * aff17
            + 4 * app21 * aff15 * ass16 * aff17
            - aff14 * aff15 * ass16 * aff17
            + 3 * bjj26 * abb3 * app16 * ass16 * aff17
            + 6 * app08 * aaa2 * app10 * aff17
            - 12 * bjj08 * ajj15 * bjj11 * aff17
            - 12 * app08 * aaa2 * app10 * ass16 * abb8 * abb9
            + 2 * aff14 * aff15 * abb8 * abb9
            - 15 * bjj26 * abb3 * app16 * abb8 * abb9
            + 12 * bjj24 * abb3 * app16 * abb8 * abb9
            + 15 * bjj18 * att19 * bjj21 * abb8 * abb9
            - 6 * app08 * aaa2 * app10 * ass16 * ass23 * abb9
            - 2 * aff14 * aff15 * ass23 * abb9
            + 15 * bjj26 * abb3 * app16 * ass23 * abb9
            + 6 * bjj24 * abb3 * app16 * ass23 * abb9
            - 15 * bjj18 * att19 * bjj21 * ass23 * abb9
            + 4 * app21 * aff15 * ass16 * aff13
            - aff14 * aff15 * ass16 * aff13
            + 3 * bjj26 * abb3 * app16 * ass16 * aff13
            + 6 * app08 * aaa2 * app10 * aff13
            - 12 * bjj08 * ajj15 * bjj11 * aff13
            + 2 * app21 * aff15 * ass16
            + aff14 * aff15 * ass16
            - 3 * bjj26 * abb3 * app16 * ass16
            - 6 * app08 * aaa2 * app10
            + 12 * bjj08 * ajj15 * bjj11
            + (24 * exp1 ** ((-4 * t) - 4 * p) * aaa2) / app04**2
            - (72 * exp1 ** ((-6 * t) - 6 * p) * ajj15) / app04**3
            + (48 * exp1 ** ((-8 * t) - 8 * p) * aaa2**5) / app04**4
        )
        n2 = (
            (-(6 * bjj14 * abb3 * app10 * abb9**4) / abb8**4)
            + 10 * app21 * aaa2 * aff15 * bkk33 * bkk34
            - 12 * bjj24 * ajj15 * app16 * bkk33 * bkk34
            - 4 * aff05 * aff10 * aqq27 * aff17
            + 8 * bjj14 * abb3 * app10 * aqq27 * aff17
            + 19 * app08 * abb3 * app10 * aqq27 * aff17
            - 15 * bjj08 * att19 * bjj11 * aqq27 * aff17
            - 4 * aff05 * aff10 * aff17
            - 4 * bjj14 * abb3 * app10 * aff17
            + 19 * app08 * abb3 * app10 * aff17
            - 15 * bjj08 * att19 * bjj11 * aff17
            - 20 * app21 * aaa2 * aff15 * abb8 * abb9
            + 9 * aff14 * aaa2 * aff15 * abb8 * abb9
            - 24 * bjj26 * ajj15 * app16 * abb8 * abb9
            + 24 * bjj24 * ajj15 * app16 * abb8 * abb9
            + 15 * bjj18 * azz19 * bjj21 * abb8 * abb9
            - 10 * app21 * aaa2 * aff15 * agg17 * abb9
            - 9 * aff14 * aaa2 * aff15 * agg17 * abb9
            + 24 * bjj26 * ajj15 * app16 * agg17 * abb9
            + 12 * bjj24 * ajj15 * app16 * agg17 * abb9
            - 15 * bjj18 * azz19 * bjj21 * agg17 * abb9
            - 4 * aff05 * aff10 * aff13
            - 4 * bjj14 * abb3 * app10 * aff13
            + 19 * app08 * abb3 * app10 * aff13
            - 15 * bjj08 * att19 * bjj11 * aff13
            + 4 * aff05 * aff10
            - 2 * bjj14 * abb3 * app10
            - 19 * app08 * abb3 * app10
            + 15 * bjj08 * att19 * bjj11
            - (4 * aqq03) / aqq05
        ...
        o2 = (
            (-(6 * app21 * aaa2 * aff15 * abb9**4) / abb8**4)
            + 4 * aff05 * aff10 * bkk33 * bkk34
            - 6 * bll16 * abb3 * app10 * bkk33 * bkk34
            + 8 * app21 * aaa2 * aff15 * aqq27 * aff17
            + 3 * aff14 * aaa2 * aff15 * aqq27 * aff17
            - 3 * app14 * ajj15 * app16 * aqq27 * aff17
            - 4 * app21 * aaa2 * aff15 * aff17
            + 3 * aff14 * aaa2 * aff15 * aff17
            - 3 * app14 * ajj15 * app16 * aff17
            - 8 * aff05 * aff10 * abb8 * abb9
            + 12 * bll16 * abb3 * app10 * abb8 * abb9
            - 4 * aff05 * aff10 * agg17 * abb9
            + 6 * bll16 * abb3 * app10 * agg17 * abb9
            - 4 * app21 * aaa2 * aff15 * aff13
            + 3 * aff14 * aaa2 * aff15 * aff13
            - 3 * app14 * ajj15 * app16 * aff13
            - 2 * app21 * aaa2 * aff15
            - 3 * aff14 * aaa2 * aff15
            + 3 * app14 * ajj15 * app16
        )
        p2 = (
            (6 * app21 * aaa2 * aff15 * ass16 * abb9**4) / abb8**4
            - 4 * aff05 * aff10 * ass16 * bmm34 * bmm35
            + 6 * app08 * abb3 * app10 * ass16 * bmm34 * bmm35
            - 6 * bjj24 * ajj15 * app16 * bmm34 * bmm35
            - 8 * app21 * aaa2 * aff15 * ass16 * ass28 * aff17
            - 3 * aff14 * aaa2 * aff15 * ass16 * ass28 * aff17
            + 3 * bjj26 * ajj15 * app16 * ass16 * ass28 * aff17
            + 10 * app08 * abb3 * app10 * ass28 * aff17
            - 12 * bjj08 * att19 * bjj11 * ass28 * aff17
            + 4 * app21 * aaa2 * aff15 * ass16 * aff17
            - 3 * aff14 * aaa2 * aff15 * ass16 * aff17
            + 3 * bjj26 * ajj15 * app16 * ass16 * aff17
            + 10 * app08 * abb3 * app10 * aff17
            - 12 * bjj08 * att19 * bjj11 * aff17
            + 8 * aff05 * aff10 * ass16 * abb8 * abb9
            - 12 * app08 * abb3 * app10 * ass16 * abb8 * abb9
            + 6 * aff14 * aaa2 * aff15 * abb8 * abb9
            - 21 * bjj26 * ajj15 * app16 * abb8 * abb9
            + 12 * bjj24 * ajj15 * app16 * abb8 * abb9
            + 15 * bjj18 * azz19 * bjj21 * abb8 * abb9
            + 4 * aff05 * aff10 * ass16 * ass23 * abb9
            - 6 * app08 * abb3 * app10 * ass16 * ass23 * abb9
            - 6 * aff14 * aaa2 * aff15 * ass23 * abb9
            + 21 * bjj26 * ajj15 * app16 * ass23 * abb9
            + 6 * bjj24 * ajj15 * app16 * ass23 * abb9
            - 15 * bjj18 * azz19 * bjj21 * ass23 * abb9
            + 4 * app21 * aaa2 * aff15 * ass16 * aff13
            - 3 * aff14 * aaa2 * aff15 * ass16 * aff13
            + 3 * bjj26 * ajj15 * app16 * ass16 * aff13
            + 10 * app08 * abb3 * app10 * aff13
            - 12 * bjj08 * att19 * bjj11 * aff13
            + 2 * app21 * aaa2 * aff15 * ass16
            + 3 * aff14 * aaa2 * aff15 * ass16
            - 3 * bjj26 * ajj15 * app16 * ass16
            - 10 * app08 * abb3 * app10
            + 12 * bjj08 * att19 * bjj11
            - (4 * aqq03) / aqq05
            + (44 * exp1 ** ((-4 * t) - 4 * p) * abb3) / aqq05**2
            - (88 * exp1 ** ((-6 * t) - 6 * p) * att19) / aqq05**3
            + (48 * exp1 ** ((-8 * t) - 8 * p) * aaa2**6) / aqq05**4
        ...
        q2 = (
            ...
            + (8 * aff05 * arr12 * aff17) / aff13
            - 4 * aff05 * arr12 * aff17
            + 4 * aff14 * aaa2 * arr07 * abb8 * abb9
            + (2 * aff14 * aaa2 * arr07 * abb9) / abb8
            - 4 * aff05 * arr12 * aff13
            - 2 * aff05 * arr12
        )
        r2 = (
            (6 * aff05 * aff10 * ass16 * abb9**4) / abb8**4
            + 2 * aff14 * aaa2 * aff15 * ass16 * bmm34 * bmm35
            - 4 * bll16 * abb3 * app10 * bmm34 * bmm35
            - 8 * aff05 * aff10 * ass16 * ass28 * aff17
            + 2 * aff14 * aaa2 * aff15 * ass28 * aff17
            - 3 * app14 * ajj15 * app16 * ass28 * aff17
            + 4 * aff05 * aff10 * ass16 * aff17
            + 2 * aff14 * aaa2 * aff15 * aff17
            - 3 * app14 * ajj15 * app16 * aff17
            - 4 * aff14 * aaa2 * aff15 * ass16 * abb8 * abb9
            + 8 * bll16 * abb3 * app10 * abb8 * abb9
            - 2 * aff14 * aaa2 * aff15 * ass16 * ass23 * abb9
            + 4 * bll16 * abb3 * app10 * ass23 * abb9
            + 4 * aff05 * aff10 * ass16 * aff13
            + 2 * aff14 * aaa2 * aff15 * aff13
            - 3 * app14 * ajj15 * app16 * aff13
            + 2 * aff05 * aff10 * ass16
            - 2 * aff14 * aaa2 * aff15
            + 3 * app14 * ajj15 * app16
        ...
        bss33 = (
            (-4 * aff14 * aaa2 * aff15)
            + 18 * bjj26 * ajj15 * app16
            - (15 * exp1 ** (aaa1 + 6 * abb5) * aaa2**5) / agg15**7
        )
        s2 = (
            (-(6 * aff05 * aff10 * bss29 * abb9**4) / abb8**4)
            - 2 * aff14 * aaa2 * aff15 * bss29 * bmm34 * bmm35
            + 2 * aff05 * aff10 * bss26 * bmm34 * bmm35
            + 8 * app08 * abb3 * app10 * bss25 * bmm34 * bmm35
            + 8 * aff05 * aff10 * bss29 * ass28 * aff17
            + aff14 * aaa2 * aff15 * bss26 * ass28 * aff17
            - 4 * aff14 * aaa2 * aff15 * bss25 * ass28 * aff17
            + 6 * bjj26 * ajj15 * app16 * bss25 * ass28 * aff17
            + 2 * abb4 * abb6 * bss21 * ass28 * aff17
            - 2 * bjj08 * att19 * bjj11 * ass28 * aff17
            - 4 * aff05 * aff10 * bss29 * aff17
            + aff14 * aaa2 * aff15 * bss26 * aff17
            - 4 * aff14 * aaa2 * aff15 * bss25 * aff17
            + 6 * bjj26 * ajj15 * app16 * bss25 * aff17
            + 2 * abb4 * abb6 * bss21 * aff17
            - 2 * bjj08 * att19 * bjj11 * aff17
            + 4 * aff14 * aaa2 * aff15 * bss29 * abb8 * abb9
            - 4 * aff05 * aff10 * bss26 * abb8 * abb9
            - 16 * app08 * abb3 * app10 * bss25 * abb8 * abb9
            - bss33 * abb8 * abb9
            + 2 * aff14 * aaa2 * aff15 * bss29 * ass23 * abb9
            - 2 * aff05 * aff10 * bss26 * ass23 * abb9
            - 8 * app08 * abb3 * app10 * bss25 * ass23 * abb9
            + bss33 * ass23 * abb9
            - 4 * aff05 * aff10 * bss29 * aff13
            + aff14 * aaa2 * aff15 * bss26 * aff13
            - 4 * aff14 * aaa2 * aff15 * bss25 * aff13
            + 6 * bjj26 * ajj15 * app16 * bss25 * aff13
            + 2 * abb4 * abb6 * bss21 * aff13
            - 2 * bjj08 * att19 * bjj11 * aff13
            - 2 * aff05 * aff10 * bss29
            - aff14 * aaa2 * aff15 * bss26
            + 4 * aff14 * aaa2 * aff15 * bss25
            - 6 * bjj26 * ajj15 * app16 * bss25
            - 2 * abb4 * abb6 * bss21
            + 2 * bjj08 * att19 * bjj11
            - (4 * aqq03) / aqq05
            + (44 * exp1 ** ((-4 * t) - 4 * p) * abb3) / aqq05**2
        ...
        btt24 = aaa2**6
        t2 = (
            (-(6 * bjj14 * ajj15 * app10 * abb9**4) / abb8**4)
            + 12 * app21 * abb3 * aff15 * bkk33 * bkk34
            - 12 * bjj24 * att19 * app16 * bkk33 * bkk34
            - 7 * aff05 * aaa2 * aff10 * aqq27 * aff17
            + 8 * bjj14 * ajj15 * app10 * aqq27 * aff17
            + 22 * app08 * ajj15 * app10 * aqq27 * aff17
            - 15 * bjj08 * azz19 * bjj11 * aqq27 * aff17
            - 7 * aff05 * aaa2 * aff10 * aff17
            - 4 * bjj14 * ajj15 * app10 * aff17
            + 22 * app08 * ajj15 * app10 * aff17
            - 15 * bjj08 * azz19 * bjj11 * aff17
            - abb4 * abb6 * abb8 * abb9
            - 24 * app21 * abb3 * aff15 * abb8 * abb9
            + 13 * aff14 * abb3 * aff15 * abb8 * abb9
            - 27 * bjj26 * att19 * app16 * abb8 * abb9
            + 24 * bjj24 * att19 * app16 * abb8 * abb9
            + 15 * bjj18 * btt24 * bjj21 * abb8 * abb9
            + abb4 * abb6 * agg17 * abb9
            - 12 * app21 * abb3 * aff15 * agg17 * abb9
            - 13 * aff14 * abb3 * aff15 * agg17 * abb9
            + 27 * bjj26 * att19 * app16 * agg17 * abb9
            + 12 * bjj24 * att19 * app16 * agg17 * abb9
            - 15 * bjj18 * btt24 * bjj21 * agg17 * abb9
            - 7 * aff05 * aaa2 * aff10 * aff13
            - 4 * bjj14 * ajj15 * app10 * aff13
            + 22 * app08 * ajj15 * app10 * aff13
            - 15 * bjj08 * azz19 * bjj11 * aff13
            + 7 * aff05 * aaa2 * aff10
            - 2 * bjj14 * ajj15 * app10
            - 22 * app08 * ajj15 * app10
            + 15 * bjj08 * azz19 * bjj11
            - (8 * aqq03 * aaa2) / aqq05
        ...
        u2 = (
            (-(6 * app21 * abb3 * aff15 * abb9**4) / abb8**4)
            + 6 * aff05 * aaa2 * aff10 * bkk33 * bkk34
            - 6 * bll16 * ajj15 * app10 * bkk33 * bkk34
            - abb4 * abb6 * aqq27 * aff17
            + 8 * app21 * abb3 * aff15 * aqq27 * aff17
            + 4 * aff14 * abb3 * aff15 * aqq27 * aff17
            - 3 * app14 * att19 * app16 * aqq27 * aff17
            - abb4 * abb6 * aff17
            - 4 * app21 * abb3 * aff15 * aff17
            + 4 * aff14 * abb3 * aff15 * aff17
            - 3 * app14 * att19 * app16 * aff17
            - 12 * aff05 * aaa2 * aff10 * abb8 * abb9
            + 12 * bll16 * ajj15 * app10 * abb8 * abb9
            - 6 * aff05 * aaa2 * aff10 * agg17 * abb9
            + 6 * bll16 * ajj15 * app10 * agg17 * abb9
            - abb4 * abb6 * aff13
            - 4 * app21 * abb3 * aff15 * aff13
            + 4 * aff14 * abb3 * aff15 * aff13
            - 3 * app14 * att19 * app16 * aff13
            + abb4 * abb6
            - 2 * app21 * abb3 * aff15
            - 4 * aff14 * abb3 * aff15
            + 3 * app14 * att19 * app16
        ...
        v2 = (
            (6 * app21 * abb3 * aff15 * avv19 * abb9**4) / abb8**4
            - 6 * aff05 * aaa2 * aff10 * avv19 * bmm34 * bmm35
            + 6 * app08 * ajj15 * app10 * avv19 * bmm34 * bmm35
            - 6 * bjj24 * att19 * app16 * bmm34 * bmm35
            + abb4 * abb6 * avv19 * ass28 * aff17
            - 8 * app21 * abb3 * aff15 * avv19 * ass28 * aff17
            - 4 * aff14 * abb3 * aff15 * avv19 * ass28 * aff17
            + 3 * bjj26 * att19 * app16 * avv19 * ass28 * aff17
            + 12 * app08 * ajj15 * app10 * ass28 * aff17
            - 12 * bjj08 * azz19 * bjj11 * ass28 * aff17
            + abb4 * abb6 * avv19 * aff17
            + 4 * app21 * abb3 * aff15 * avv19 * aff17
            - 4 * aff14 * abb3 * aff15 * avv19 * aff17
            + 3 * bjj26 * att19 * app16 * avv19 * aff17
            + 12 * app08 * ajj15 * app10 * aff17
            - 12 * bjj08 * azz19 * bjj11 * aff17
            + 12 * aff05 * aaa2 * aff10 * avv19 * abb8 * abb9
            - 12 * app08 * ajj15 * app10 * avv19 * abb8 * abb9
            + 9 * aff14 * abb3 * aff15 * abb8 * abb9
            - 24 * bjj26 * att19 * app16 * abb8 * abb9
            + 12 * bjj24 * att19 * app16 * abb8 * abb9
            + 15 * bjj18 * btt24 * bjj21 * abb8 * abb9
            + 6 * aff05 * aaa2 * aff10 * avv19 * ass23 * abb9
            - 6 * app08 * ajj15 * app10 * avv19 * ass23 * abb9
            - 9 * aff14 * abb3 * aff15 * ass23 * abb9
            + 24 * bjj26 * att19 * app16 * ass23 * abb9
            + 6 * bjj24 * att19 * app16 * ass23 * abb9
            - 15 * bjj18 * btt24 * bjj21 * ass23 * abb9
            + abb4 * abb6 * avv19 * aff13
            + 4 * app21 * abb3 * aff15 * avv19 * aff13
            - 4 * aff14 * abb3 * aff15 * avv19 * aff13
            + 3 * bjj26 * att19 * app16 * avv19 * aff13
            + 12 * app08 * ajj15 * app10 * aff13
            - 12 * bjj08 * azz19 * bjj11 * aff13
            - abb4 * abb6 * avv19
            + 2 * app21 * abb3 * aff15 * avv19
            + 4 * aff14 * abb3 * aff15 * avv19
            - 3 * bjj26 * att19 * app16 * avv19
            - 12 * app08 * ajj15 * app10
            + 12 * bjj08 * azz19 * bjj11
            - (8 * aqq03 * aaa2) / aqq05
            + (56 * exp1 ** ((-4 * t) - 4 * p) * ajj15) / aqq05**2
            - (96 * exp1 ** ((-6 * t) - 6 * p) * azz19) / aqq05**3
            + (48 * exp1 ** ((-8 * t) - 8 * p) * aaa2**7) / aqq05**4
        ...
        w2 = (
            (-(6 * aff05 * aaa2 * aff10 * abb9**4) / abb8**4)
            + 2 * abb4 * abb6 * bkk33 * bkk34
            - 2 * aff14 * abb3 * aff15 * bkk33 * bkk34
            + (8 * aff05 * aaa2 * aff10 * aff17) / aff13
            - 4 * aff05 * aaa2 * aff10 * aff17
            - 4 * abb4 * abb6 * abb8 * abb9
            + 4 * aff14 * abb3 * aff15 * abb8 * abb9
            - 2 * abb4 * abb6 * agg17 * abb9
            + 2 * aff14 * abb3 * aff15 * agg17 * abb9
            - 4 * aff05 * aaa2 * aff10 * aff13
            - 2 * aff05 * aaa2 * aff10
        )
        x2 = (
            (6 * aff05 * aaa2 * aff10 * avv19 * abb9**4) / abb8**4
            - 2 * abb4 * abb6 * avv19 * bmm34 * bmm35
            + 2 * aff14 * abb3 * aff15 * avv19 * bmm34 * bmm35
            - 4 * bll16 * ajj15 * app10 * bmm34 * bmm35
            - 8 * aff05 * aaa2 * aff10 * avv19 * ass28 * aff17
            + 3 * aff14 * abb3 * aff15 * ass28 * aff17
            - 3 * app14 * att19 * app16 * ass28 * aff17
            + 4 * aff05 * aaa2 * aff10 * avv19 * aff17
            + 3 * aff14 * abb3 * aff15 * aff17
            - 3 * app14 * att19 * app16 * aff17
            + 4 * abb4 * abb6 * avv19 * abb8 * abb9
            - 4 * aff14 * abb3 * aff15 * avv19 * abb8 * abb9
            + 8 * bll16 * ajj15 * app10 * abb8 * abb9
            + 2 * abb4 * abb6 * avv19 * ass23 * abb9
            - 2 * aff14 * abb3 * aff15 * avv19 * ass23 * abb9
            + 4 * bll16 * ajj15 * app10 * ass23 * abb9
            + 4 * aff05 * aaa2 * aff10 * avv19 * aff13
            + 3 * aff14 * abb3 * aff15 * aff13
            - 3 * app14 * att19 * app16 * aff13
            + 2 * aff05 * aaa2 * aff10 * avv19
            - 3 * aff14 * abb3 * aff15
            + 3 * app14 * att19 * app16
        )
        ...
        byy35 = (
            (-6 * aff14 * abb3 * aff15)
            + 21 * bjj26 * att19 * app16
            - (15 * exp1 ** (aaa1 + 6 * abb5) * aaa2**6) / agg15**7
        )
        y2 = (
            (-(6 * aff05 * aaa2 * aff10 * bss29 * abb9**4) / abb8**4)
            + 2 * abb4 * abb6 * bss29 * bmm34 * bmm35
            - 2 * aff14 * abb3 * aff15 * bss29 * bmm34 * bmm35
            + 2 * aff05 * aaa2 * aff10 * bss26 * bmm34 * bmm35
            + 8 * app08 * ajj15 * app10 * bss25 * bmm34 * bmm35
            + 8 * aff05 * aaa2 * aff10 * bss29 * ass28 * aff17
            - abb4 * abb6 * bss26 * ass28 * aff17
            + aff14 * abb3 * aff15 * bss26 * ass28 * aff17
            - 6 * aff14 * abb3 * aff15 * bss25 * ass28 * aff17
            + 6 * bjj26 * att19 * app16 * bss25 * ass28 * aff17
            + abb4 * abb6 * byy24 * ass28 * aff17
            + abb4 * aaa2 * abb6 * bss21 * ass28 * aff17
            - 2 * bjj08 * azz19 * bjj11 * ass28 * aff17
            - 4 * aff05 * aaa2 * aff10 * bss29 * aff17
            - abb4 * abb6 * bss26 * aff17
            + aff14 * abb3 * aff15 * bss26 * aff17
            - 6 * aff14 * abb3 * aff15 * bss25 * aff17
            + 6 * bjj26 * att19 * app16 * bss25 * aff17
            + abb4 * abb6 * byy24 * aff17
            + abb4 * aaa2 * abb6 * bss21 * aff17
            - 2 * bjj08 * azz19 * bjj11 * aff17
            - 4 * abb4 * abb6 * bss29 * abb8 * abb9
            + 4 * aff14 * abb3 * aff15 * bss29 * abb8 * abb9
            - 4 * aff05 * aaa2 * aff10 * bss26 * abb8 * abb9
            - 16 * app08 * ajj15 * app10 * bss25 * abb8 * abb9
            - byy35 * abb8 * abb9
            - 2 * abb4 * abb6 * bss29 * ass23 * abb9
            + 2 * aff14 * abb3 * aff15 * bss29 * ass23 * abb9
            - 2 * aff05 * aaa2 * aff10 * bss26 * ass23 * abb9
            - 8 * app08 * ajj15 * app10 * bss25 * ass23 * abb9
            + byy35 * ass23 * abb9
            - 4 * aff05 * aaa2 * aff10 * bss29 * aff13
            - abb4 * abb6 * bss26 * aff13
            + aff14 * abb3 * aff15 * bss26 * aff13
            - 6 * aff14 * abb3 * aff15 * bss25 * aff13
            + 6 * bjj26 * att19 * app16 * bss25 * aff13
            + abb4 * abb6 * byy24 * aff13
            + abb4 * aaa2 * abb6 * bss21 * aff13
            - 2 * bjj08 * azz19 * bjj11 * aff13
            - 2 * aff05 * aaa2 * aff10 * bss29
            + abb4 * abb6 * bss26
            - aff14 * abb3 * aff15 * bss26
            + 6 * aff14 * abb3 * aff15 * bss25
            - 6 * bjj26 * att19 * app16 * bss25
            - abb4 * abb6 * byy24
            - abb4 * aaa2 * abb6 * bss21
            + 2 * bjj08 * azz19 * bjj11
            - (8 * aqq03 * aaa2) / aqq05
            + (56 * exp1 ** ((-4 * t) - 4 * p) * ajj15) / aqq05**2
            - (96 * exp1 ** ((-6 * t) - 6 * p) * azz19) / aqq05**3
        ...
        a3 = (
            ...
            - (8 * abb4 * abb6 * aii12 * aff17) / aff13
            + 4 * abb4 * abb6 * aii12 * aff17
            + 4 * aff14 * abb3 * aii17 * abb8 * abb9
            + (2 * aff14 * abb3 * aii17 * abb9) / abb8
            + 4 * abb4 * abb6 * aii12 * aff13
            + 2 * abb4 * abb6 * aii12
        )
        cbb09 = 1 / agg15**5
        ...
        b3 = (
            (-(6 * abb4 * abb6 * ayy24 * abb9**4) / abb8**4)
            + 2 * abb4 * abb6 * cbb24 * bmm34 * bmm35
            + 4 * aff14 * abb3 * aii17 * ayy16 * bmm34 * bmm35
            + 8 * abb4 * abb6 * ayy24 * ass28 * aff17
            + cbb18 * ass28 * aff17
            - 4 * abb4 * abb6 * ayy24 * aff17
            + cbb18 * aff17
            - 4 * abb4 * abb6 * cbb24 * abb8 * abb9
            - 8 * aff14 * abb3 * aii17 * ayy16 * abb8 * abb9
            - 2 * abb4 * abb6 * cbb24 * ass23 * abb9
            - 4 * aff14 * abb3 * aii17 * ayy16 * ass23 * abb9
            - 4 * abb4 * abb6 * ayy24 * aff13
            + cbb18 * aff13
            - 2 * abb4 * abb6 * ayy24
            - 2 * aff14 * abb3 * aii17
            + 3 * app14 * att19 * cbb09
        ...
        ccc28 = (
            (-4 * aff14 * abb3 * aii17)
            + 18 * app14 * att19 * cbb09
            - (15 * exp1 ** (aaa1 + 6 * abb5) * aaa2**6) / agg15**7
        )
        c3 = (
            (6 * abb4 * abb6 * ccc24 * abb9**4) / abb8**4
            - 6 * aff14 * abb3 * aii17 * ayy24 * bmm34 * bmm35
            - 6 * abb4 * abb6 * ayy16 * ayy17 * bmm34 * bmm35
            - 8 * abb4 * abb6 * ccc24 * ass28 * aff17
            + abb4 * abb6 * ccc23 * ass28 * aff17
            + 3 * aff14 * abb3 * aii17 * ayy17 * ass28 * aff17
            - 3 * cbb18 * ayy16 * ass28 * aff17
            + 4 * abb4 * abb6 * ccc24 * aff17
            + abb4 * abb6 * ccc23 * aff17
            + 3 * aff14 * abb3 * aii17 * ayy17 * aff17
            - 3 * cbb18 * ayy16 * aff17
            + 12 * aff14 * abb3 * aii17 * ayy24 * abb8 * abb9
            + 12 * abb4 * abb6 * ayy16 * ayy17 * abb8 * abb9
            - ccc28 * abb8 * abb9
            + 6 * aff14 * abb3 * aii17 * ayy24 * ass23 * abb9
            + 6 * abb4 * abb6 * ayy16 * ayy17 * ass23 * abb9
            + ccc28 * ass23 * abb9
            + 4 * abb4 * abb6 * ccc24 * aff13
            + abb4 * abb6 * ccc23 * aff13
            + 3 * aff14 * abb3 * aii17 * ayy17 * aff13
            - 3 * cbb18 * ayy16 * aff13
            + 2 * abb4 * abb6 * ccc24
            - abb4 * abb6 * ccc23
            - 3 * aff14 * abb3 * aii17 * ayy17
            + 3 * cbb18 * ayy16
            - (8 * abb1 * aaa2) / aff04
        ...
        cdd24 = aaa2**7
        d3 = (
            (-(6 * bjj14 * att19 * app10 * abb9**4) / abb8**4)
            + 12 * app21 * ajj15 * aff15 * bkk33 * bkk34
            - 12 * bjj24 * azz19 * app16 * bkk33 * bkk34
            - 7 * aff05 * abb3 * aff10 * aqq27 * aff17
            + 8 * bjj14 * att19 * app10 * aqq27 * aff17
            + 22 * app08 * att19 * app10 * aqq27 * aff17
            - 15 * bjj08 * btt24 * bjj11 * aqq27 * aff17
            - 7 * aff05 * abb3 * aff10 * aff17
            - 4 * bjj14 * att19 * app10 * aff17
            + 22 * app08 * att19 * app10 * aff17
            - 15 * bjj08 * btt24 * bjj11 * aff17
            - abb4 * aaa2 * abb6 * abb8 * abb9
            - 24 * app21 * ajj15 * aff15 * abb8 * abb9
            + 13 * aff14 * ajj15 * aff15 * abb8 * abb9
            - 27 * bjj26 * azz19 * app16 * abb8 * abb9
            + 24 * bjj24 * azz19 * app16 * abb8 * abb9
            + 15 * bjj18 * cdd24 * bjj21 * abb8 * abb9
            + abb4 * aaa2 * abb6 * agg17 * abb9
            - 12 * app21 * ajj15 * aff15 * agg17 * abb9
            - 13 * aff14 * ajj15 * aff15 * agg17 * abb9
            + 27 * bjj26 * azz19 * app16 * agg17 * abb9
            + 12 * bjj24 * azz19 * app16 * agg17 * abb9
            - 15 * bjj18 * cdd24 * bjj21 * agg17 * abb9
            - 7 * aff05 * abb3 * aff10 * aff13
            - 4 * bjj14 * att19 * app10 * aff13
            + 22 * app08 * att19 * app10 * aff13
            - 15 * bjj08 * btt24 * bjj11 * aff13
            + 7 * aff05 * abb3 * aff10
            - 2 * bjj14 * att19 * app10
            - 22 * app08 * att19 * app10
            + 15 * bjj08 * btt24 * bjj11
            - (8 * aqq03 * abb3) / aqq05
        ...
        e3 = (
            (-(6 * app21 * ajj15 * aff15 * abb9**4) / abb8**4)
            + 6 * aff05 * abb3 * aff10 * bkk33 * bkk34
            - 6 * bll16 * att19 * app10 * bkk33 * bkk34
            - abb4 * aaa2 * abb6 * aqq27 * aff17
            + 8 * app21 * ajj15 * aff15 * aqq27 * aff17
            + 4 * aff14 * ajj15 * aff15 * aqq27 * aff17
            - 3 * app14 * azz19 * app16 * aqq27 * aff17
            - abb4 * aaa2 * abb6 * aff17
            - 4 * app21 * ajj15 * aff15 * aff17
            + 4 * aff14 * ajj15 * aff15 * aff17
            - 3 * app14 * azz19 * app16 * aff17
            - 12 * aff05 * abb3 * aff10 * abb8 * abb9
            + 12 * bll16 * att19 * app10 * abb8 * abb9
            - 6 * aff05 * abb3 * aff10 * agg17 * abb9
            + 6 * bll16 * att19 * app10 * agg17 * abb9
            - abb4 * aaa2 * abb6 * aff13
            - 4 * app21 * ajj15 * aff15 * aff13
            + 4 * aff14 * ajj15 * aff15 * aff13
            - 3 * app14 * azz19 * app16 * aff13
            + abb4 * aaa2 * abb6
            - 2 * app21 * ajj15 * aff15
            - 4 * aff14 * ajj15 * aff15
            + 3 * app14 * azz19 * app16
        ...
        f3 = (
            (6 * app21 * ajj15 * aff15 * avv19 * abb9**4) / abb8**4
            - 6 * aff05 * abb3 * aff10 * avv19 * bmm34 * bmm35
            + 6 * app08 * att19 * app10 * avv19 * bmm34 * bmm35
            - 6 * bjj24 * azz19 * app16 * bmm34 * bmm35
            + abb4 * aaa2 * abb6 * avv19 * ass28 * aff17
            - 8 * app21 * ajj15 * aff15 * avv19 * ass28 * aff17
            - 4 * aff14 * ajj15 * aff15 * avv19 * ass28 * aff17
            + 3 * bjj26 * azz19 * app16 * avv19 * ass28 * aff17
            + 12 * app08 * att19 * app10 * ass28 * aff17
            - 12 * bjj08 * btt24 * bjj11 * ass28 * aff17
            + abb4 * aaa2 * abb6 * avv19 * aff17
            + 4 * app21 * ajj15 * aff15 * avv19 * aff17
            - 4 * aff14 * ajj15 * aff15 * avv19 * aff17
            + 3 * bjj26 * azz19 * app16 * avv19 * aff17
            + 12 * app08 * att19 * app10 * aff17
            - 12 * bjj08 * btt24 * bjj11 * aff17
            + 12 * aff05 * abb3 * aff10 * avv19 * abb8 * abb9
            - 12 * app08 * att19 * app10 * avv19 * abb8 * abb9
            + 9 * aff14 * ajj15 * aff15 * abb8 * abb9
            - 24 * bjj26 * azz19 * app16 * abb8 * abb9
            + 12 * bjj24 * azz19 * app16 * abb8 * abb9
            + 15 * bjj18 * cdd24 * bjj21 * abb8 * abb9
            + 6 * aff05 * abb3 * aff10 * avv19 * ass23 * abb9
            - 6 * app08 * att19 * app10 * avv19 * ass23 * abb9
            - 9 * aff14 * ajj15 * aff15 * ass23 * abb9
            + 24 * bjj26 * azz19 * app16 * ass23 * abb9
            + 6 * bjj24 * azz19 * app16 * ass23 * abb9
            - 15 * bjj18 * cdd24 * bjj21 * ass23 * abb9
            + abb4 * aaa2 * abb6 * avv19 * aff13
            + 4 * app21 * ajj15 * aff15 * avv19 * aff13
            - 4 * aff14 * ajj15 * aff15 * avv19 * aff13
            + 3 * bjj26 * azz19 * app16 * avv19 * aff13
            + 12 * app08 * att19 * app10 * aff13
            - 12 * bjj08 * btt24 * bjj11 * aff13
            - abb4 * aaa2 * abb6 * avv19
            + 2 * app21 * ajj15 * aff15 * avv19
            + 4 * aff14 * ajj15 * aff15 * avv19
            - 3 * bjj26 * azz19 * app16 * avv19
            - 12 * app08 * att19 * app10
            + 12 * bjj08 * btt24 * bjj11
            - (8 * aqq03 * abb3) / aqq05
            + (56 * exp1 ** ((-4 * t) - 4 * p) * att19) / aqq05**2
            - (96 * exp1 ** ((-6 * t) - 6 * p) * btt24) / aqq05**3
            + (48 * exp1 ** ((-8 * t) - 8 * p) * aaa2**8) / aqq05**4
        ...
        g3 = (
            (-(6 * aff05 * abb3 * aff10 * abb9**4) / abb8**4)
            + 2 * abb4 * aaa2 * abb6 * bkk33 * bkk34
            - 2 * aff14 * ajj15 * aff15 * bkk33 * bkk34
            + (8 * aff05 * abb3 * aff10 * aff17) / aff13
            - 4 * aff05 * abb3 * aff10 * aff17
            - 4 * abb4 * aaa2 * abb6 * abb8 * abb9
            + 4 * aff14 * ajj15 * aff15 * abb8 * abb9
            - 2 * abb4 * aaa2 * abb6 * agg17 * abb9
            + 2 * aff14 * ajj15 * aff15 * agg17 * abb9
            - 4 * aff05 * abb3 * aff10 * aff13
            - 2 * aff05 * abb3 * aff10
        )
        h3 = (
            (6 * aff05 * abb3 * aff10 * avv19 * abb9**4) / abb8**4
            - 2 * abb4 * aaa2 * abb6 * avv19 * bmm34 * bmm35
            + 2 * aff14 * ajj15 * aff15 * avv19 * bmm34 * bmm35
            - 4 * bll16 * att19 * app10 * bmm34 * bmm35
            - 8 * aff05 * abb3 * aff10 * avv19 * ass28 * aff17
            + 3 * aff14 * ajj15 * aff15 * ass28 * aff17
            - 3 * app14 * azz19 * app16 * ass28 * aff17
            + 4 * aff05 * abb3 * aff10 * avv19 * aff17
            + 3 * aff14 * ajj15 * aff15 * aff17
            - 3 * app14 * azz19 * app16 * aff17
            + 4 * abb4 * aaa2 * abb6 * avv19 * abb8 * abb9
            - 4 * aff14 * ajj15 * aff15 * avv19 * abb8 * abb9
            + 8 * bll16 * att19 * app10 * abb8 * abb9
            + 2 * abb4 * aaa2 * abb6 * avv19 * ass23 * abb9
            - 2 * aff14 * ajj15 * aff15 * avv19 * ass23 * abb9
            + 4 * bll16 * att19 * app10 * ass23 * abb9
            + 4 * aff05 * abb3 * aff10 * avv19 * aff13
            + 3 * aff14 * ajj15 * aff15 * aff13
            - 3 * app14 * azz19 * app16 * aff13
            + 2 * aff05 * abb3 * aff10 * avv19
            - 3 * aff14 * ajj15 * aff15
            + 3 * app14 * azz19 * app16
        )
        i3 = (
            (-(6 * aff05 * abb3 * aff10 * bss29 * abb9**4) / abb8**4)
            + 2 * abb4 * aaa2 * abb6 * bss29 * bmm34 * bmm35
            - 2 * aff14 * ajj15 * aff15 * bss29 * bmm34 * bmm35
            + 2 * aff05 * abb3 * aff10 * bss26 * bmm34 * bmm35
            + 8 * app08 * att19 * app10 * bss25 * bmm34 * bmm35
            + 8 * aff05 * abb3 * aff10 * bss29 * ass28 * aff17
            - abb4 * aaa2 * abb6 * bss26 * ass28 * aff17
            + aff14 * ajj15 * aff15 * bss26 * ass28 * aff17
            - 6 * aff14 * ajj15 * aff15 * bss25 * ass28 * aff17
            + 6 * bjj26 * azz19 * app16 * bss25 * ass28 * aff17
            + 4 * app08 * att19 * app10 * ass28 * aff17
            - 8 * bjj08 * btt24 * bjj11 * ass28 * aff17
            - 4 * aff05 * abb3 * aff10 * bss29 * aff17
            - abb4 * aaa2 * abb6 * bss26 * aff17
            + aff14 * ajj15 * aff15 * bss26 * aff17
            - 6 * aff14 * ajj15 * aff15 * bss25 * aff17
            + 6 * bjj26 * azz19 * app16 * bss25 * aff17
            + 4 * app08 * att19 * app10 * aff17
            - 8 * bjj08 * btt24 * bjj11 * aff17
            - 4 * abb4 * aaa2 * abb6 * bss29 * abb8 * abb9
            + 4 * aff14 * ajj15 * aff15 * bss29 * abb8 * abb9
            - 4 * aff05 * abb3 * aff10 * bss26 * abb8 * abb9
            - 16 * app08 * att19 * app10 * bss25 * abb8 * abb9
            + 6 * aff14 * ajj15 * aff15 * abb8 * abb9
            - 21 * bjj26 * azz19 * app16 * abb8 * abb9
            + 15 * bjj18 * cdd24 * bjj21 * abb8 * abb9
            - 2 * abb4 * aaa2 * abb6 * bss29 * ass23 * abb9
            + 2 * aff14 * ajj15 * aff15 * bss29 * ass23 * abb9
            - 2 * aff05 * abb3 * aff10 * bss26 * ass23 * abb9
            - 8 * app08 * att19 * app10 * bss25 * ass23 * abb9
            - 6 * aff14 * ajj15 * aff15 * ass23 * abb9
            + 21 * bjj26 * azz19 * app16 * ass23 * abb9
            - 15 * bjj18 * cdd24 * bjj21 * ass23 * abb9
            - 4 * aff05 * abb3 * aff10 * bss29 * aff13
            - abb4 * aaa2 * abb6 * bss26 * aff13
            + aff14 * ajj15 * aff15 * bss26 * aff13
            - 6 * aff14 * ajj15 * aff15 * bss25 * aff13
            + 6 * bjj26 * azz19 * app16 * bss25 * aff13
            + 4 * app08 * att19 * app10 * aff13
            - 8 * bjj08 * btt24 * bjj11 * aff13
            - 2 * aff05 * abb3 * aff10 * bss29
            + abb4 * aaa2 * abb6 * bss26
            - aff14 * ajj15 * aff15 * bss26
            + 6 * aff14 * ajj15 * aff15 * bss25
            - 6 * bjj26 * azz19 * app16 * bss25
            - 4 * app08 * att19 * app10
            + 8 * bjj08 * btt24 * bjj11
            - (8 * aqq03 * abb3) / aqq05
            + (56 * exp1 ** ((-4 * t) - 4 * p) * att19) / aqq05**2
        ...
        j3 = (
            ...
            + (8 * abb4 * aaa2 * abb6 * bzz9) / bzz7
            - 4 * abb4 * aaa2 * abb6 * bzz9
            - 4 * abb4 * aaa2 * abb6 * bzz7
            - 2 * abb4 * aaa2 * abb6
        )
        k3 = (
            (6 * abb4 * aaa2 * bdd14 * bdd15 * abb9**4) / abb8**4
            - (2 * aff14 * ajj15 * bdd08 * abb9**3) / abb8**3
            - (8 * abb4 * aaa2 * bdd14 * bdd15 * aff17) / aff13
            + 4 * abb4 * aaa2 * bdd14 * bdd15 * aff17
            + 4 * aff14 * ajj15 * bdd08 * abb8 * abb9
            + (2 * aff14 * ajj15 * bdd08 * abb9) / abb8
            + 4 * abb4 * aaa2 * bdd14 * bdd15 * aff13
            + 2 * abb4 * aaa2 * bdd14 * bdd15
        )
        cll08 = 1 / bdd07**5
        cll16 = aii11 + bhh13
        cll17 = cll16**2
        ...
        l3 = (
            (-(6 * abb4 * aaa2 * bdd14 * cll17 * abb9**4) / abb8**4)
            + 2 * abb4 * aaa2 * bdd14 * cll24 * bmm34 * bmm35
            + 4 * aff14 * ajj15 * bdd08 * cll16 * bmm34 * bmm35
            + 8 * abb4 * aaa2 * bdd14 * cll17 * ass28 * aff17
            + cll18 * ass28 * aff17
            - 4 * abb4 * aaa2 * bdd14 * cll17 * aff17
            + cll18 * aff17
            - 4 * abb4 * aaa2 * bdd14 * cll24 * abb8 * abb9
            - 8 * aff14 * ajj15 * bdd08 * cll16 * abb8 * abb9
            - 2 * abb4 * aaa2 * bdd14 * cll24 * ass23 * abb9
            - 4 * aff14 * ajj15 * bdd08 * cll16 * ass23 * abb9
            - 4 * abb4 * aaa2 * bdd14 * cll17 * aff13
            + cll18 * aff13
            - 2 * abb4 * aaa2 * bdd14 * cll17
            - 2 * aff14 * ajj15 * bdd08
            + 3 * app14 * azz19 * cll08
        )
        ...
        cmm28 = (
            (-4 * aff14 * ajj15 * aii17)
            + 18 * app14 * azz19 * cbb09
            - (15 * exp1 ** (aaa1 + 6 * abb5) * aaa2**7) / agg15**7
        )
        m3 = (
            (6 * abb4 * aaa2 * abb6 * ccc24 * abb9**4) / abb8**4
            - 6 * aff14 * ajj15 * aii17 * ayy24 * bmm34 * bmm35
            - 6 * abb4 * aaa2 * abb6 * ayy16 * ayy17 * bmm34 * bmm35
            - 8 * abb4 * aaa2 * abb6 * ccc24 * ass28 * aff17
            + abb4 * aaa2 * abb6 * cmm23 * ass28 * aff17
            + 3 * aff14 * ajj15 * aii17 * ayy17 * ass28 * aff17
            - 3 * cmm16 * ayy16 * ass28 * aff17
            + 4 * abb4 * aaa2 * abb6 * ccc24 * aff17
            + abb4 * aaa2 * abb6 * cmm23 * aff17
            + 3 * aff14 * ajj15 * aii17 * ayy17 * aff17
            - 3 * cmm16 * ayy16 * aff17
            + 12 * aff14 * ajj15 * aii17 * ayy24 * abb8 * abb9
            + 12 * abb4 * aaa2 * abb6 * ayy16 * ayy17 * abb8 * abb9
            - cmm28 * abb8 * abb9
            + 6 * aff14 * ajj15 * aii17 * ayy24 * ass23 * abb9
            + 6 * abb4 * aaa2 * abb6 * ayy16 * ayy17 * ass23 * abb9
            + cmm28 * ass23 * abb9
            + 4 * abb4 * aaa2 * abb6 * ccc24 * aff13
            + abb4 * aaa2 * abb6 * cmm23 * aff13
            + 3 * aff14 * ajj15 * aii17 * ayy17 * aff13
            - 3 * cmm16 * ayy16 * aff13
            + 2 * abb4 * aaa2 * abb6 * ccc24
            - abb4 * aaa2 * abb6 * cmm23
            - 3 * aff14 * ajj15 * aii17 * ayy17
            + 3 * cmm16 * ayy16
            - (8 * abb1 * abb3) / aff04
        ...
        p3 = (
            (-(6 * cpp08 * abb9**4) / abb8**4)
            + (2 * cpp12 * abb9**3) / abb8**3
            + (8 * cpp08 * aff17) / aff13
            - 4 * cpp08 * aff17
            - 4 * cpp12 * abb8 * abb9
            - (2 * cpp12 * abb9) / abb8
        ...
        q3 = (
            ...
            - (6 * cqq19 * cqq25 * abb9**3) / abb8**3
            - 8 * cqq20 * cqq28 * aff17
            + cqq21 * cqq28 * aff17
            + 4 * cqq20 * aff17
            + cqq21 * aff17
            + 12 * cqq19 * cqq25 * abb8 * abb9
            + (6 * cqq19 * cqq25 * abb9) / abb8
            + 4 * cqq20 * aff13
            + cqq21 * aff13
            + 2 * cqq20
            - ann05 * ann06
        ...
        crr25 = (
            ...
            + aoo09
            - 3 * aff14 * ajj15 * aii17
            + 15 * app14 * azz19 * cbb09
            - (15 * exp1 ** (aaa1 + 6 * abb5) * aaa2**7) / agg15**7
        )
        crr28 = bii11**2
        r3 = (
            (-(6 * crr19 * abb9**4) / abb8**4)
            + (12 * crr28 * bii15 * abb9**3) / abb8**3
            - 3 * crr21 * ass28 * aff17
            + 8 * crr19 * ass28 * aff17
            - 4 * bii11 * crr18 * ass28 * aff17
            - 3 * crr21 * aff17
            - 4 * crr19 * aff17
            - 4 * bii11 * crr18 * aff17
            - 24 * crr28 * bii15 * abb8 * abb9
            - crr25 * abb8 * abb9
            - 12 * crr28 * bii15 * ass23 * abb9
            + crr25 * ass23 * abb9
            - 3 * crr21 * aff13
            - 4 * crr19 * aff13
            - 4 * bii11 * crr18 * aff13
...
class Tweedie(Family):
    ...
    def d2var(self, mu):
        return self.p * (self.p - 1.0) * np.asarray(mu, dtype=float) ** (self.p - 2.0)
    ...
    def dev_resids(self, y, mu, wt, theta=None):
        ...
        d_pos = 2.0 * (y * (y_safe**om1 - mu**om1) / om1 - (y_safe**tm - mu**tm) / tm)
        d_zero = 2.0 * mu**tm / tm
    ...
    def initialize(self, y, wt):
        y = np.asarray(y, dtype=float)
        if np.any(y < 0):
            raise ValueError("negative values not allowed for the 'Tweedie' family")
    ...
    def ls(self, y, wt, scale):
        ...
        if np.any(~zero):
            la_, jb_, jv_ = self._saturated_series(y_g[~zero], phi_i[~zero])[:3]
            log_a[~zero] = la_
            j_bar[~zero] = jb_
            j_var[~zero] = jv_

        # log f_sat per observation; y=0 row is 0 by the closed form.
        log_f_sat = np.where(zero, 0.0, -np.log(y_safe) + log_a + cum / phi_i)
        ls0 = float(np.sum(w_g * log_f_sat))

        d1_per = np.where(zero, 0.0, -one_minus_alpha * j_bar - cum / phi_i)
        d2_per = np.where(
            zero, 0.0, one_minus_alpha * one_minus_alpha * j_var + cum / phi_i
        )
    ...
    def dD_dp(self, y, mu, wt):
        ...
        dA1 = y * (mu_om1 * log_mu - y_om1 * log_y) / om1 + y * u / (om1 * om1)
        # ∂[v/tm]/∂p:    ∂v/∂p / tm + v/tm²
        #   ∂v/∂p = -y^tm·log y + μ^tm·log μ
        dA2 = (mu_tm * log_mu - y_tm * log_y) / tm + v / (tm * tm)
    ...
    def dls_dp(self, y, wt, scale):
        ...
        y_tm = y_safe**tm
        dcum_dp = np.where(
            zero, 0.0, y_tm * ((3.0 - 2.0 * p) - log_y * om1_tm) / (om1_tm * om1_tm)
        )
        ...
        if np.any(~zero):
            _, jb_, _, jpb_, *_rest2 = self._saturated_series(y_g[~zero], phi_i[~zero])
            j_bar[~zero] = jb_
            j_psi_bar[~zero] = jpb_
    ...
    def _d2ls_dp(self, y, wt, scale):
        ...
        d2p_dens = (
            th_y * (L * L - 2.0 * L / om1 + 2.0 / (om1 * om1))
            - k_y * (L * L - 2.0 * L / tm + 2.0 / (tm * tm))
        ) / phi_i
        ...
        return (float(np.sum(w_g * d2p)), float(np.sum(w_g * cross)))
...
class tw(Tweedie):
    ...
    def __init__(
        self, theta: float | None = None, link=None, a: float = 1.01, b: float = 1.99
    ):
        if not (1.0 <= a < b <= 2.0):
            raise ValueError(f"tw() requires 1 ≤ a < b ≤ 2; got a={a!r}, b={b!r}")
        self.a = float(a)
        self.b = float(b)
        if theta is None:
    ...
    def set_theta(self, theta) -> None:
        ...
        if hasattr(theta, "__len__"):
            if len(theta) != 1:
                raise ValueError(f"tw expects a single theta; got length {len(theta)}")
            theta = theta[0]
    ...
    def aic(self, y, mu, dev, wt, n, theta=None):
        ...
        if theta is None:
            ...
            a, b = self.a, self.b
            p = (
                (b + a * math.exp(-th)) / (1.0 + math.exp(-th))
                if th > 0
    ...
    def ls_extended(self, y, wt, theta=None, scale: float = 1.0) -> dict:
        ...
        try:
            ...
            ld = _ld_tweedie_work(
                yv,
                yv,
                np.full(yv.shape, self.theta),
                np.full(yv.shape, math.log(scale)),
                self.a,
                self.b,
            )
            Ls = wv[:, None] * ld
            LS = np.array([_rsum(Ls[:, j]) for j in range(6)])
            lsth2 = np.array([[LS[4], LS[5]], [LS[5], LS[2]]])
            return {
                "ls": float(LS[0]),
    ...
    def Dd(self, y, mu, theta, wt, level: int = 0) -> dict:
        ...
        if th > 0:
            ...
            d2 = np.exp(-th) + 1
            dpth2 = ((a - b) * np.exp(-th) + (b - a) * np.exp(-2 * th)) / (d2 * d2 * d2)
        else:
            p = (b * np.exp(th) + a) / (np.exp(th) + 1)
            d1 = np.exp(th) + 1
            dpth1 = np.exp(th) * (b - a) / (d1 * d1)
            d2 = np.exp(th) + 1
            dpth2 = ((a - b) * np.exp(2 * th) + (b - a) * np.exp(th)) / (d2 * d2 * d2)
        mu1p = mu ** (1 - p)
        mup = mu**p
        ...
        if level > 1:
            ...
            r["Dmuth2"] = (
                2 * wt * ((mu1p * logmu2 - logmu2 * ymupi) * dpth1_2)
                + r["Dmuth"] * dpth2 / dpth1
            )
            r["Dmu2th2"] = (
                2
                ...
                * (
                    (
                        mup1 * logmu * y * (logmu * p - 2)
                        + logmu / mup * (logmu * (1 - p) + 2)
                    )
                    * dpth1_2
                )
    ...
    def __repr__(self):
        return f"tw(p={self.p:.4g}, link={self.link.name}, a={self.a!r}, b={self.b!r})"
...
class Scat(Family):
    ...
    def __init__(self, theta=None, link: str = "identity", min_df: float = 3.0):
        ...
        if theta is not None and not np.any(np.asarray(theta) == 0.0):
            ...
            if t.shape != (2,):
                raise ValueError(
                    f"scat theta must be a length-2 array (ν, σ); got shape {t.shape}"
                )
            if abs(t[0]) <= min_df:
                import warnings
            ...
            if np.any(t < 0):
                ini = np.array(
                    [np.log(abs(t[0]) - min_df), np.log(abs(t[1]))], dtype=float
                )
            else:
                ini = np.array([np.log(t[0] - min_df), np.log(t[1])], dtype=float)
                n_theta = 0
        else:
    ...
    def set_theta(self, values) -> None:
        ...
        if v.shape != (2,):
            raise ValueError(
                f"Scat.set_theta expects length-2 array (log θ); got shape {v.shape}"
            )
        self._theta = v.copy()
    ...
    def variance(self, mu):
        ...
        return np.full(np.shape(mu), sig * sig * nu / max(nu - 2.0, 1e-10), dtype=float)
    ...
    def ls_extended(self, y, wt, theta=None, scale: float = 1.0) -> dict:
        ...
        term0 = gammaln(nu12) - gammaln(nu / 2.0) - np.log(sig * np.sqrt(np.pi * nu))
        ...
        col0 = nu2 * digamma(nu12) / 2.0 - nu2 * digamma(nu / 2.0) / 2.0 - 0.5 * nu2nu
    ...
    def Dd(self, y, mu, theta, wt, level: int = 0) -> dict:
        ...
        if level > 0:
            ...
            Dmu2th[:, 1] = 4.0 * w * (-nu1nusig2a + ff1 * 5.0 - 4.0 * ff1 * f1ym)
            EDmu3 = np.zeros(n, dtype=float)
            EDmu2th[:, 0] = (
                4.0 / (sig * sig * (nu + 3.0) ** 2) * np.float64(np.exp(th[0]))
            )
        ...
        if level > 1:
            ...
            Dmu4 = (
                12.0
                * w
                * (-nu1nusig2a / nusig2a + 8.0 * ff1 / nusig2a - 8.0 * ff1 * f1**2)
            )
            n2d = 3
            ...
            Dmu3th[:, 1] = (
                48.0 * w * f * (-1.0 / nusig2a + 3.0 * f1**2 - 2.0 * f1ymf1 * f1)
            )
    ...
    def preinitialize(self, y) -> dict | None:
        ...
        if self.n_theta > 0:
            ...
            return {"Theta": np.array([1.5, np.log(0.8 * sd_y)], dtype=float)}
        return None
...
class nb(Family):
    ...
    def Dd(self, y, mu, theta, wt, level: int = 0) -> dict:
        ...
        if level > 0:
            r["Dth"] = -2.0 * wt * Th * (np.log(yth / muth) + (1.0 - yth / muth))
            ...
            r["Dmu3"] = 4.0 * wt * (yth / _rpow_int(muth, 3) - y / _rpow_int(mu, 3))
            r["Dmu2th"] = 2.0 * wt * Th * (2.0 * yth / muth - 1.0) / muth**2
            r["EDmu2th"] = 2.0 * wt / muth**2
        if level > 1:
            r["Dmu4"] = (
                2.0 * wt * (6.0 * y / _rpow_int(mu, 4) - 6.0 * yth / _rpow_int(muth, 4))
            )
            r["Dth2"] = (
                -2.0
                * wt
            ...
            r["Dmuth2"] = (
                ...
                * wt
                * Th
                * (2.0 * Th * yth / muth**2 - yth / muth - 2.0 * Th / muth + 1.0)
                / muth
            )
            r["Dmu2th2"] = (
                2.0
                * wt
                * Th
                * (-6.0 * yth * Th / muth**2 + 2.0 * yth / muth + 4.0 * Th / muth - 1.0)
                / muth**2
            )
            r["Dmu3th"] = 4.0 * wt * Th * (1.0 - 3.0 * yth / muth) / _rpow_int(muth, 3)
        return r
    ...
    def ls_extended(self, y, wt, theta=None, scale: float = 1.0) -> dict:
        ...
        key = (
            ...
            float(y.sum()),
            float(w.sum()) if w.size else 0.0,
        )
        hit = cache.get(key)
        if hit is not None:
            return hit
    ...
    def initialize(self, y, wt):
        ...
        if np.any(y < 0):
            raise ValueError(
                "negative values not allowed for the negative binomial family"
            )
...
class negbin(Family):
    ...
    def __init__(self, theta=None, link: str = "log"):
        ...
        if th.size < 1 or not np.all(np.isfinite(th)) or np.any(th <= 0):
            raise ValueError(
                "negbin theta must be positive and finite; use nb() to estimate theta"
            )
    ...
    def qf(self, p, mu, wt, scale):
        ...
        return _nmath._disp(
            "qnbinom_mu",
            _nmath.qnbinom_mu,
            [p, self._th, np.asarray(mu, dtype=float)],
            (True, False),
        )
...
class betar(Family):
    ...
    def __init__(
        self, theta: float | None = None, link: str = "logit", eps: float | None = None
    ):
        if link not in self._OK_LINKS:
            raise ValueError(
        ...
        self._eps = float(np.finfo(float).eps * 100) if eps is None else float(eps)
        super().__init__(link=link)
    ...
    def set_theta(self, values) -> None:
        ...
        if v.shape != (1,):
            raise ValueError(
                f"betar.set_theta expects a single log φ; got shape {v.shape}"
            )
        self._theta = v.copy()
    ...
    def d2var(self, mu):
        th = float(self._theta[0])
        return np.full_like(np.asarray(mu, dtype=float), -2.0 / (1.0 + np.exp(th)))
    ...
    def saturated_ll(self, y, wt, theta):
        ...
        def gbh(yy, eta, deriv=False, a=1e-8):
            ...
            ind = eta > 0
            expeta = np.where(
                ind, np.exp(-np.where(ind, eta, 0.0)), np.exp(np.where(ind, 0.0, eta))
            )
            mu = np.where(
                ind,
                (a * expeta + b) / (1.0 + expeta),
            ...
            if deriv:
                g = (
                    ...
                    - phi * np.log1p(-yy)
                    - phi * digamma(mu * phi)
                    + phi * digamma((1.0 - mu) * phi)
                )
                h = -(phi**2) * (
                    _polygamma(1, mu * phi) + _polygamma(1, (1.0 - mu) * phi)
                )
                dmueta1 = expeta * (b - a) / (1.0 + expeta) ** 2
                dmueta2 = (
                    np.sign(eta)
                    * ((a - b) * expeta + (b - a) * expeta**2)
                    / (expeta + 1.0) ** 3
                )
        ...
        for _ in range(200):
            ...
            while np.sum(fail) > 0 and k < 20:
                ...
                ls1_l[fail] = gbh(
                    yw[fail], eta[fail] + delta[fail], False, a=eps / 10.0
                )[0]
                fail = ls1_l < ls_l
            eta = eta + delta
        if ii.size > 0:
            ...
            warnings.warn("saturated likelihood may be inaccurate", stacklevel=2)
    ...
    def postproc(
        ...
        null_dev = 2.0 * lf["f"] + float(np.sum(self.dev_resids(y, mu_null, wt)))
        return {
            "deviance": dev,
    ...
    def residuals_extended(self, y, mu, wt, type: str = "deviance"):
        ...
        if type != "deviance":
            raise ValueError(
                f"betar residuals are 'deviance', 'response' or 'pearson'; got {type!r}"
            )
        theta = float(self.get_theta(trans=True)[0])
        lf = self.saturated_ll(y, wt, theta)
...
def _ocat_Fdiff(a: np.ndarray, b: np.ndarray) -> np.ndarray:
    ...
    inda = a > 0
    out[inda] = (ea[inda] - eb[inda]) / ((ea[inda] + 1.0) * (eb[inda] + 1.0))
    indm = (~indb) & (~inda)
    out[indm] = (1.0 - ea[indm] * eb[indm]) / ((eb[indm] + 1.0) * (ea[indm] + 1.0))
    return out
        ...
        for jj in range(R - 2):
            for kk in range(jj, R - 2):
                ...
                if kk == jj:
                    ...
                    Dmu3th[ind, kk] = wt[ind] * (
                        Dmu3a1[ind] * ar_k[ind] + Dmu3a0[ind] * ar1_k[ind]
                    )
                Dth2[:, i] = wt * (
                    Da1a1 * ar_k * ar_j
                    + Da0a1 * ar_k * ar1_j
...
def _ocat_ini(R: int, y0) -> np.ndarray | None:
    ...
    if R < 3:
        return None
    yy = np.concatenate([np.arange(1, R + 1), np.asarray(y0).astype(int) + 1]).astype(
        float
    )
...
class ocat(Family):
    ...
    def __init__(self, theta=None, R: int | None = None, link: str = "identity"):
        if link not in self._OK_LINKS:
            raise ValueError(
    ...
    def postproc(
        ...
        null_dev = find_null_dev(
            self, y, eta=linear_predictors, offset=offset, weights=prior_weights
        )
    ...
    def predict(
        self, se=False, X=None, beta=None, off=None, Vb=None, eta=None, y=None, lpi=None
    ) -> dict:
        ...
        if eta is None:
            ...
            se_v = None
            if se:
                se_v = np.sqrt(np.maximum(0.0, np.einsum("ij,jk,ik->i", X, Vb, X)))
...
def _zip_Dd(y, mu, theta, wt, b, level: int = 0) -> dict:
    ...
    oo["Dmu2"] = (
        -2.0 * wt * (l2[:, 0] + 2.0 * l2[:, 1] * pL + l2[:, 2] * pL**2 + l1[:, 1] * pll)
    )
    oo["EDmu2"] = -2.0 * wt * (El2[:, 0] + 2.0 * El2[:, 1] * pL + El2[:, 2] * pL**2)
    if level > 0:
        l3 = z["l3"]
        ...
        oo["Dmu2th"] = (
            ...
            * (
                l3[:, 1][:, None] * pth
                + 2.0 * l3[:, 2][:, None] * pL * pth
                + 2.0 * c2g * plth
                + l3[:, 3][:, None] * pL**2 * pth
                + c2e * (2.0 * pL * plth + pth * pll)
                + c1 * pllth
            )
        )
        oo["Dmu3"] = (
...
class ziP(Family):
    ...
    def _presence_lp(self, mu, theta):
        return theta[0] + (self._b + np.exp(theta[1])) * np.asarray(mu, dtype=float)
    ...
    def dev_resids(self, y, mu, wt, theta=None):
        ...
        with np.errstate(over="ignore", invalid="ignore"):
            return (
                -2.0
                * _zipll(
                    np.asarray(y, dtype=float), np.asarray(mu, dtype=float), p, deriv=0
                )["l"]
            )

    def Dd(self, y, mu, theta, wt, level: int = 0) -> dict:
        return _zip_Dd(y, mu, np.asarray(theta, dtype=float), wt, self._b, level=level)
    ...
    def initialize(self, y, wt):
        ...
        if np.any(y < 0):
            raise ValueError(
                "negative values not allowed for the zero inflated Poisson family"
            )
        if not np.allclose(y, np.round(y)):
            raise ValueError("Non-integer response variables are not allowed with ziP ")
    ...
    def postproc(
        ...
        def fnull(gamma):
            return float(np.sum(self.dev_resids(y, np.full(y.shape, gamma), wt)))
    ...
    def rd(self, rng, mu, wt, scale):
        ...
        with np.errstate(over="ignore", invalid="ignore"):
            ...
            mlam = max(
                float(np.max(lam[finite])) if np.any(finite) else 0.0,
...
def _cnorm_dev_resids(y, mu, wt, theta, censor):
    ...
    if ii.size:
        ...
        ethi = np.exp(-th[ii])
        zz = (y1 - y0) * ethi / 2.0
        d[ii] = 2.0 * _cnorm_dpnorm(-zz, zz, log_p=True) - 2.0 * _cnorm_dpnorm(
            (y0 - mu[ii]) * ethi, (y1 - mu[ii]) * ethi, log_p=True
        )
    if il.size:
...
def _cnorm_Dd(y, mu, theta, wt, censor, level=0):
    ...
    if ii.size:  # interval censored
        ...
        with np.errstate(**_es):
            ...
            ldzdz, szdz = _cnorm_ddnorm(
                z0, z1, np.log(np.abs(z0)), np.log(np.abs(z1)), np.sign(z0), np.sign(z1)
            )
            ...
            Dmu2[ii] = Dmu2i
            if level > 0:
                ldz2, sz2 = _cnorm_ddnorm(z0, z1, np.log(z0**2), np.log(z1**2))
                ldz3, sz3 = _cnorm_ddnorm(
                    z0,
                    z1,
                    np.log(np.abs(z0**3)),
                    np.log(np.abs(z1**3)),
                ...
                z03 = z02 * z0
                Dmu3i = Dmui * (
                    3.0 * Dmu2i / 2.0 - Dmui**2 / 4.0 - e2thi
                ) + 2.0 * sz2 * np.exp(ldz2 - ldp - th3i)
                Dmt = Dmui * Dt / 2.0 - Dmui + 2.0 * sz2 * np.exp(ldz2 - ldp - thi)
                ...
                if level > 1:
                    ...
                    Dmu4[ii] = (
                        Dmu2i * (3.0 * Dmu2i / 2.0 - Dmui**2 / 4.0 - e2thi)
                        + Dmui * (3.0 * Dmu3i - Dmui * Dmu2i) / 2.0
                        + sa1 * np.exp(lda1 - ldp - th3i)
                    )
                    ldz4, sz4 = _cnorm_ddnorm(z0, z1, np.log(z0**4), np.log(z1**4))
                    Dmu3th[ii] = (
                        Dmt * (3.0 * Dmu2i / 2.0 - Dmui**2 / 4.0)
                        + Dmui * (3.0 * Dmu2thi - Dmui * Dmt) / 2.0
                        + e2thi * (2.0 * Dmui - Dmt)
                        + (Dt - 10.0) * sz2 * np.exp(ldz2 - ldp - th3i)
                    ...
                    Dttt = (
                        Dtt * (Dt - 1.0)
                        + Dt * sz3 * np.exp(ldz3 - ldp)
                        + 2.0 * sa6 * np.exp(lda6 - ldp)
                    )
                    Dmu2th2[ii] = (
                        Dmt**2 + Dmui * Dmtt + e2thi * (Dttt - 4.0 * Dtt + 4.0 * Dt)
                    )
    ...
    if il.size:  # left censored (y0 = −∞)
        ...
        with np.errstate(**_es):
            ...
            if level > 0:
                ...
                Dmu3i = Dmui * (
                    3.0 * Dmu2i / 2.0 - Dmui**2 / 4.0 - e2thi
                ) + 2.0 * np.sign(z12) * np.exp(ldn + np.log(np.abs(z12)) - ldp - th3i)
                Dmt = (
                    Dmui * Dt / 2.0
                    - Dmui
                    + 2.0 * np.sign(z12) * np.exp(ldn + np.log(np.abs(z12)) - ldp - thi)
                )
                Dtt = (
                    Dt**2 / 2.0
                    - Dt
                    + 2.0 * np.sign(z13) * np.exp(ldn + np.log(np.abs(z13)) - ldp)
                )
                Dmu2thi = Dmui * Dmt + e2thi * (Dtt - 2.0 * Dt)
                Dth[il] = Dt
                Dmuth[il] = Dmt
                ...
                if level > 1:
                    ...
                    Dmu4[il] = (
                        Dmu2i * (3.0 * Dmu2i / 2.0 - Dmui**2 / 4.0 - e2thi)
                        + Dmui * (3.0 * Dmu3i - Dmui * Dmu2i) / 2.0
                        + np.sign(a1) * np.exp(ldn + np.log(np.abs(a1)) - ldp - th3i)
                    )
                    Dmu3th[il] = (
                        Dmt * (3.0 * Dmu2i / 2.0 - Dmui**2 / 4.0)
                        + Dmui * (3.0 * Dmu2thi - Dmui * Dmt) / 2.0
                        + e2thi * (2.0 * Dmui - Dmt)
                        + (Dt - 10.0) * np.exp(ldn + np.log(z12) - ldp - th3i)
                        + 2.0 * np.exp(ldn + np.log(z14) - ldp - th3i)
                    )
                    Dth2[il] = Dtt
                    Dmtt = (
                        (Dmt * Dt + Dmui * Dtt) / 2.0
                        - Dmt
                        + (Dt - 6.0) * np.exp(ldn + np.log(z12) - ldp - thi)
                        + 2.0
                        * np.sign(z14)
                    ...
                    Dttt = (
                        Dtt * (Dt - 1.0)
                        + Dt * np.sign(z13) * np.exp(ldn + np.log(np.abs(z13)) - ldp)
                        + 2.0 * np.sign(a1b) * np.exp(ldn + np.log(np.abs(a1b)) - ldp)
                    )
                    Dmu2th2[il] = (
                        Dmt**2 + Dmui * Dmtt + e2thi * (Dttt - 4.0 * Dtt + 4.0 * Dt)
                    )
    ...
    if ir.size:  # right censored (y1 = +∞)
        ...
        with np.errstate(**_es):
            ...
            if level > 0:
                ...
                Dmu3i = Dmui * (
                    3.0 * Dmu2i / 2.0 - Dmui**2 / 4.0 - e2thi
                ) - 2.0 * np.sign(z02) * np.exp(ldn + np.log(np.abs(z02)) - ldp - th3i)
                Dmt = (
                    Dmui * Dt / 2.0
                    - Dmui
                    - 2.0 * np.sign(z02) * np.exp(ldn + np.log(np.abs(z02)) - ldp - thi)
                )
                Dtt = (
                    Dt**2 / 2.0
                    - Dt
                    - 2.0 * np.sign(z03) * np.exp(ldn + np.log(np.abs(z03)) - ldp)
                )
                Dmu2thi = Dmui * Dmt + e2thi * (Dtt - 2.0 * Dt)
                Dth[ir] = Dt
                Dmuth[ir] = Dmt
                ...
                if level > 1:
                    ...
                    Dmu3th[ir] = (
                        ...
                        + Dmui * (3.0 * Dmu2thi - Dmui * Dmt) / 2.0
                        + e2thi * (2.0 * Dmui - Dmt)
                        - (Dt - 10.0) * np.exp(ldn + np.log(z02) - ldp - th3i)
                        - 2.0 * np.exp(ldn + np.log(z04) - ldp - th3i)
                    )
                    Dth2[ir] = Dtt
                    Dmtt = (
                        (Dmt * Dt + Dmui * Dtt) / 2.0
                        - Dmt
                        - (Dt - 6.0) * np.exp(ldn + np.log(z02) - ldp - thi)
                    ...
                    Dttt = (
                        Dtt * (Dt - 1.0)
                        - Dt * np.sign(z03) * np.exp(ldn + np.log(np.abs(z03)) - ldp)
                        - 2.0 * np.sign(a1b) * np.exp(ldn + np.log(np.abs(a1b)) - ldp)
                    )
                    Dmu2th2[ir] = (
                        Dmt**2 + Dmui * Dmtt + e2thi * (Dttt - 4.0 * Dtt + 4.0 * Dt)
...
class cnorm(Family):
    ...
    def set_censor(self, censor) -> None:
        ...
        self._censor = None if censor is None else np.asarray(censor, dtype=float)
        self._censorfull = None
...
def _cpois_dpois_log(x, lam):
    ...
    return _nmath._disp(
        "dpois",
        _nmath.dpois,
        [np.asarray(x, dtype=float), np.asarray(lam, dtype=float)],
        (True,),
    )
...
def _cpois_dev_resids(y, mu, censor):
    ...
    with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
        if iu.size:
            d[iu] = 2.0 * (
                _cpois_dpois_log(y[iu], y[iu]) - _cpois_dpois_log(y[iu], mu[iu])
            )
        if ii.size:
            y1 = np.maximum(yat[ii], y[ii])
            y0 = np.minimum(yat[ii], y[ii])
            ...
            fy0 = np.floor(y0)
            musat = np.exp(
                (_lgammafn_arr(fy1 + 1.0) - _lgammafn_arr(fy0 + 1.0)) / (fy1 - fy0)
            )
    ...
    for lt in (True, False):  # do left then right censoring...
        ...
        if idx.size:
            ...
            with np.errstate(**_es):
                ...
                if level > 0:
                    f3[idx] = np.exp(_cpois_ppois_log(yil - 3.0, mil, lt) - lf)
                if level > 1:
                    f4[idx] = np.exp(_cpois_ppois_log(yil - 4.0, mil, lt) - lf)
...
def _clog_dev_resids(y, mu, wt, theta, censor):
    ...
    if ii.size:  # interval censored
        ...
        line = (
            _clog_log1mexp((yr - yl) / si)
            - _clog_log1pexp((yl - mui) / si)
    ...
    if iu.size:  # uncensored
        ...
        with np.errstate(**_es):
            ...
            if level > 0:
                ...
                Dmu3[iu] = Dmu3i
                Dmu2thi = -2.0 * Dmu2i + (1.0 / 2.0) * (mui - yi) * Dmui * Dmu2i
                Dmu2th[iu] = Dmu2thi
            if level > 1:
                Dth2[iu] = (yi - mui) * Dmuthi
                Dmuth2[iu] = -Dmuthi + (yi - mui) * Dmu2thi
                Dmu4[iu] = (-1.0 / 2.0) * (_rpow_int(Dmu2i, 2) + Dmui * Dmu3i)
                Dmu3th[iu] = (-1.0 / 2.0) * (Dmu2i * Dmuthi + Dmui * Dmu2thi)
                Dmu2th2[iu] = -2.0 * Dmu2thi + (1.0 / 2.0) * (mui - yi) * (
    ...
    if ii.size:  # interval censored
        ...
        with np.errstate(**_es):
            ...
            Dmu2i = (
                2.0
                / _rpow_int(si, 2)
                * (alphai - _rpow_int(alphai, 2) + betai - _rpow_int(betai, 2))
            )
            Dmu2[ii] = Dmu2i
            if level > 0:
                ...
                lth = (1.0 / si) * (
                    ...
                    + (yl - mui) * (1.0 / (np.expm1((mui - yl) / si) + 2.0))
                    - (yr - mui) * (1.0 / (np.expm1((yr - mui) / si) + 2.0))
                )
                Dth[ii] = 2.0 * (lmth - lth)
                Dmu3i = (
                    4.0
                ...
                Dmu2thi = (
                    ...
                    / _rpow_int(si, 3)
                    * (
                        (yr - mui) * (_rpow_int(alphai, 2) - _rpow_int(alphai, 3))
                        + (yl - mui) * (_rpow_int(betai, 2) - _rpow_int(betai, 3))
                    )
                )
                Dmu2th[ii] = Dmu2thi
            if level > 1:
                ...
                Dmu3th[ii] = -2.0 * Dmu3i + 2.0 * (1.0 / _rpow_int(si, 3)) * (
                    (yr - mui)
                    * (alphai - _rpow_int(alphai, 2))
                    * (1.0 - 6.0 * alphai + 6.0 * _rpow_int(alphai, 2))
                    + (yl - mui)
                    * (betai - _rpow_int(betai, 2))
                    * (1.0 - 6.0 * betai + 6.0 * _rpow_int(betai, 2))
                )
                Dmu2th2[ii] = (
                    -2.0 * Dmu2thi
                    + (1.0 / si) * (Dmui - Dmuth2i)
    ...
    if il.size:  # left censored
        ...
        with np.errstate(**_es):
            ...
            if level > 0:
                ...
                Dmu2thi = (
                    -2.0 * Dmu2i
                    + (1.0 / si) * (yr - mui) * (2.0 * alphai - 1.0) * Dmu2i
                )
                Dmu2th[il] = Dmu2thi
            if level > 1:
                ...
                Dmu4[il] = (
                    -_rpow_int(Dmu2i, 2) + (1.0 / si) * (2.0 * alphai - 1.0) * Dmu3i
                )
                Dmu3th[il] = -(yr - mui) * _rpow_int(Dmu2i, 2) + (1.0 / si) * (
                    2.0 * alphai - 1.0
                ) * (Dmu2thi - Dmu2i)
                Dmu2th2[il] = (
                    -2.0 * Dmu2thi
                    - _rpow_int(yr - mui, 2) * _rpow_int(Dmu2i, 2)
                    + (1.0 / si) * (yr - mui) * (2.0 * alphai - 1.0) * (Dmu2thi - Dmu2i)
                )
    ...
    if ir.size:  # right censored
        ...
        with np.errstate(**_es):
            ...
            if level > 0:
                ...
                Dmu3[ir] = Dmu3i
                Dmu2thi = -(2.0 + (1.0 / si) * (yl - mui) * (1.0 - 2.0 * betai)) * Dmu2i
                Dmu2th[ir] = Dmu2thi
            if level > 1:
                ...
                Dmuth2[ir] = Dmuth2i
                Dmu4[ir] = (
                    -_rpow_int(Dmu2i, 2) + (-1.0 / si) * (1.0 - 2.0 * betai) * Dmu3i
                )
                Dmu3th[ir] = (1.0 / si) * (1.0 - 2.0 * betai) * (Dmu2i - Dmu2thi) - (
                    yl - mui
                ) * _rpow_int(Dmu2i, 2)
                Dmu2th2[ir] = (
                    (1.0 / si) * (yl - mui) * (1.0 - 2.0 * betai) * (Dmu2i - Dmu2thi)
                    + -_rpow_int(yl - mui, 2) * _rpow_int(Dmu2i, 2)
                    - 2.0 * Dmu2thi
                )
...
def _clog_ls(y, wt, theta, censor):
    ...
    if ii.size:
        ...
        lmth = (
            ...
            * (
                1.0
                / (np.expm1(-(yr - yl) / (2.0 * si)) - np.expm1((yr - yl) / (2.0 * si)))
            )
        )
        l1[ii] = lmth
        lmth2 = -lmth - (1.0 / 2.0) * _rpow_int(yr - yl, 2) * (
            ...
            / (np.expm1(-(yr - yl) / (2.0 * si)) + -np.expm1((yr - yl) / (2.0 * si))),
            2,
        ) * (2.0 + np.expm1(-(yr - yl) / (2.0 * si)) + np.expm1((yr - yl) / (2.0 * si)))
        l2[ii] = lmth2
...
def _bcg_bc_z(y, la, ls_variant=False):
    ...
    with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
        ...
        if abs(la) < 1e-7:
            ...
            yl2 = ylly * logy
            z = np.where(nina, ylly - yl2 * la / 2.0 + yl2 * logy * (la * la) / 6.0, z)
        else:
    ...
    with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
        ...
        if deriv > 0:
            if abs(la) < 1e-4:
                ...
                if deriv > 1:
                    z2 = np.where(
                        nina, ylly * (logy - 2.0 / la) / la + 2.0 * z / (la * la), z2
                    )
            ii = nina & (((y == np.inf) & (la < 0.0)) | ((y == 0.0) & (la > 0.0)))
            z1[ii] = 1.0 / (la * la)
            if deriv > 1:
                z2[ii] = -2.0 * z1[ii] / la
...
def _bcg_dev_resids(y, mu, wt, theta, censor):
    ...
    with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
        if iu.size:
            d[iu] = _rpow_int(_bcg_bc_z(y[iu], la) - mu[iu], 2) * np.exp(-2.0 * th[iu])
        if ii.size:
            y1 = np.maximum(yat[ii], y[ii])
            y0 = np.minimum(yat[ii], y[ii])
            ethi = np.exp(-th[ii])
            zz = (_bcg_bc_z(y1, la) - _bcg_bc_z(y0, la)) * ethi / 2.0
            d[ii] = 2.0 * _bcg_dpnorm(-zz, zz, log_p=True) - 2.0 * _bcg_dpnorm(
        ...
        if ir.size:
            d[ir] = -2.0 * pnorm5_vec(
                -(_bcg_bc_z(y[ir], la) - mu[ir]) * np.exp(-th[ir]), log_p=True
            )
...
def _bcg_aic(y, mu, wt, theta, censor):
    ...
    with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
        if iu.size:
            d[iu] = -2.0 * dnorm5_vec(
                (_bcg_bc_z(y[iu], la) - mu[iu]) * np.exp(-th[iu]), give_log=True
            )
        if ii.size:
            y1 = np.maximum(yat[ii], y[ii])
            y0 = np.minimum(yat[ii], y[ii])
...
def _bcg_ls(y, wt, theta, censor):
    ...
    with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
        if iu.size:
            ls = ls + _rsum((la - 1.0) * np.log(y[iu]) - th[iu] - _LOG2PI / 2.0)
        if ii.size:
            y1 = np.maximum(yat[ii], y[ii])
            y0 = np.minimum(yat[ii], y[ii])
            zz = (
                (
                    _bcg_bc_z(y1, la, ls_variant=True)
                    - _bcg_bc_z(y0, la, ls_variant=True)
                )
                * np.exp(-th[ii])
                / 2.0
            )
...
def _bcg_Dd(y, mu, theta, wt, censor, level=0):
    ...
    if iu.size:  # uncensored
        ...
        with np.errstate(**_es):
            ...
            if level > 1:
                Dmu4[iu] = 0.0
                Dmu3th[iu, :] = 0.0
                Dth2[iu, 0] = 2.0 * (e2thi * _rpow_int(bly, 2) + ethi * z * blly)
    ...
    if ii.size:  # interval censored
        ...
        with np.errstate(**_es):
            ...
            Dmu2[ii] = Dmu2i
            if level > 0:
                ldz2, sz2 = _bcg_ddnorm(
                    z0, z1, np.log(_rpow_int(z0, 2)), np.log(_rpow_int(z1, 2))
                )
                ldz3, sz3 = _bcg_ddnorm(
                    z0,
                    z1,
                ...
                ldbz, sbz = _bcg_ddnorm(
                    z0,
                    z1,
                    np.log(np.abs(bly0 * z0)),
                ...
                ldbz2, sbz2 = _bcg_ddnorm(
                    z0,
                    z1,
                    np.log(np.abs(bly0 * _rpow_int(z0, 2))),
                ...
                z03 = z02 * z0
                Dmu3i = Dmui * (
                    3.0 * Dmu2i / 2.0 - _rpow_int(Dmui, 2) / 4.0 - e2thi
                ) + 2.0 * sz2 * np.exp(ldz2 - ldp - th3i)
                Dmu3[ii] = Dmu3i
                Dl = -2.0 * sb * np.exp(ldb - ldp - thi)
                Dth[ii, 0] = Dl
                Dth[ii, 1] = Dt
                Dml = Dl * Dmui / 2.0 - 2.0 * sbz * np.exp(ldbz - ldp - th2i)
                Dmuth[ii, 0] = Dml
                Dmt = Dmui * Dt / 2.0 - Dmui + 2.0 * sz2 * np.exp(ldz2 - ldp - thi)
                Dmuth[ii, 1] = Dmt
                Dtt = _rpow_int(Dt, 2) / 2.0 - Dt + 2.0 * sz3 * np.exp(ldz3 - ldp)
                Dlt = Dl * Dt / 2.0 - Dl - 2.0 * sbz2 * np.exp(ldbz2 - ldp - thi)
            ...
            if level > 1:
                ...
                Dmu4[ii] = (
                    Dmu2i * (3.0 * Dmu2i / 2.0 - _rpow_int(Dmui, 2) / 4.0 - e2thi)
                    + Dmui * (3.0 * Dmu3i - Dmui * Dmu2i) / 2.0
                    + sa1 * np.exp(lda1 - ldp - th3i)
                )
                a1 = bly1 * z1 * (2.0 - z12)
                a0 = bly0 * z0 * (2.0 - z02)
                lda2, sa2 = _bcg_ddnorm(
                    z0,
                    z1,
                    np.log(np.abs(a0)),
                    np.log(np.abs(a1)),
                ...
                Dmu3th[ii, 0] = (
                    Dml * (3.0 * Dmu2i / 2.0 - _rpow_int(Dmui, 2) / 4.0)
                    + Dmui * (3.0 * Dmu2l - Dmui * Dml) / 2.0
                    - e2thi * Dml
                    + Dl * sz2 * np.exp(ldz2 - ldp - th3i)
                    + 2.0 * sa2 * np.exp(lda2 - ldp - th4i)
                )
                ldz4, sz4 = _bcg_ddnorm(
                    z0, z1, np.log(_rpow_int(z0, 4)), np.log(_rpow_int(z1, 4))
                )
                Dmu3th[ii, 1] = (
                    Dmt * (3.0 * Dmu2i / 2.0 - _rpow_int(Dmui, 2) / 4.0)
                ...
                Dmuth2[ii, 2] = Dmtt
                a1 = z1 * (_rpow_int(bly1, 2) * (z12 - 2.0) * ethi - blly1 * z1)
                a0 = z0 * (_rpow_int(bly0, 2) * (z02 - 2.0) * ethi - blly0 * z0)
                lda5, sa5 = _bcg_ddnorm(
                    z0,
                    z1,
                    np.log(np.abs(a0)),
                    np.log(np.abs(a1)),
                ...
                Dmu2th2[ii, 0] = _rpow_int(Dml, 2) + Dmui * Dmll + e2thi * Dllt
                Dmu2th2[ii, 1] = Dml * Dmt + Dmui * Dmlt + e2thi * (Dltt - 2.0 * Dlt)
                Dmu2th2[ii, 2] = (
                    _rpow_int(Dmt, 2)
                    + Dmui * Dmtt
    ...
    if il.size:  # left censoring (y0 = 0, z0 = -Inf, basically)
        ...
        with np.errstate(**_es):
            ...
            if level > 0:
                ...
                Dth[il, 1] = Dt
                a1 = bly1 * z1
                Dml = Dl * Dmui / 2.0 - 2.0 * np.sign(a1) * np.exp(
                    ldn + np.log(np.abs(a1)) - ldp - th2i
                )
                Dmuth[il, 0] = Dml
                Dmt = (
                    Dmui * Dt / 2.0
                ...
                Dlt = (
                    Dl * Dt / 2.0
                    - Dl
                    - 2.0 * np.sign(a1) * np.exp(ldn + np.log(np.abs(a1)) - ldp - thi)
                )
            ...
            if level > 1:
                ...
                Dmu3th[il, 0] = (
                    ...
                    + Dmui * (3.0 * Dmu2l - Dmui * Dml) / 2.0
                    - e2thi * Dml
                    + Dl * np.sign(z12) * np.exp(ldn + np.log(np.abs(z12)) - ldp - th3i)
                    + 2.0 * np.sign(a1) * np.exp(ldn + np.log(np.abs(a1)) - ldp - th4i)
                )
                Dmu3th[il, 1] = (
                    Dmt * (3.0 * Dmu2i / 2.0 - _rpow_int(Dmui, 2) / 4.0)
                    + Dmui * (3.0 * Dmu2t - Dmui * Dmt) / 2.0
                ...
                a1 = _rpow_int(bly1, 2) * z1 * ethi - blly1
                Dll = _rpow_int(Dl, 2) / 2.0 + 2.0 * np.sign(a1) * np.exp(
                    ldn + np.log(np.abs(a1)) - ldp - thi
                )
                Dth2[il, 0] = Dll
                Dth2[il, 1] = Dlt
                Dth2[il, 2] = Dtt
                ...
                Dmll = (
                    (Dl * Dml + Dmui * Dll) / 2.0
                    - Dl * np.sign(a1) * np.exp(ldn + np.log(np.abs(a1)) - ldp - th2i)
                    + 2.0 * np.sign(a2) * np.exp(ldn + np.log(np.abs(a2)) - ldp - th2i)
                )
                Dmuth2[il, 0] = Dmll
                a2 = bly1 * z13
                Dmlt = (
                ...
                Dmtt = (
                    ...
                    - Dmt
                    + (Dt - 6.0) * np.exp(ldn + np.log(z12) - ldp - thi)
                    + 2.0 * np.sign(z14) * np.exp(ldn + np.log(np.abs(z14)) - ldp - thi)
                )
                Dmuth2[il, 2] = Dmtt
                a2 = z1 * (_rpow_int(bly1, 2) * (z12 - 2.0) * ethi - blly1 * z1)
                a1 = bly1 * z12
                Dllt = (
                    (Dl * Dlt + Dt * Dll) / 2.0
                    - Dll
                    - Dl * np.sign(a1) * np.exp(ldn + np.log(np.abs(a1)) - ldp - thi)
                    + 2.0 * np.sign(a2) * np.exp(ldn + np.log(np.abs(a2)) - ldp - thi)
                )
                a1 = bly1 * z12
                a2 = bly1 * z14
                Dltt = (
                ...
                Dttt = (
                    Dtt * (Dt - 1.0)
                    + Dt * np.sign(z13) * np.exp(ldn + np.log(np.abs(z13)) - ldp)
                    + 2.0 * np.sign(a1) * np.exp(ldn + np.log(np.abs(a1)) - ldp)
                )
                Dmu2th2[il, 0] = _rpow_int(Dml, 2) + Dmui * Dmll + e2thi * Dllt
                Dmu2th2[il, 1] = Dml * Dmt + Dmui * Dmlt + e2thi * (Dltt - 2.0 * Dlt)
                Dmu2th2[il, 2] = (
                    _rpow_int(Dmt, 2)
                    + Dmui * Dmtt
                    + e2thi * (Dttt - 4.0 * Dtt + 4.0 * Dt)
    ...
    if ir.size:  # right censoring - basically y1 = Inf
        ...
        with np.errstate(**_es):
            ...
            if level > 0:
                ...
                Dmu3[ir] = Dmu3i
                Dl = (
                    2.0 * np.sign(bly0) * np.exp(ldn + np.log(np.abs(bly0)) - ldp - thi)
                )
                Dth[ir, 0] = Dl
                Dth[ir, 1] = Dt
                a1 = bly0 * z0
            ...
            if level > 1:
                ...
                Dmu3th[ir, 0] = (
                    ...
                    + Dmui * (3.0 * Dmu2l - Dmui * Dml) / 2.0
                    - e2thi * Dml
                    - Dl * np.sign(z02) * np.exp(ldn + np.log(np.abs(z02)) - ldp - th3i)
                    - 2.0 * np.sign(a1) * np.exp(ldn + np.log(np.abs(a1)) - ldp - th4i)
                )
                Dmu3th[ir, 1] = (
                    Dmt * (3.0 * Dmu2i / 2.0 - _rpow_int(Dmui, 2) / 4.0)
                ...
                Dmuth2[ir, 2] = Dmtt
                a2 = z0 * (_rpow_int(bly0, 2) * (z02 - 2.0) * ethi - blly0 * z0)
                a1 = bly0 * z02
                Dllt = (
                    (Dl * Dlt + Dt * Dll) / 2.0
                    - Dll
                ...
                Dmu2th2[ir, 0] = _rpow_int(Dml, 2) + Dmui * Dmll + e2thi * Dllt
                Dmu2th2[ir, 1] = Dml * Dmt + Dmui * Dmlt + e2thi * (Dltt - 2.0 * Dlt)
                Dmu2th2[ir, 2] = (
                    _rpow_int(Dmt, 2)
                    + Dmui * Dmtt
                    + e2thi * (Dttt - 4.0 * Dtt + 4.0 * Dt)
...
class bcg(Family):
    ...
    def dev_resids(self, y, mu, wt, theta=None):
        th = (
            self._theta if theta is None else np.asarray(theta, dtype=float).reshape(-1)
        )
    ...
    def ls(self, y, wt, scale):
        return np.array([_bcg_ls(y, wt, self._theta, self._censor), 0.0, 0.0])
    ...
    def residuals_extended(self, y, mu, wt, type: str = "deviance"):
        ...
        if type != "deviance":
            raise NotImplementedError(f"bcg residuals: type {type!r} not implemented")
        d, sgn = _bcg_dev_resids(
...
def _gfam_exp_ls(fam, y, w, scale):
    ...
    if name == "gaussian":
        ...
        return np.array(
            [
                -nobs * math.log(2.0 * math.pi * scale) / 2.0 + _rsum(lw) / 2.0,
                -nobs / (2.0 * scale),
    ...
    if name == "binomial":
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            logp = _dbinom_raw_disp(np.rint(m * y), np.rint(m), y, 1.0 - y, True)
    ...
    if name == "inverse.gaussian":
        ...
        return np.array(
            [
                -_rsum(np.log(2.0 * math.pi * scale * _rpow_int(y[good], 3))) / 2.0
                + _rsum(lw) / 2.0,
                -nobs / (2.0 * scale),
                nobs / (2.0 * scale * scale),
            ]
...
def _gfam_filth(n, j, nth):
    ...
    reps = np.repeat(_gfam_kj(np.arange(nth), n - j + 1), np.arange(nth, 0, -1))
...
def _gfam_filsc(n, j, nth):
    ...
    k = np.arange(nth + 1)
    return (_gfam_kj(k, n - j + 1) + np.arange(nth + 1, 0, -1) + _gfam_kj(j - 1, n)) - 1
...
class _GfamLink(Link):
    ...
    def link(self, mu):
        return self._dispatch(mu, "link")

    def linkinv(self, eta):
        return self._dispatch(eta, "linkinv")

    def mu_eta(self, eta):
        return self._dispatch(eta, "mu_eta")

    def g2g(self, mu):
        return self._dispatch(mu, "g2g")

    def g3g(self, mu):
        return self._dispatch(mu, "g3g")

    def g4g(self, mu):
        return self._dispatch(mu, "g4g")
...
class gfam(Family):
    ...
    def __init__(self, fl):
        ...
        for f in fl:
            if isinstance(f, GeneralFamily):
                ...
                raise NotImplementedError("general familes not implemented so far")
            ...
            if _gfam_is_ext(f):
                if scale < 0:
                    ...
                    theta_parts.append(
                        np.concatenate(
                            [np.asarray(f.get_theta(), dtype=float).reshape(-1), [0.0]]
                        )
                    )
                else:
                    n_theta += f.n_theta
            ...
            if (
                getattr(f, "residuals", None) is not None
        ...
        # any slot exists.
        self.estimate_theta_callback = self.n_theta > 0
        self._theta = np.concatenate(theta_parts) if theta_parts else np.zeros(0)
    ...
    def set_ind(self, ind) -> None:
        ...
        self._fi = (
            self._fifull if ind is None else self._fifull[np.asarray(ind, dtype=int)]
        )
    ...
    def set_theta(self, values) -> None:
        ...
        if v.shape[0] != self.n_theta:
            raise ValueError(f"gfam expects {self.n_theta} params; got {v.shape[0]}")
        self._theta = v.copy()
        ...
        for f, ii, nth, fscale, i0 in self._blocks(n):
            ...
            if _gfam_is_ext(f):
                ...
                if level > 0:
                    if nth:
                        r["Dth"][np.ix_(ii, ith)] = (
                            ri["Dth"].reshape(ii.size, nth) / scale
                        )
                        r["Dmuth"][np.ix_(ii, ith)] = (
                            ri["Dmuth"].reshape(ii.size, nth) / scale
                        )
                        r["Dmu2th"][np.ix_(ii, ith)] = (
                            ri["Dmu2th"].reshape(ii.size, nth) / scale
                        )
                        r["EDmu2th"][np.ix_(ii, ith)] = (
                            ri["EDmu2th"].reshape(ii.size, nth) / scale
                        )
                ...
                if level > 1:
                    ...
                    if nth > 0:
                        ...
                        r["Dmu3th"][np.ix_(ii, ith)] = (
                            ri["Dmu3th"].reshape(ii.size, nth) / scale
                        )
                        npr = nth * (nth + 1) // 2
                        r["Dth2"][np.ix_(ii, ijth)] = (
                            ri["Dth2"].reshape(ii.size, npr) / scale
                        )
                        r["Dmuth2"][np.ix_(ii, ijth)] = (
                            ri["Dmuth2"].reshape(ii.size, npr) / scale
                        )
                        r["Dmu2th2"][np.ix_(ii, ijth)] = (
                            ri["Dmu2th2"].reshape(ii.size, npr) / scale
                        )
                    if fscale < 0:
                        ijsc = _gfam_filsc(n_theta, i0 + 1, nth)
                        r["Dmu3th"][ii, isc] = -ri["Dmu3"] / scale
                        r["Dth2"][np.ix_(ii, ijsc)] = -r["Dth"][np.ix_(ii, its)]
                        r["Dmuth2"][np.ix_(ii, ijsc)] = -r["Dmuth"][np.ix_(ii, its)]
                        r["Dmu2th2"][np.ix_(ii, ijsc)] = -r["Dmu2th"][np.ix_(ii, its)]
                ...
                if level > 0:
                    d2v = f.d2var(mu[ii])
                    r["Dmu3"][ii] = -r["Dmu2"][ii] * dv / vi + 2.0 * (
                        ri * (d2v / vi - _rpow_int(dv / vi, 2)) - dv / vi
                    ) / (vi * scale)
                    if fscale < 0:
                ...
                if level > 1:
                    ...
                    if fscale < 0:
                        ...
                        r["Dth2"][np.ix_(ii, ijsc)] = -r["Dth"][ii, isc].reshape(-1, 1)
                        r["Dmuth2"][np.ix_(ii, ijsc)] = -r["Dmuth"][ii, isc].reshape(
                            -1, 1
                        )
                        r["Dmu2th2"][np.ix_(ii, ijsc)] = -r["Dmu2th"][ii, isc].reshape(
                            -1, 1
                        )
        return r
    ...
    def ls_extended(self, y, wt, theta=None, scale: float = 1.0) -> dict:
        ...
        for f, ii, nth, fscale, i0 in self._blocks(y.shape[0]):
            if _gfam_is_ext(f):
                ...
                if nth1 > 0:
                    lsth1[ith] = np.asarray(li["lsth1"], dtype=float).reshape(-1)
                    LSTH1[np.ix_(ii, ith)] = np.asarray(
                        li["LSTH1"], dtype=float
                    ).reshape(ii.size, nth1)
                    lsth2[np.ix_(ith, ith)] = np.asarray(
                        li["lsth2"], dtype=float
                    ).reshape(nth1, nth1)
            else:
                if fscale < 0:
                ...
                if fscale < 0:
                    ...
                    LSTH1[ii, i0] = (sca * w01) * li[1] / float(np.sum(wt[ii] > 0))
    ...
    def aic(self, y, mu, dev, wt, n, theta=None) -> float:
        ...
        for f, ii, nth, fscale, i0 in self._blocks(y.shape[0]):
            if _gfam_is_ext(f):
                ...
                aic += float(f.aic(y[ii], mu[ii], dev_i, wt[ii], n1[ii], theta=th))
            else:
    ...
    def preinitialize(self, y) -> dict | None:
        ...
        ui = np.unique(fi)
        ok = (
            np.isin(ui, np.arange(1, nf + 1)).all()
            and np.isin(np.arange(1, nf + 1), ui).all()
        )
        if not ok:
        ...
        theta_mod = False
        for f, ii, nth, fscale, i0 in self._blocks(y.shape[0]):
            if _gfam_is_ext(f) and (type(f).preinitialize is not Family.preinitialize):
                ...
                if pri.get("Theta") is None:
                    Theta[i0 : i0 + nth] = np.asarray(
                        f.get_theta(), dtype=float
                    ).reshape(-1)[:nth]
                else:
                    theta_mod = True
                    Theta[i0 : i0 + nth] = np.asarray(
                        pri["Theta"], dtype=float
                    ).reshape(-1)
        ret: dict = {"y": y}
        if theta_mod:
            ret["Theta"] = Theta
    ...
    def postproc(
        ...
        for f, ii, nth, fscale, i0 in self._blocks(y.shape[0]):
            if _gfam_is_ext(f):
                ...
                wtdmu = (
                    float(_rsum(pw[ii] * y[ii]) / _rsum(pw[ii]))
                    if intercept
                    else f.link.linkinv(off[ii])
                )
                nulldev += _rsum(
                    f.dev_resids(
                        y[ii],
                        np.full(ii.size, wtdmu) if np.isscalar(wtdmu) else wtdmu,
                        pw[ii],
                    )
                )
        out = {"family_name": "gfam{" + ",".join(names) + "}", "null_deviance": nulldev}
        if dev_mod:
            out["deviance"] = dev
        return out
    ...
    def _residuals_gfam(self, y, fitted, type: str = "deviance", prior_weights=None):
        y = np.asarray(y, dtype=float)
        fitted = np.asarray(fitted, dtype=float)
        wt = (
            np.ones(y.shape[0])
        ...
        for f, ii, nth, fscale, i0 in self._blocks(y.shape[0]):
            ...
            if ext is not None:
                ...
                rsd[ii] = (y[ii] - fitted[ii]) * np.sqrt(
                    wt[ii] / f.variance(fitted[ii])
                )
            else:
    ...
    def predict(
        ...
        if y is None:
            ...
            if y.ndim == 2:
                if y.shape[1] != 2:
                    raise ValueError("if response is a matrix it must have 2 columns")
                fi = y[:, 1]
            else:
                fi = y
        nf = len(self._fl)
        ...
        y_col = (
            y[:, 0] if (y is not None and y.ndim == 2) else y
        )  # R's y[ii] linear-indexes column 1
        for i, f in enumerate(self._fl):
            ii = np.where(fi == i + 1)[0]
            if ii.size == 0:
                continue
            ...
            if f_pred is None:
                ...
                if se:
                    se_fit[ii] = np.sqrt(
                        np.maximum(0.0, np.einsum("ij,jk,ik->i", X[ii], Vb, X[ii]))
                    )
                ...
                if pf.ndim > 1:
                    raise ValueError(
                        "gfam mixed response scale prediction not possible here"
                    )
    ...
    def get_null_coef(self, X, y, wt, offset) -> tuple[np.ndarray, float]:
        ...
        null_scale = float(_rsum(self.dev_resids(y, mum, wt)) / X.shape[0])
...
def trind_generator(
    K: int = 2, ifunc: bool = False, reverse: bool | None = None
) -> dict:
            ...
            for j in range(i, K):
                for ll_ in range(j, K):
                    ...
                    if mo == 3:
                        ...
                        d3[:, k] = l3[:, k] * ig1[:, i] * ig1[:, j] * ig1[:, ll_]
                    else:
                        k1 = int(ii[ord_ == 1][0])
                        k2 = int(ii[ord_ == 2][0])
                        mind = i2[k2, k1]
                        d3[:, k] = (
                            (l3[:, k] - l2[:, mind] * g2[:, k2] * ig1[:, k2])
                            * ig1[:, k1]
                            * ig1[:, k2] ** 2
                        )
                    k += 1
...
class _DiscreteLPSolve:
    ...
    def solve(self, Xty):
        ...
        z = solve_triangular(self._U, Xty[self._piv], lower=False, trans="T")
...
def gamlss_gH(
    ...
    if sandwich:
        ...
        if deriv > 0:
            ...
            warnings.warn("sandwich requested with higher derivatives", stacklevel=2)
        l2 = l2.copy()
        for i in range(K):
            for j in range(i, K):
                l2[:, i2[i, j]] = l1[:, i] * l1[:, j]
    ...
    for i in range(K):
        for j in range(i, K):
            if discrete:
                ...
                A = _XWXd(design, l2[:, i2[i, j]], lt=lpid[i], rt=lpid[j])
            else:
                Xi = X[:, jj[i]]
            ...
            for i in range(K):
                for j in range(i, K):
                    ...
                    for ll_ in range(m):
                        v = np.zeros(n)
                        for q in range(K):
                            v += l3[:, i3[i, j, q]] * d1eta[q * n : (q + 1) * n, ll_]
                        XVX = _XWXd(design, v, lt=lpid[i], rt=lpid[j])
                        d1H[ll_] += mult * float(np.sum(XVX * fh[np.ix_(jj[i], jj[j])]))
        else:
            for i in range(K):
                for j in range(i, K):
    ...
    if deriv > 1 and discrete:
        ...
        raise NotImplementedError("er... no discrete methods for higher derivatives")
            ...
            for ll_ in range(k_, m):
                ...
                for i in range(K):
                    for j in range(K):
                        ...
                        VX[j * n : (j + 1) * n, jj[i]] = v[:, None] * X[:, jj[i]]
...
def _pen_reg(x: np.ndarray, e: np.ndarray, y: np.ndarray) -> np.ndarray:
    ...
    re = (
        min(int(np.sum(np.abs(e).sum(axis=0) != 0)), e.shape[0])
        - _R_rank(R_a, tol=float(np.finfo(float).eps) ** 0.9)
        + rr
    )
...
class LogbLink(Link):
    ...
    def _mub(self, mu):
        return np.maximum(
            1.0 - np.asarray(mu, dtype=float) * self.b, np.finfo(float).eps
        )
    ...
    **Authoring contract.** This is hea's public extension API for
    ...
    def sandwich(self, y, X, coef, wt, *, lpi, offset=None):
        ...
        if not self.has_sandwich:
            ...
            raise NotImplementedError("no sandwich estimate available for this model")
        return self.ll(y, X, coef, wt, lpi=lpi, offset=None, deriv=1, sandwich=True)[
            "lbb"
        ]
    ...
    def initialize_coef(
        self, y, X, lpi, E=None, offset=None, use_unscaled: bool = False
    ) -> np.ndarray:
...
class gaulss(GeneralFamily):
    ...
    def __init__(self, link: tuple[str, str] = ("identity", "logb"), b: float = 0.01):
    ...
    def ll(
        ...
        *,
        ...
        l2 = np.column_stack([-tau2, 2.0 * l1[:, 0] / tau, -ymu2 - 1.0 / tau2])
        ig1 = np.column_stack([self.links[0].mu_eta(eta), self.links[1].mu_eta(eta1)])
        g2 = np.column_stack([self.links[0].d2link(mu), self.links[1].d2link(tau)])
        l3 = l4 = g3 = g4 = None
        if deriv > 1:
            # third derivatives, packed (mmm, mms, mss, sss)
            zeros = np.zeros(n)
            l3 = np.column_stack([zeros, -2.0 * tau, 2.0 * ymu, 2.0 / tau**3])
            g3 = np.column_stack([self.links[0].d3link(mu), self.links[1].d3link(tau)])
        if deriv > 3:
            # fourth derivatives, packed (mmmm, mmms, mmss, msss, ssss)
            zeros = np.zeros(n)
            l4 = np.column_stack(
                [zeros, zeros, np.full(n, -2.0), zeros, -6.0 / (tau2 * tau2)]
            )
            g4 = np.column_stack([self.links[0].d4link(mu), self.links[1].d4link(tau)])

        tri = self.tri
        l1, l2, l3, l4 = self._apply_prior_weights(wt, l1, l2, l3, l4)
        de = gamlss_etamu(
            l1, l2, l3, l4, ig1, g2, g3, g4, tri["i2"], tri["i3"], tri["i4"], deriv - 1
        )
        gh = gamlss_gH(
            X,
            jj,
    ...
    def initialize_coef(
        ...
        def _reg(cols, target):
            if use_unscaled:
                ...
                b, *_ = np.linalg.lstsq(
                    xa, np.concatenate([target, np.zeros(E.shape[0])]), rcond=None
                )
                b[~np.isfinite(b)] = 0.0
                return b
        ...
        b1 = _reg(jj[0], yt1)
        start[jj[0]] = b1
        lres1 = np.log(np.abs(y - self.links[0].linkinv(X[:, jj[0]] @ b1)))
        if offset is not None and len(offset) > 1 and offset[1] is not None:
            lres1 = lres1 - offset[1]
        start[jj[1]] = _reg(jj[1], lres1)
        return start

    def _initialize_coef_discrete(self, y, X: DiscreteX, jj, E, offset) -> np.ndarray:
    ...
    def postproc(
        ...
        return {
            "null_deviance": float(
                np.sum(((y - float(np.mean(y))) * fitted[:, 1]) ** 2)
            )
        }
    ...
    def __repr__(self):
        return f"gaulss(link=({self.links[0].name!r}, 'logb'), b={self.b:g})"
...
class twlss(GeneralFamily):
    ...
    def __init__(
        self,
        link: tuple[str, str, str] = ("log", "identity", "identity"),
        a: float = 1.01,
        b: float = 1.99,
    ):
        ...
        self.b = float(b)
        links = [
            {"log": LogLink, "identity": IdentityLink, "sqrt": SqrtLink}[mu_link](),
            IdentityLink(),
            IdentityLink(),
        ]
        super().__init__(links)
    ...
    def ll(
        ...
        *,
        ...
        wt = np.ones(y.shape[0]) if wt is None else np.asarray(wt, dtype=float).ravel()
        l0 = ld[:, 0]
        ret: dict = {"l": float(np.sum(wt * l0)), "l0": l0}
        if deriv == 0:
            return ret
        ...
        de = gamlss_etamu(
            l1, l2, None, None, ig1, g2, None, None, tri["i2"], tri["i3"], tri["i4"], 0
        )
        gh = gamlss_gH(
            X,
            jj,
    ...
    def postproc(
        ...
        nd = np.sum(np.maximum(2.0 * (y * th0 - ka0) * pw / np.exp(fitted[:, 2]), 0.0))
        return {"null_deviance": float(nd)}

    def residuals(
        self, y, fitted, type: str = "deviance", prior_weights=None
    ) -> np.ndarray:
    ...
    def __repr__(self):
        return (
            f"twlss(link=({self.links[0].name!r}, 'identity', "
...
class LogebLink(Link):
    ...
    def d4link(self, mu):
        em = np.exp(np.asarray(mu, dtype=float))
        b = self.b
        return -b * em * (b**2 + 4.0 * b * em + em**2) / (em - b) ** 4
...
class shash(GeneralFamily):
    ...
    def __init__(
        self,
        link: tuple = ("identity", "logeb", "identity", "identity"),
        b: float = 1e-2,
        phiPen: float = 1e-3,
    ):
        mu_link, tau_link, eps_link, phi_link = link
        if mu_link != "identity" or eps_link != "identity" or phi_link != "identity":
            raise ValueError(
        ...
        if tau_link != "logeb":
            raise ValueError(
                'only the "logeb" link is available for the scale parameter of shash'
            )
        self.b = float(b)
        self.phiPen = float(phiPen)
        super().__init__([IdentityLink(), LogebLink(b), IdentityLink(), IdentityLink()])
    ...
    def ll(
        ...
        *,
        ...
        if offset is not None and any(
            o is not None and np.any(np.asarray(o) != 0.0) for o in offset
        ):
            raise NotImplementedError(
                "offset not still available for this family (mgcv shash, gamlss.r:3470)"
            )
        y = np.asarray(y, dtype=float)
        X = np.asarray(X, dtype=float)
        coef = np.asarray(coef, dtype=float)
        ...
        l0, L1, L2, L3, L4 = _shash_derivs(y, mu, tau, eps, phi, self.phiPen, deriv)
        wt = np.ones(y.shape[0]) if wt is None else np.asarray(wt, dtype=float).ravel()
        ret: dict = {"l": float(np.sum(wt * l0)), "l0": l0}
        if deriv == 0:
            return ret
        params = (mu, tau, eps, phi)
        ig1 = np.column_stack([lnk.mu_eta(eta) for lnk, eta in zip(self.links, etas)])
        g2 = np.column_stack([lnk.d2link(par) for lnk, par in zip(self.links, params)])
        g3 = g4 = None
        if deriv > 1:
            g3 = np.column_stack(
                [lnk.d3link(par) for lnk, par in zip(self.links, params)]
            )
        if deriv > 3:
            g4 = np.column_stack(
                [lnk.d4link(par) for lnk, par in zip(self.links, params)]
            )
        tri = self.tri
        L1, L2, L3, L4 = self._apply_prior_weights(wt, L1, L2, L3, L4)
        de = gamlss_etamu(
    ...
    def residuals(self, y, fitted, type: str = "deviance") -> np.ndarray:
        ...
        y = np.asarray(y, dtype=float)
        fitted = np.asarray(fitted, dtype=float)
        mu, tau, eps, phi = (fitted[:, 0], fitted[:, 1], fitted[:, 2], fitted[:, 3])
        sig = np.exp(tau)
        delta = np.exp(phi)
        rsd = (
            y
        ...
        ll = (
            -tau
            - 0.5 * np.log(2.0 * np.pi)
            + np.log(np.cosh(dTasMe))
            - 0.5 * np.log1p(z**2)
            - 0.5 * np.sinh(dTasMe) ** 2
        )
    ...
    def cdf(self, q, mu, wt, scale, logp: bool = False):
        ...
        eps_e = mu[:, 2]
        del_e = np.exp(mu[:, 3])
        s = np.sinh((np.arcsinh((q - mu_e) / (del_e * sig_e)) - eps_e / del_e) * del_e)
        return _nmath.pnorm5_vec(s, log_p=True) if logp else _nmath.pnorm5_vec(s)

    def __repr__(self):
        return (
            f"shash(link=('identity', 'logeb', 'identity', "
...
class gammals(GeneralFamily):
    ...
    def __init__(self, link: tuple[str, str] = ("identity", "log"), b: float = -7.0):
    ...
    def ll(
        ...
        *,
        ...
        ig1 = np.column_stack([self.links[0].mu_eta(eta), self.links[1].mu_eta(etat)])
        g2 = np.column_stack([self.links[0].d2link(mu), self.links[1].d2link(th)])
        l3 = l4 = g3 = g4 = None
        g3eth = None
        if deriv > 1:
            eth3 = eth2 * eth
            ...
            g3 = np.column_stack([self.links[0].d3link(mu), self.links[1].d3link(th)])
        if deriv > 3:
            eth4 = eth3 * eth
            l4 = np.column_stack(
                [
            ...
            g4 = np.column_stack([self.links[0].d4link(mu), self.links[1].d4link(th)])
    ...
    def initialize_coef(
        ...
        lres1 = self.links[1].link(
            np.log(np.abs(y - self.links[0].linkinv(X[:, jj[0]] @ b1)))
        )
        if offset is not None and len(offset) > 1 and offset[1] is not None:
            lres1 = lres1 - offset[1]
        start[jj[1]] = _reg(jj[1], lres1)
    ...
    def _initialize_coef_discrete(self, y, X: DiscreteX, jj, E, offset) -> np.ndarray:
        ...
        if offset is not None and offset[0] is not None:
            yt1 = yt1 - offset[0]
        startji = _DiscreteLPSolve(design, lpid[0], E[:, jj[0]], ones_n).solve_target(
            yt1
        )
        startji[~np.isfinite(startji)] = 0.0
        start[jj[0]] = startji
        eta1 = _Xbd(design, start, lt=lpid[0])
        lres1 = self.links[1].link(np.log(np.abs(y - self.links[0].linkinv(eta1))))
        if offset is not None and len(offset) > 1 and offset[1] is not None:
            lres1 = lres1 - offset[1]
        start[jj[1]] = _DiscreteLPSolve(
            design, lpid[1], E[:, jj[1]], ones_n
        ).solve_target(lres1)
        return start
    ...
    def postproc(
        ...
        my = float(np.mean(y))
        nd = 2.0 * float(
            np.sum(((y - my) / my - np.log(y / my)) * np.exp(-fitted[:, 1]))
        )
        ...
        if eta is None:
            ...
            for i in range(2):
                ...
                if se:
                    ...
                    ve[:, i] = np.maximum(0.0, np.einsum("ij,jk,ik->i", Xi, Vii, Xi))
        else:
            eta = np.asarray(eta, dtype=float)
            se = False
        gamma = np.column_stack([np.exp(eta[:, 0]), self.links[1].linkinv(eta[:, 1])])
        if se:
            vp = np.column_stack(
                [
    ...
    def __repr__(self):
        return f"gammals(link=('identity', {self._scale_link_name!r}), b={self.b:g})"
    ...
    **location** μ (identity link only, η₁ ≡ μ); LP2 ``log β`` (the
    ...
    def ll(
        ...
        *,
        ...
        ig1 = np.column_stack([self.links[0].mu_eta(eta), self.links[1].mu_eta(etab)])
        g2 = np.column_stack([self.links[0].d2link(mu), self.links[1].d2link(beta)])
        l3 = l4 = g3 = g4 = None
        if deriv > 1:
            lzzz = ez
            zbbb = -z
            ...
            g3 = np.column_stack([self.links[0].d3link(mu), self.links[1].d3link(beta)])
        if deriv > 3:
            lzzzz = -ez
            zbbbb = z
            zmbbb = eb
            ...
            g4 = np.column_stack([self.links[0].d4link(mu), self.links[1].d4link(beta)])
    ...
    def initialize_coef(
        ...
        lres1 = self.links[1].link(
            np.log((y - self.links[0].linkinv(X[:, jj[0]] @ start[jj[0]])) ** 2) / 2.0
            - 0.25
        )
        if offset is not None and len(offset) > 1 and offset[1] is not None:
            lres1 = lres1 - offset[1]
    ...
    def predict(
        ...
        *,
        ...
        if eta is None:
            ...
            eta = np.asarray(eta, dtype=float)
            se = False
        gamma = np.column_stack([eta[:, 0], self.links[1].linkinv(eta[:, 1])])
        if se:
            vp = np.column_stack(
                [
                    np.sqrt(ve[:, 0]),
    ...
    def __repr__(self):
        return f"gumbls(link=('identity', {self._scale_link_name!r}), b={self.b:g})"
...
def _gevlss_derivs(y, mu, rho, xi, deriv):
    ...
    cc3 = cc0 + 1.0
    l1[:, 1] = (
        -bb1 * cc2 * cc3 ** ((-1.0 / xi) - 1.0) + bb1 * (xi + 1.0) * cc2 / cc3 - 1.0
    )
    dd3 = xi + 1.0
    dd6 = 1.0 / cc3
    dd7 = log_cc3
    dd8 = 1.0 / xi**2
    l1[:, 2] = (
        -(dd8 * dd7 - bb1 * aa2 * cc2 * dd6) / cc3**aa2
        + dd8 * dd3 * dd7
        - aa2 * dd7
        - bb1 * aa2 * dd3 * cc2 * dd6
    )
    ...
    ee3 = -1.0 / xi
    l2[:, 0] = (
        ee1 * (ee3 - 1.0) * xi * aa1 ** (ee3 - 2.0) + ee1 * xi * (xi + 1.0) / aa1**2
    )
    ff7 = ee3 - 1.0
    l2[:, 1] = (
        bb1 * cc3**ff7
    ...
    if deriv > 1:
        ...
        kk1 = 1.0 / er**3
        kk2 = xi**2
        l3[:, 0] = 2.0 * kk1 * kk2 * (xi + 1.0) / aa1**3 - kk1 * (ee3 - 2.0) * (
            ee3 - 1.0
        ) * kk2 * aa1 ** (ee3 - 3.0)
        ll5 = xi * cc2 / er + 1.0
        ll8 = ee3 - 2.0
        l3[:, 1] = (
            -2.0 * ee1 * ff7 * xi * ll5**ll8
        ...
        mm12 = cc3**mm11
        l3[:, 2] = (
            ee1 * (gg7 - 1.0) * xi * mm12 * (log_cc3 / xi**2 - bb1 * aa2 * cc2 / cc3)
            - ee1 * mm12
        ...
        pp08 = cc3**pp07
        l3[:, 5] = (
            -bb1 * pp08 * (bb1 * pp07 * cc2 * dd6 + dd8 * dd7) ** 2
            - bb1
            * pp08
            * (
                -ee1 * pp07 * hh4 * jj08
        ...
        l3[:, 8] = (
            -bb1 * cc2 * pp08 * (bb1 * pp07 * cc2 * dd6 + dd8 * dd7) ** 2
            - bb1
            * cc2
            * pp08
            * (
    ...
    if deriv > 3:
        ...
        uu2 = xi**3
        l4[:, 0] = (
            uu1 * (ee3 - 3.0) * (ee3 - 2.0) * (ee3 - 1.0) * uu2 * aa1 ** (ee3 - 4.0)
            + 6.0 * uu1 * uu2 * (xi + 1.0) / aa1**4
        )
        vv09 = ee3 - 3.0
        l4[:, 1] = (
            3.0 * kk1 * ll8 * ff7 * kk2 * ll5**vv09
        ...
        ww15 = cc3**ww11
        l4[:, 2] = (
            -kk1 * mm11 * oo10 * kk2 * ww15 * (log_cc3 / kk2 - bb1 * aa2 * cc2 / cc3)
            + 2.0 * kk1 * mm11 * xi * ww15
            - kk1 * ww15
            + uu1 * ww11 * mm11 * kk2 * cc2 * ww12
            - uu1 * oo10 * xi * cc2 * ww12
        ...
        yy18 = log_cc3 / kk2
        l4[:, 4] = (
            -2.0 * ee1 * oo10 * xi * mm12 * (bb1 * mm11 * cc2 * dd6 + yy18)
        ...
        l4[:, 5] = (
            ...
            * xi
            * mm12
            * (ee1 * aa2 * hh4 * jj08 + 2.0 * bb1 * dd8 * cc2 * dd6 - 2.0 * dd7 / xi**3)
            + 4.0 * kk1 * cc2 * ww15
        ...
        l4[:, 7] = (
            -bb1 * cc3**oo10 * (bb1 * oo10 * cc2 * dd6 + yy18)
            - 3.0 * ee1 * oo10 * xi * cc2 * mm12 * (bb1 * mm11 * cc2 * dd6 + yy18)
            - kk1 * mm11 * oo10 * kk2 * hh4 * ww15 * (bb1 * ww11 * cc2 * dd6 + yy18)
            - 3.0 * ee1 * aa2 * cc2 * mm12
            - 3.0 * ee1 * oo10 * cc2 * mm12
        ...
        ah24 = -2.0 * dd7 / xi**3 + 2.0 * bb1 * dd8 * cc2 * dd6 + ee1 * aa2 * hh4 * jj08
        ah27 = tt18**2
        l4[:, 12] = (
            bb1 * cc2 * ad20 * ah27
            + ee1 * oo10 * xi * hh4 * mm12 * ah27
            - 4.0 * ee1 * hh4 * mm12 * tt18
            - 2.0 * kk1 * mm11 * xi * qq05 * ww15 * tt18
            + 2.0 * kk1 * qq05 * ww15 * tt18
            + bb1 * cc2 * ad20 * ah24
            + ee1 * oo10 * xi * hh4 * mm12 * ah24
            + 6.0 * kk1 * qq05 * ww15
        ...
        aj08 = 1.0 / cc3**4
        aj20 = 1.0 / xi**5
        aj23 = -2.0 * jj12 * dd7 + 2.0 * bb1 * dd8 * cc2 * dd6 + ee1 * aa2 * hh4 * jj08
        l4[:, 14] = (
            -jj13 * tt18**4
            - 6.0 * jj13 * aj23 * tt18**2
            - 3.0 * jj13 * aj23**2
            - 4.0
            * jj13
            * (
...
class gevlss(GeneralFamily):
    ...
    def __init__(self, link: tuple[str, str, str] = ("identity", "identity", "logit")):
        if len(link) != 3:
        ...
        scale = IdentityLink()
        shape = ShiftedLogitLink() if link[2] == "logit" else IdentityLink()
    ...
    def initialize_coef(
        ...
        if self._link_names[0] == "identity":
            ...
            yt1 = self.links[0].link(np.abs(y) + float(np.max(y)) * 1e-7)
        start[jj[0]] = _reg(jj[0], yt1)
        lres1 = np.log(np.abs(y - self.links[0].linkinv(X[:, jj[0]] @ start[jj[0]])))
        start[jj[1]] = _reg(jj[1], lres1)
    ...
    def residuals(self, y, fitted, type: str = "deviance") -> np.ndarray:
        ...
        rsd = (
            (xi + 1.0) / xi * np.log(1.0 + ymr)
            + (1.0 + ymr) ** (-1.0 / xi)
            + (1.0 + xi) * np.log(1.0 + xi)
            - (1.0 + xi)
        )
...
def _coxlpl(eta, X, d, time, deriv, d1b=None, d2b=None, D=None, eigen=None):
    ...
    H = -np.sum(
        ...
        * (
            ...
            - b_p[:, :, None] * b_p[:, None, :] / (gamma_p**2)[:, None, None]
        ),
        0,
    )
    ...
    if deriv <= 2:  # ∂H/∂ρ matrices, original basis
        ...
        d1A_p = np.cumsum(
            d1gamma[:, None, None, :] * gXX[..., None] / gamma[:, None, None, None], 0
        )[last]
        xx0 = dr / gamma_p
        xx1 = xx0 / gamma_p
        d1H = np.zeros((p, p, M))
    ...
    d1A_p = np.cumsum(
        d1gamma[:, None, None, :] * gXXp[..., None] / gamma[:, None, None, None], 0
    )[last]
    ...
    for off, (m, k) in enumerate(pairs):
        ...
        contrib = (
            xx3[:, None] * (Adiag * d1gamma_p[:, k][:, None] + 2 * d1b_p[:, :, k] * b_p)
            + xx0[:, None]
            * (
                d1Adiag[:, m, :] * d1gamma_p[:, k][:, None]
            ...
            * xx1[:, None]
            * (
                2 * d1b_p[:, :, m] * b_p * d1gamma_p[:, k][:, None]
...
def _coxpp(eta, X, d, time):
    ...
    b_p = np.cumsum(gamma[:, None] * X, 0)[last] if p else np.zeros((nt, 0))
    dr = np.zeros(nt)
    ...
    km = _rev(dr / gamma_np)
    q = _rev(dr / gamma_p**2)
    a = _rev(b_p * (dr / gamma_p**2)[:, None]) if p else np.zeros((nt, 0))
...
class cox_ph(GeneralFamily):
        ...
        *,
        ...
        if cderiv < 3:
            ...
            if deriv == 2:  # d1H as a TRACE vector (fh=Hp⁻¹)
                ...
                out["d1H"] = [res["d1H"][:, :, m] for m in range(res["d1H"].shape[2])]
            return out
        # deriv == 4 → trHid2H: rebuild eigen(raw Hp) from fh=(L,piv)+D
        eigen = self._eigen_from_fh(fh, D, X.shape[1])
        res = _coxlpl(
            eta, X, d, y, 3, d1b=d1b, d2b=d2b, D=np.asarray(D, float), eigen=eigen
        )
        return {
            "l": res["l"],
            "lb": res["lb"],
    ...
    def postproc(
        ...
        null_dev = 2.0 * float(np.sum(np.abs(w + np.log(s0) + w * np.log(-np.log(s0)))))
        return {"fitted": s_model, "null_deviance": null_dev}

    def residuals(self, y, fitted, type: str = "deviance", prior_weights=None):
        ...
        return np.sign(res) * np.sqrt(
            np.maximum(-2.0 * (res + w * np.log(-log_s)), 0.0)
        )
    ...
    with np.errstate(over="ignore", invalid="ignore"):
        ...
        if deriv > 2:
            ...
            l4[ind] = a[ind] * (
                ...
                - 12.0 * b * (2.0 - b)
                + 4.0 * egi * (1.0 + b)
                + 7.0 * (egi**2 + 2.0 * egi + b * egi**2 + 2.0 * b * egi + b)
                - (4.0 + 3.0 * egi) * egi
...
def _zipll(y, g, eta, deriv=0):
    ...
    with np.errstate(over="ignore", invalid="ignore"):
        ...
        l = et.copy()
        l[zind] = -et[zind]
        l[nz] = _l1ee(eta[nz]) + yp * g[nz] - _lee1(g[nz]) - gammaln(yp + 1.0)
    ret = {"l": l}
    if not deriv:
        return ret
    le = _lde(eta, deriv)
...
def _ziplss_ls(y):
    ...
    ind2 = y > 1
    l[ind2] = _zipll(y[ind2], np.log(g[ind2]), np.full(int(ind2.sum()), 1e10))["l"]
    return l
...
class ziplss(GeneralFamily):
    ...
    def __init__(self, link: tuple[str, str] = ("identity", "identity")):
        ...
        if la != "identity" or lb != "identity":
            raise ValueError(
                'only the "identity" link is available for both parameters of ziplss'
            )
        self.tri = trind_generator(2)
    ...
    def ll(
        ...
        *,
        ...
        if deriv == 0:
            return ret
        ig1 = np.column_stack([self.links[0].mu_eta(eta), self.links[1].mu_eta(eta1)])
        g2 = np.column_stack([self.links[0].d2link(lam), self.links[1].d2link(p)])
        g3 = g4 = None
        if deriv > 1:
            g3 = np.column_stack([self.links[0].d3link(lam), self.links[1].d3link(p)])
        if deriv > 3:
            g4 = np.column_stack([self.links[0].d4link(lam), self.links[1].d4link(p)])
        tri = self.tri
    ...
    def initialize_coef(
        ...
        y = np.asarray(y, dtype=float)
        if not np.allclose(y, np.round(y)):
            raise ValueError("non-integer response values are not allowed with ziplss")
        if y.min() == 0 and y.max() == 1:
    ...
    def postproc(
        ...
        def flam(lam):
            return float(
                np.sum(
                    ypos * np.log(lam) - np.log(np.exp(lam) - 1.0) - gammaln(ypos + 1.0)
                )
            )
    ...
    def residuals(self, y, fitted, type: str = "deviance") -> np.ndarray:
        ...
        if type == "response":
            return rsd
        d = np.maximum(
            0.0, 2.0 * (_ziplss_ls(y) - _zipll(y, fitted[:, 0], fitted[:, 1])["l"])
        )
    ...
    def predict(
        ...
        *,
        ...
        if eta is None:
            ...
            if se:
                v_g = np.maximum(
                    0.0, np.einsum("ij,jk,ik->i", X1, Vb[np.ix_(c1, c1)], X1)
                )
                v_e = v_g  # mgcv copy-paste (gamlss.r:1718 reuses X1/lpi1)
                v_eg = np.maximum(
                    0.0, np.einsum("ij,jk,ik->i", X1, Vb[np.ix_(c1, c2)], X2)
                )
        else:
            eta = np.asarray(eta, dtype=float)
            se = False
        ...
        with np.errstate(over="ignore", invalid="ignore"):
            ...
            lam = np.exp(gamma)
            mu = np.where(
                gamma < np.log(np.finfo(float).eps) / 2.0,
                1.0,
                lam / (1.0 - np.exp(-lam)),
            )
            ey = p * mu
        ...
        with np.errstate(over="ignore", invalid="ignore"):
            df_de = (
                np.where(
                    eta_p < np.log(np.finfo(float).max) / 2.0, np.exp(-et) * et, 0.0
                )
                * mu
            )
            df_dg = ((lam + 1.0) * mu - mu**2) * p
            se_fit = np.sqrt(
                df_dg**2 * v_g + df_de**2 * v_e + 2.0 * df_de * df_dg * v_eg
            )
                ...
                for k in range(j, K):
                    for m in range(k, K):
                        ...
                        if nun == 1:
                            ...
                            l4[:, col] = (
                                l3[:, i3[uni[0], uni[1], uni[2]]] + 6.0 * eijkl / b4
                            )
                        else:  # nun == 2: split 2+2 or 3+1
                            cnt0 = int(np.sum(np.array([i, j, k, m]) == uni[0]))
                            if cnt0 == 2:
                                l4[:, col] = (
                                    l3[:, i3[uni[0], uni[1], uni[1]]]
                                    - 2.0 * ee[:, uni[0]] ** 2 * ee[:, uni[1]] / b3
                                    + 6.0 * eijkl / b4
                                )
...
class multinom(GeneralFamily):
    ...
    def __init__(self, K: int = 1):
        if K < 1:
            raise ValueError(
                "number of categories must be at least 2 (multinom K must be >= 1)"
            )
        self.n_lp = int(K)
    ...
    def ll(
        ...
        *,
        ...
        for i in range(K):
            ...
            eta[:, i] = (
                _Xbd(X.design, coef, lt=X.lpid[i])
                if discrete
                else X[:, jj[i]] @ coef[jj[i]]
            )
            if offset is not None and i < len(offset) and offset[i] is not None:
    ...
    def _initialize_coef_discrete(self, y, X: DiscreteX, jj, E, offset) -> np.ndarray:
        ...
        for k in range(len(jj)):
            yt = 6.0 * (y == k + 1).astype(float) - 3.0
            startji = _DiscreteLPSolve(
                design, lpid[k], E[:, jj[k]], ones_n
            ).solve_target(yt)
            startji[~np.isfinite(startji)] = 0.0
            start[jj[k]] = startji
        return start
    ...
    def residuals(self, y, fitted, type: str = "deviance") -> np.ndarray:
        ...
        if type != "deviance":
            raise ValueError(
                f"only 'deviance' residuals are available for multinom; got {type!r}"
            )
        y = np.round(np.asarray(y, dtype=float)).astype(int)
        ...
        py = p[np.arange(n), y]
        return sgn * np.sqrt(-2.0 * np.log(np.maximum(np.finfo(float).eps, py)))
        ...
        *,
        ...
        if eta is None:
            ...
            for i in range(K):
                ...
                if se:
                    ve[:, i] = np.maximum(
                        0.0, np.einsum("ij,jk,ik->i", Xi, Vb[np.ix_(ci, ci)], Xi)
                    )
                    for k in range(i + 1, K):
                        ck = np.asarray(lpi[k], dtype=int)
                        ce[:, ii] = np.maximum(
                            0.0,
                            np.einsum("ij,jk,ik->i", Xi, Vb[np.ix_(ci, ck)], X[:, ck]),
                        )
                        ii += 1
        else:
            eta = np.asarray(eta, dtype=float)
...
def _mvn_ll(y, X, coef, lpi, m, *, deriv=0, d1b=None, fh=None) -> dict:
    ...
    for kk in range(ntheta):
        i, j = rri[kk], rci[kk]
        lb[ncoef + kk] = (
            n - dth[kk] * Ree_diag[i] if i == j else -float(np.dot(Re[:, i], e[:, j]))
        )
    # ---- Hessian ----
    lbb = np.zeros((nb, nb))
    ...
    for j in range(ntheta):
        ri, rj = rri[j], rci[j]
        col = dth[j] * ((din == rj) * XRe[:, ri] + (ri <= din) * R[ri, din] * Xe[:, rj])
    ...
    for r in range(nsp):
        ...
        for j in range(ntheta):
            ...
            for kk2 in range(ntheta):
                ...
                if ri == rik:
                    c = dth[j] * dth[kk2]
                    z2 = (din == rj) * (yX[rjk, :] * c) + (din == rjk) * (yX[rj, :] * c)
                    if kk2 == j and rik == rjk:
                        z2 = z2 + (
                            ((din == rj) | (din == rjk)) * (dth[kk2] * yRX[rj, :])
                        )
                    partK = partK + z2 * dtheta[kk2]
                if kk2 == j and rik == rjk:
                    partK = partK + (dtheta[kk2] * dth[kk2]) * (R[rj, din] * yX[rj, :])
            col = partQ + partK
        ...
        for j in range(ntheta):
            ...
            for kk2 in range(j, ntheta):
                ...
                if rij == rik:
                    m_jj = din == rjj
                    m_jk = din == rjk
                    z2 = m_jj * (yX[rjk, :] * dth[j] * dth[kk2]) + m_jk * (
                        yX[rjj, :] * dth[j] * dth[kk2]
                    )
                    if kk2 == j and rik == rjk:
                        z2 = z2 + m_jj * (dth[kk2] * yRX[rjj, :])
                    contrib = contrib + z2 * dbq
                if kk2 == j and rij == rjj:
                    contrib = contrib + dbq * dth[kk2] * (R[rjj, din] * yX[rjj, :])
...
class mvn(GeneralFamily):
    ...
    def initialize_coef(
        ...
        for kdim in range(m):
            ...
            scale = max(float(resid @ resid) / max(len(resid) - df, 1), 1e-8)
    ...
    def postproc(
        ...
        y = np.asarray(y, dtype=float)
        fitted = np.asarray(fitted, dtype=float)
        R = self._R if self._R is not None else self._R_from_coef(np.zeros(self.d))
        rsd = (y - fitted) @ R.T
    ...
    def residuals(self, y, fitted, type: str = "deviance") -> np.ndarray:
        ...
        if type != "deviance":
            raise ValueError(
                f"mvn residuals are 'response' or 'deviance'; got {type!r}"
            )
        R = self._R if self._R is not None else self._R_from_coef(np.zeros(self.d))
        return res @ R.T
