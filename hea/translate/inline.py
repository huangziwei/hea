...
def from_R(
    ...
    *,
    ...
    ----------
    r_source
        R source code (typically a triple-quoted string in a notebook
        cell), or a :class:`Result` from a previous call — in which
        case ``.source`` is unwrapped, so round-trip wrapping like
        ``from_R(to_R(r_src))`` Just Works.
    execute
...
def to_R(
    ...
    *,
    ...
    py_source
        ...
        or a :class:`Result` from a previous call — in which case
        ``.source`` is unwrapped, so round-trip wrapping like
        ``to_R(from_R(r_src))`` Just Works.
    save_to
    ...
    Result
        ...
        unless ``execute=True``.
    """
    if isinstance(py_source, Result):
        py_source = py_source.source
