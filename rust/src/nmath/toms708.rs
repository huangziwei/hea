...
fn gamln1(a: f64) -> f64 {
    if a < 0.6 {
        ...
        let w =
            horner(a, &[p6, p5, p4, p3, p2, p1, p0]) / horner(a, &[q6, q5, q4, q3, q2, q1, 1.0]);
        return -a * w;
    }
...
fn bcorr(a0: f64, b0: f64) -> f64 {
    ...
    let mut w = rfma(
        rfma(
            rfma(rfma(rfma(c5 * s11, t, c4 * s9), t, c3 * s7), t, c2 * s5),
            t,
            c1 * s3,
        ),
        t,
...
pub(crate) fn lbeta_scalar(a: f64, b: f64) -> f64 {
    ...
    if p < 1e-306 {
        // C lbeta.c:76 calls the PLATFORM lgamma here, not lgammafn.
        return libm_lgamma(p) + (libm_lgamma(q) - libm_lgamma(p + q));
    }
