...
def test_link_values_match_mgcv(
    ...
    np.testing.assert_allclose(
        lk.link(MUS), link_oracle, rtol=1e-12, atol=0, err_msg=f"{lk.name}.link"
    )
    np.testing.assert_allclose(
        lk.d2link(MUS), d2_oracle, rtol=1e-12, atol=0, err_msg=f"{lk.name}.d2link"
    )
    np.testing.assert_allclose(
        lk.d3link(MUS), d3_oracle, rtol=1e-12, atol=0, err_msg=f"{lk.name}.d3link"
    )
    np.testing.assert_allclose(
    ...
    np.testing.assert_allclose(
        lk.linkinv(ETAS[mask]),
        linkinv_oracle[mask],
        rtol=1e-12,
        atol=0,
        err_msg=f"{lk.name}.linkinv",
    )
    np.testing.assert_allclose(
        lk.mu_eta(ETAS[mask]),
        mu_eta_oracle[mask],
        rtol=1e-12,
        atol=0,
        err_msg=f"{lk.name}.mu_eta",
    )


@pytest.mark.parametrize(
    "cls", [LogitLink, ProbitLink, CauchitLink, CloglogLink, SqrtLink]
)
...
def test_poisson_oracle():
    ...
    np.testing.assert_allclose(
        f.aic(y, mu, None, wt, len(y)), 21.297689743799772, rtol=1e-12, atol=0
    )
    np.testing.assert_allclose(
        f.ls(y, wt, 1.0), [-10.0236819644984, 0.0, 0.0], rtol=1e-12, atol=0
    )
...
def test_binomial_proportion_oracle():
    ...
    np.testing.assert_allclose(
        f.dev_resids(y, mu, wt),
        [0.257320924779854, 0.163287978081021, 0.432017082870932, 0.632163093946958],
        rtol=1e-12,
        atol=0,
    )
    np.testing.assert_allclose(
        f.aic(y, mu, None, wt, len(y)), 7.87389855174967, rtol=1e-12, atol=0
    )
    np.testing.assert_allclose(
        f.ls(y, wt, 1.0), [-3.19455473603545, 0.0, 0.0], rtol=1e-11, atol=0
    )
...
def test_inverse_gaussian_oracle():
    ...
    np.testing.assert_allclose(
        f.ls(y, wt, 2.0), [-6.36339333666077, -2.0, 0.0], rtol=1e-12, atol=0
    )
...
def test_gaussian_unchanged_oracle():
    ...
    np.testing.assert_allclose(
        out, [-0.5 * 4 * np.log(2 * np.pi), -2.0, 0.0], rtol=1e-12, atol=0
    )
...
def test_tweedie_validmu_and_initialize():
    ...
    np.testing.assert_allclose(f.initialize(y, np.ones(3)), np.array([0.1, 1.0, 2.5]))
    with pytest.raises(ValueError, match="negative values"):
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            [
                0.2052672581380978467,
                0.1100000000000001116,
                0.0589475404394961822,
                0.0258416167479364467,
            ],
            [
                -0.3694810646485760519,
                -0.0990000000000000879,
                -0.0265263931977732792,
                -0.0046514910146285603,
            ],
        ),
        (
            1.5,
            ...
            [
                1.060660171779821415,
                0.750000000000000000,
                0.530330085889910707,
                0.335410196624968460,
            ],
            [
                -1.0606601717798214146,
                -0.3750000000000000000,
                -0.1325825214724776768,
                -0.0335410196624968474,
            ],
        ),
        (
            1.9,
            ...
            [
                -0.3665465241874125146,
                -0.1710000000000001241,
                -0.0797743207763970952,
                -0.0291158253502108513,
            ],
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            1.1,
            [
                1.4032130388652341857,
                0.0665577307825964692,
                0.0349267878477985683,
                0.1071552861439123427,
                0.0599447148388876361,
            ],
        ),
        (
            1.5,
            [
                3.0983866769659336171,
                0.0802430753127092444,
                0.0332641767424362023,
                0.0788114206843384402,
                0.0356745147454633482,
            ],
        ),
        (
            1.9,
            [
                19.0040043301135099796,
                0.0968397017685943551,
                0.0316900713608997964,
                0.0579905047662896411,
                0.0212341107387551409,
            ],
        ),
    ],
)
...
def test_tweedie_dev_resids_weighted_oracle():
    ...
    oracle = [
        1.5491933384829668086,
        0.0802430753127092444,
        0.0665283534848724045,
        0.0788114206843384402,
        0.0178372573727316741,
    ]
    np.testing.assert_allclose(
        f.dev_resids(_TW_DEV_Y, _TW_DEV_MU, wt),
        oracle,
        rtol=1e-13,
    )
...
def test_tweedie_dev_limit_to_poisson():
    ...
    np.testing.assert_allclose(
        Tweedie(p=1.001).dev_resids(y, mu, np.ones(4)),
        pois,
        rtol=1e-2,
    )
...
def test_tweedie_dev_limit_to_gamma():
    ...
    np.testing.assert_allclose(
        Tweedie(p=1.999).dev_resids(y, mu, np.ones(4)),
        gam_dev,
        rtol=1e-2,
    )
...
def test_tweedie_log_density_oracle_p15():
    ...
    oracle = [
        -1.549193338482966809,
        -0.608940759999017533,
        -1.045247308713201040,
        -1.710387087424116714,
...
def test_tweedie_log_density_oracle_p17():
    ...
    oracle = [
        -1.429862000740157901,
        -0.970520782376560809,
        -1.491788879150033331,
        -2.222992852580873091,
...
def test_tw_dls_dp_chain_to_theta_oracle_p15():
    ...
    np.testing.assert_allclose(dls_dp * f.dp_dtheta(), -0.1740833687231026, rtol=1e-9)
...
def test_tw_dls_dp_chain_to_theta_oracle_p17():
    ...
    np.testing.assert_allclose(
        dls_dp * f.dp_dtheta(), -0.0302015497694411161, rtol=1e-9
    )
...
def test_tw_ls_extended_lsth2_matches_mgcv():
    ...
    expected = {
        0.8: (
            -133.8139693830,
            [-4.6929474731, -66.9089188162],
            [[-0.7111684232, 2.4000778201], [2.4000778201, -13.8069254790]],
        ),
        1.3: (
            -168.3807909970,
            [-2.3840557958, -76.5635517288],
            [[-3.3107988091, 8.0792314245], [8.0792314245, -27.1024409298]],
        ),
    }
    for sc, (ls_r, lsth1_r, lsth2_r) in expected.items():
        ...
        np.testing.assert_allclose(out["lsth1"], lsth1_r, rtol=0, atol=1e-8)
        np.testing.assert_allclose(out["lsth2"], lsth2_r, rtol=0, atol=1e-8)
...
@pytest.mark.skipif(
    ...
    reason="scat estimate.theta oracle missing — run tests/r_oracle/dump_scat_estth.R",
)
...
def test_scat_estimate_theta(init_name: str):
    ...
    s = Scat(min_df=4)
    s.set_theta(th0)
    th_h = _estimate_theta(s, y, mu, scale=1.0, wt=np.ones(len(y)), tol=1e-7)
...
def _scat_bam_have(name: str) -> bool:
    sub = _SCAT_BAM / name
    return all(
        (sub / f).exists() for f in ("data.csv", "sp.csv", "theta.csv", "fitted.csv")
    )


def _scat_bam_have_force(name: str) -> bool:
    return _scat_bam_have(name) and (_SCAT_BAM / name / "fitted_force.csv").exists()
...
def test_scat_bam_simple_force_theta_sp():
    ...
    dat = {"y": df["y"].to_numpy().astype(float), "x": df["x"].to_numpy().astype(float)}
...
def test_scat_bam_simple_auto_fit():
    ...
    dat = {"y": df["y"].to_numpy().astype(float), "x": df["x"].to_numpy().astype(float)}
    m = hea.models.bam("y ~ s(x, k=10)", dat, family=Scat(min_df=5), discrete=True)
    fit_h = np.asarray(m.fitted_values)
...
def test_scat_bam_factor_auto_fit():
    ...
    m = hea.models.bam(
        "y ~ g + s(x, by=g, k=10)", dat, family=Scat(min_df=5), discrete=True
    )
    fit_h = np.asarray(m.fitted_values)
    rel = float(np.linalg.norm(fit_h - fit_mgcv) / np.linalg.norm(fit_mgcv))
...
def test_scat_bam_nondiscrete_matches_mgcv(sub, formula, has_g):
    ...
    if has_g:
        dat["g"] = df["g"].to_numpy()
    m = hea.models.bam(
        formula, dat, family=Scat(min_df=5), method="fREML", discrete=False
    )
    fit_h = np.asarray(m.fitted_values)
    rel = float(np.linalg.norm(fit_h - fit_mgcv) / np.linalg.norm(fit_mgcv))
    assert rel < 1e-8, f"scat non-discrete {sub} fitted rel diff {rel:.3e}"
    np.testing.assert_allclose(
        np.asarray(m.family.get_theta(trans=True)), theta_mgcv, rtol=1e-6, atol=0
    )
    np.testing.assert_allclose(
        np.sort(np.asarray(m.sp)), np.sort(sp_mgcv), rtol=1e-6, atol=0
    )
...
def test_trind_generator_ifunc_reverse_match_mgcv():
    ...
    np.testing.assert_array_equal(ta["i2r"] + 1, [1, 2, 3, 4, 6, 7, 8, 11, 12, 16])
    np.testing.assert_array_equal(
        ta["i3r"] + 1,
        [1, 2, 3, 4, 6, 7, 8, 11, 12, 16, 22, 23, 24, 27, 28, 32, 43, 44, 48, 64],
    )
    np.testing.assert_array_equal(
        ta["i4r"] + 1,
        [
    ...
    for K in range(2, 6):
        ...
        np.testing.assert_array_equal(packed3[a["i3"]].ravel()[a["i3r"]], packed3)
        packed2 = np.arange(a["i2"].max() + 1, dtype=float)
        np.testing.assert_array_equal(packed2[a["i2"]].ravel()[a["i2r"]], packed2)
...
def test_gaulss_ll_matches_mgcv_oracle():
    ...
    np.testing.assert_allclose(r1["l"], -57.9372871859, rtol=0, atol=1e-10)
    np.testing.assert_allclose(
        r1["lb"],
        [4.0145452690, -1.1669739590, 6.2728855900, -22.5854933110, -7.3560025880],
        rtol=0,
        atol=1e-9,
    )
    np.testing.assert_allclose(r1["lbb"][0, 0], -21.7376940450, rtol=0, atol=1e-9)
    np.testing.assert_allclose(
        float(np.sum(np.abs(r1["lbb"]))), 269.7531350500, rtol=0, atol=1e-8
    )
    np.testing.assert_allclose(r1["lbb"][0, 4], 0.6777580010, rtol=0, atol=1e-9)

    Hp = -r1["lbb"] + np.eye(5) * 0.5
    r2 = fam.ll(y, X, coef, lpi=lpi, deriv=2, d1b=d1b, fh=np.linalg.inv(Hp))
    np.testing.assert_allclose(
        r2["d1H"], [-3.0610301460, -0.2929096240], rtol=0, atol=1e-9
    )

    r3 = fam.ll(y, X, coef, lpi=lpi, deriv=3, d1b=d1b)
    np.testing.assert_allclose(
        float(np.sum(np.abs(r3["d1H"][0]))), 406.9875674700, rtol=0, atol=1e-8
    )
    np.testing.assert_allclose(
        float(np.sum(np.abs(r3["d1H"][1]))), 116.4523491000, rtol=0, atol=1e-8
    )
    np.testing.assert_allclose(r3["d1H"][0][0, 0], -13.7169040930, rtol=0, atol=1e-9)

    D = 1.0 / np.sqrt(np.diag(Hp))
    R = cholesky(D[:, None] * Hp * D[None, :], lower=False)
    r4 = fam.ll(
        y, X, coef, lpi=lpi, deriv=4, d1b=d1b, d2b=d2b, fh=(R, np.arange(5)), D=D
    )
    np.testing.assert_allclose(
...
def test_gaulss_ll_derivatives_match_fd():
    ...
    for k in range(p):
        ...
        cm = coef.copy()
        cm[k] -= h
        fd_lb[k] = (fam.ll(y, X, cp, lpi=lpi)["l"] - fam.ll(y, X, cm, lpi=lpi)["l"]) / (
            2 * h
        )
        fd_lbb[:, k] = (
            fam.ll(y, X, cp, lpi=lpi, deriv=1)["lb"]
            - fam.ll(y, X, cm, lpi=lpi, deriv=1)["lb"]
        ) / (2 * h)
    ...
    for j in range(d1b.shape[1]):
        fdH = (
            fam.ll(y, X, coef + h * d1b[:, j], lpi=lpi, deriv=1)["lbb"]
            - fam.ll(y, X, coef - h * d1b[:, j], lpi=lpi, deriv=1)["lbb"]
        ) / (2 * h)
...
def test_gammals_ll_matches_mgcv_oracle():
    ...
    np.testing.assert_allclose(
        r1["lb"],
        [10.4292062387, 11.368308003, -18.2209272134, -1.5743414661],
        rtol=0,
        atol=1e-8,
    )
    np.testing.assert_allclose(r1["lbb"][0, 0], -99.9024955545, rtol=0, atol=1e-8)
    np.testing.assert_allclose(
        float(np.sum(np.abs(r1["lbb"]))), 311.139546477, rtol=0, atol=1e-7
    )
    np.testing.assert_allclose(r1["lbb"][0, 2], -10.4058942597, rtol=0, atol=1e-8)
    np.testing.assert_allclose(r1["lbb"][2, 2], -4.74349105844, rtol=0, atol=1e-8)
    ...
    r3 = fam.ll(y, X, coef, lpi=lpi, deriv=3, d1b=d1b)
    np.testing.assert_allclose(
        float(np.sum(np.abs(r3["d1H"][0]))), 136.804494605, rtol=0, atol=1e-7
    )
    np.testing.assert_allclose(
        float(np.sum(np.abs(r3["d1H"][1]))), 193.697974752, rtol=0, atol=1e-7
    )
    np.testing.assert_allclose(r3["d1H"][0][0, 0], -12.4965794289, rtol=0, atol=1e-8)

    D = 1.0 / np.sqrt(np.diag(Hp))
    R = cholesky(D[:, None] * Hp * D[None, :], lower=False)
    r4 = fam.ll(
        y, X, coef, lpi=lpi, deriv=4, d1b=d1b, d2b=d2b, fh=(R, np.arange(p)), D=D
    )
    np.testing.assert_allclose(
        r4["trHid2H"],
        [-2.07694884277, 3.65876948038, -9.30036542487],
        rtol=0,
        atol=1e-8,
    )
...
def test_gammals_initialize_residuals_and_link():
    ...
    np.testing.assert_allclose(
        fam.residuals(yy, fitted, type="response"), yy - mu, atol=0
    )
    np.testing.assert_allclose(
        fam.residuals(yy, fitted, type="pearson"),
        (yy - mu) / (np.exp(rho * 0.5) * mu),
        atol=0,
    )
    dexp = np.sqrt(
...
def test_gumbls_ll_matches_mgcv_oracle():
    ...
    np.testing.assert_allclose(
        r1["lb"],
        [5.76161239291, 2.53486659051, 24.5358131205, -24.1421493465],
        rtol=0,
        atol=1e-8,
    )
    np.testing.assert_allclose(r1["lbb"][0, 0], -97.2333719429, rtol=0, atol=1e-8)
    np.testing.assert_allclose(
        float(np.sum(np.abs(r1["lbb"]))), 963.95750617, rtol=0, atol=1e-6
    )
    np.testing.assert_allclose(r1["lbb"][0, 2], 38.9766385199, rtol=0, atol=1e-8)
    np.testing.assert_allclose(r1["lbb"][2, 2], -133.911330844, rtol=0, atol=1e-7)
    ...
    r3 = fam.ll(y, X, coef, lpi=lpi, deriv=3, d1b=d1b)
    np.testing.assert_allclose(
        float(np.sum(np.abs(r3["d1H"][0]))), 1488.93342144, rtol=0, atol=1e-6
    )
    np.testing.assert_allclose(
        float(np.sum(np.abs(r3["d1H"][1]))), 3114.42700999, rtol=0, atol=1e-6
    )
    np.testing.assert_allclose(r3["d1H"][0][0, 0], 81.6327066253, rtol=0, atol=1e-7)
    ...
    np.testing.assert_allclose(
        r4["trHid2H"],
        [-9.32912949141, -9.22534206361, -22.1211490539],
        rtol=0,
        atol=1e-7,
    )
...
def test_gumbls_residuals_and_validation():
    ...
    mu = mean - beta * euler
    np.testing.assert_allclose(
        fam.residuals(yy, fitted, type="response"), yy - mean, atol=0
    )
    np.testing.assert_allclose(
        fam.residuals(yy, fitted, type="pearson"),
        (yy - mean) / (np.pi * beta / np.sqrt(6.0)),
        atol=0,
    )
    z = (yy - mu) / beta
    dexp = np.sqrt(np.maximum(0.0, 2.0 * (z + np.exp(-z) - 1.0))) * np.sign(yy - mu)
    np.testing.assert_allclose(fam.residuals(yy, fitted), dexp, atol=0)
...
def test_gevlss_ll_matches_mgcv_oracle():
    ...
    np.testing.assert_allclose(
        float(np.sum(np.abs(r1["lbb"]))), 279.1671661, rtol=0, atol=1e-6
    )
    ...
    np.testing.assert_allclose(
        float(np.sum(np.abs(r3["d1H"][0]))), 301.8381504, rtol=0, atol=1e-6
    )
    np.testing.assert_allclose(
        float(np.sum(np.abs(r3["d1H"][1]))), 457.9403905, rtol=0, atol=1e-6
    )
    ...
    np.testing.assert_allclose(
        r4["trHid2H"], [-2.058358974, 6.146709207, -10.58300064], rtol=0, atol=1e-7
    )
...
def test_gevlss_link_residuals_and_validation():
    ...
    np.testing.assert_allclose(
        fam.residuals(yy, fitted, type="response"), yy - fv, atol=1e-12
    )
...
def test_cox_ph_ll_matches_mgcv_oracle():
    ...
    np.testing.assert_allclose(
        r1["lb"], [-1.001195162311, 0.829726360424], rtol=0, atol=1e-9
    )
    np.testing.assert_allclose(
        r1["lbb"],
        [[-3.607277244680, -0.991185222861], [-0.991185222861, -1.796449359815]],
        rtol=0,
        atol=1e-9,
    )
    ...
    np.testing.assert_allclose(
        r3["d1H"][0],
        [[-0.0241339294506, 0.0590998865919], [0.0590998865919, 0.0727777783225]],
        rtol=0,
        atol=1e-9,
    )
    np.testing.assert_allclose(
        r3["d1H"][1],
        [[-0.0090719771863, 0.0543165991505], [0.0543165991505, 0.0539383891138]],
        rtol=0,
        atol=1e-9,
    )
...
def test_cox_ph_ll_derivatives_match_fd():
    ...
    for k in range(p):
        ...
        cm[k] -= h
        fd_lb[k] = (
            fam.ll(time, X, cp, d, lpi=lpi, deriv=0)["l"]
            - fam.ll(time, X, cm, d, lpi=lpi, deriv=0)["l"]
        ) / (2 * h)
        fd_lbb[:, k] = (
            fam.ll(time, X, cp, d, lpi=lpi, deriv=1)["lb"]
            - fam.ll(time, X, cm, d, lpi=lpi, deriv=1)["lb"]
        ) / (2 * h)
    np.testing.assert_allclose(r1["lb"], fd_lb, rtol=0, atol=1e-6)
    np.testing.assert_allclose(r1["lbb"], 0.5 * (fd_lbb + fd_lbb.T), rtol=0, atol=1e-6)
    r3 = fam.ll(
        time,
        X,
        beta,
    ...
    for j in range(d1b.shape[1]):
        fdH = (
            fam.ll(time, X, beta + h * d1b[:, j], d, lpi=lpi, deriv=1)["lbb"]
            - fam.ll(time, X, beta - h * d1b[:, j], d, lpi=lpi, deriv=1)["lbb"]
        ) / (2 * h)
        np.testing.assert_allclose(r3["d1H"][j], fdH, rtol=0, atol=1e-4)
...
def test_cox_ph_predict_se_matches_mgcv():
    ...
    m = hea.models.gam("time ~ s(x) + z", df, family=cox_ph(), weights=d, method="REML")
    ...
    np.testing.assert_allclose(
        fit[ix],
        [0.9419122191, 0.7060079479, 0.5640457392, 0.07442545795, 0.9253112995],
        rtol=1e-4,
    )
    np.testing.assert_allclose(
        se[ix],
        [0.02707868482, 0.08956395605, 0.1063837158, 0.04154851196, 0.03308801276],
        rtol=1e-4,
    )
...
def test_ziplss_ll_matches_mgcv_oracle():
    ...
    np.testing.assert_allclose(
        r1["lb"],
        [10.3739285482, 7.0452932554, -14.9712013386, 17.7409397677],
        rtol=0,
        atol=1e-8,
    )
    np.testing.assert_allclose(r1["lbb"][0, 0], -25.7869884817, rtol=0, atol=1e-8)
    np.testing.assert_allclose(
    ...
    np.testing.assert_allclose(r1["lbb"][3, 3], -31.0262755407, rtol=0, atol=1e-8)
    ...
    r3 = fam.ll(y, X, coef, lpi=lpi, deriv=3, d1b=d1b)
    np.testing.assert_allclose(
        float(np.sum(np.abs(r3["d1H"][0]))), 104.8260123501, rtol=0, atol=1e-7
    )
    np.testing.assert_allclose(
        float(np.sum(np.abs(r3["d1H"][1]))), 35.1235191591, rtol=0, atol=1e-7
    )
    np.testing.assert_allclose(r3["d1H"][0][0, 0], -25.5797222722, rtol=0, atol=1e-8)
...
def test_ziplss_helpers_initialize_and_validation():
    ...
    np.testing.assert_allclose(
        _lee1(np.array([-30.0, -1, 0.5, 3, 8])),
        [-30, -0.810427655249, 1.43516208533, 20.0855369213, 2980.95798704],
        rtol=1e-11,
        atol=0,
    )
    lg = _ldg(np.array([-30.0, -1, 0.5, 2]), 4)
    np.testing.assert_allclose(
        lg[0],
        [-1, -1.19519230416, -2.04124350898, -7.39362520396],
        rtol=1e-10,
        atol=1e-13,
    )
    np.testing.assert_allclose(
        lg[3],
        [-4.67881148442e-14, -0.272552750274, -2.10286784068, -6.80541955362],
        rtol=1e-9,
        atol=1e-13,
    )
    le = _lde(np.array([-30.0, -1, 0.5, 2]), 4)
    np.testing.assert_allclose(
        le[0],
        [1, 0.82731286299, 0.39252223828, 0.00456910503104],
        rtol=1e-10,
        atol=1e-13,
    )
    np.testing.assert_allclose(
        le[3],
        [-4.67881148442e-14, -0.0953266908979, 0.45414656998, -0.583636545308],
        rtol=1e-9,
        atol=1e-13,
    )
    ...
    ey = np.where(
        lam > np.sqrt(np.finfo(float).eps), p_pres * lam / (1.0 - np.exp(-lam)), p_pres
    )
    np.testing.assert_allclose(
        fam.residuals(yy, fitted, type="response"), yy - ey, rtol=0, atol=1e-12
    )
    dev = np.sqrt(
        np.maximum(0.0, 2.0 * (_ziplss_ls(yy) - _zipll(yy, gamma, eta)["l"]))
    ) * np.sign(yy - ey)
    np.testing.assert_allclose(fam.residuals(yy, fitted), dev, rtol=0, atol=1e-12)
    ...
    np.testing.assert_allclose((draws == 0).mean(), 1.0 - p04, rtol=0, atol=5e-3)
...
def _multinom_oracle_inputs(K, seed):
    ...
    coef = np.array(
        [0.3, 0.5, -0.2, 0.4, 0.1, -0.3, 0.25, -0.15, 0.2, -0.1, 0.35, -0.05]
    )[:p]
...
def test_multinom_ll_matches_mgcv_oracle(K, seed):
    ...
    np.testing.assert_allclose(
        float(np.sum(np.abs(r1["lb"]))), ref["lb_sumabs"], rtol=0, atol=1e-7
    )
    np.testing.assert_allclose(
        float(np.sum(np.abs(r1["lbb"]))), ref["lbb_sumabs"], rtol=0, atol=1e-7
    )
    ...
    r2 = fam.ll(y, X, coef, lpi=lpi, deriv=2, d1b=d1b, fh=np.linalg.inv(Hp))
    np.testing.assert_allclose(
        float(np.sum(np.abs(r2["d1H"]))), ref["d1H_sumabs"], rtol=0, atol=1e-8
    )

    r3 = fam.ll(y, X, coef, lpi=lpi, deriv=3, d1b=d1b)
    np.testing.assert_allclose(
        float(np.sum(np.abs(r3["d1H"][0]))), ref["d1H0mat_sumabs"], rtol=0, atol=1e-7
    )
    ...
    np.testing.assert_allclose(r4["trHid2H"], ref["trHid2H"], rtol=0, atol=1e-8)
...
def test_ld_tweedie_work_matches_mgcv():
    ...
    ld2 = _ld_tweedie_work(y, mu, np.full(12, -0.4), np.full(12, 0.3), a=1.001, b=1.999)
...
def test_tweedious_work_census_matches_mgcv():
    ...
    ref_a = np.array(
        [
            ...
            [
                ...
                -0.50475471006910233,
                -0.18716727544502287,
                -0.030795792745489936,
                0.016107960352173212,
                0.003979707263191079,
                0.52310348883908286,
                -2.2991572922796197,
                0.011180982609209232,
                -0.52310348883908275,
            ],
            [
                -4.6700608899589611,
            ...
            [
                ...
                -6.1287129375144076,
                -0.20016137880103146,
                -0.040811505725868003,
                -0.0049773355173690081,
                13.028115860440705,
                -43.999738452679857,
                2.0627180103847307,
                -13.028115860440701,
            ...
            [
                ...
                -0.31814283393871179,
                -0.36640220138995261,
                0.005789360321502901,
                -0.00076539691664390119,
                -0.045754563728800823,
                -0.31381136846395641,
                -0.062793214982665616,
                0.072441456503184301,
                0.31381136846395652,
            ],
            [
                -0.85239951038377426,
            ...
            [
                ...
                -1.43974297841309,
                0.26110063952984586,
                -0.084559943229046031,
                -0.10526036319805643,
                -0.50497081351605611,
                0.063807521808889489,
                0.13174943081423771,
                0.50497081351605611,
            ],
        ]
    ...
    ha = _ld_tweedie_work(
        y, mu, np.full(y.size, 0.42), np.full(y.size, -0.18), a=a, b=b
    )
    da = np.abs(ha - ref_a)
    ...
    ref_b = np.array(
        [
            ...
            [
                ...
                0.15708628769562827,
                -0.42965501973369941,
                0.090161408870314361,
                0.090070322911922643,
                0.42965501973369924,
            ],
            [
                -2.9436597498570594,
                0.045020071116603333,
                -0.65963055362297851,
                -0.058145984291012809,
                -0.069244179728089605,
                -0.18321781352929989,
                0.58594430812455112,
                -0.51400275838100984,
                -0.11582051842018701,
            ...
            [
                ...
                -0.57124891011751711,
                -0.21496671026132308,
                -0.024118068459787256,
                0.020153990710539027,
                0.021942680870590259,
                0.37296120126773802,
                -1.6530979336228049,
                0.0076986412837158945,
                -0.37296120126773802,
            ],
            [
                -5.3108844809731011,
            ...
            [
                ...
                0.17679190184085236,
                -0.83808098594142155,
                -0.024163778105698919,
                -0.020590476926250822,
                -0.17585840444955902,
                0.71435086150006677,
                -0.70433590658080458,
                -0.11252315767455587,
            ...
            [
                ...
                -0.36431170636571952,
                -0.39848942525833841,
                -0.022184841297629987,
                0.0085420708319730032,
                -0.12530324298298434,
                -0.29761888650271917,
                -0.090432933969829529,
                0.063495589333968275,
                0.29761888650271912,
            ],
            [
                -1.1724235641680947,
                -0.0099735379975003369,
                -2.4047859812912211,
                0.25559082276264228,
                -0.2574656548431713,
                0.9282376959228793,
                -1.0589106224895881,
                0.040140509529614565,
                0.074697317856199333,
                1.0589106224895881,
            ],
            [
                -6.3175419733565654,
            ...
            [
                ...
                0.22575174229707606,
                -0.10292788676574682,
                0.0035353533352247002,
                -0.32733907104087095,
                0.045866538242877397,
                0.081764267342503374,
                0.32733907104087118,
            ],
        ]
    )
...
def test_twlss_ll_oracle_matches_mgcv():
    ...
    np.testing.assert_allclose(ll["l"], -113.4552049409, rtol=0, atol=1e-8)
    np.testing.assert_allclose(
        ll["lb"],
        [
            14.0826693067,
    ...
    for k in range(7):
        ...
        cm[k] -= h
        fd[k] = (
            fam.ll(yt, X, cp, np.ones(n), lpi=lpi, deriv=0)["l"]
            - fam.ll(yt, X, cm, np.ones(n), lpi=lpi, deriv=0)["l"]
        ) / (2 * h)
    ...
    with pytest.raises(ValueError, match="1<a<b<2"):
        twlss(a=1.5, b=1.2)
    assert (
        repr(twlss()) == "twlss(link=('log', 'identity', 'identity'), a=1.01, b=1.99)"
    )
...
def test_shash_ll_matches_mgcv_oracle():
    ...
    np.testing.assert_allclose(r1["l"], -100.9497210553, rtol=0, atol=1e-8)
    np.testing.assert_allclose(
        r1["lb"],
        [
            37.6260962202,
    ...
    np.testing.assert_allclose(
        float(np.abs(r1["lbb"]).sum()), 10088.3601306536, rtol=0, atol=1e-7
    )
    np.testing.assert_allclose(
    ...
    r2 = fam.ll(y, X, coef, None, lpi=lpi, deriv=2, d1b=d1b, fh=np.linalg.inv(Hp))
    np.testing.assert_allclose(
        r2["d1H"], [-6.7252978570, 7.5418328552], rtol=0, atol=1e-8
    )

    r3 = fam.ll(y, X, coef, None, lpi=lpi, deriv=3, d1b=d1b)
    np.testing.assert_allclose(
        [float(np.abs(r3["d1H"][0]).sum()), r3["d1H"][0][0, 0]],
        [11283.4240316976, -202.6392545658],
        rtol=0,
        atol=1e-7,
    )
    np.testing.assert_allclose(
        [float(np.abs(r3["d1H"][1]).sum()), r3["d1H"][1][0, 0]],
        [9755.5096903696, 167.1180992130],
        rtol=0,
        atol=1e-7,
    )
    ...
    for k in range(9):
        ...
        cm[k] -= h
        fd_lb[k] = (
            fam.ll(y, X, cp, None, lpi=lpi, deriv=0)["l"]
            - fam.ll(y, X, cm, None, lpi=lpi, deriv=0)["l"]
        ) / (2 * h)
        fd_lbb[:, k] = (
            fam.ll(y, X, cp, None, lpi=lpi, deriv=1)["lb"]
            - fam.ll(y, X, cm, None, lpi=lpi, deriv=1)["lb"]
        ) / (2 * h)
...
def test_shash_hooks_match_mgcv():
    ...
    np.testing.assert_allclose(
        ...
        [-1.3300547156, -1.0212378918, 2.4582366281],
        rtol=0,
        atol=1e-8,
    )
    np.testing.assert_allclose(
        fam.residuals(y, F, "response")[:3],
        [-0.0399933645, -0.2195953901, 0.4355807611],
        rtol=0,
        atol=1e-8,
    )
    with pytest.raises(ValueError, match="deviance"):
    ...
    np.testing.assert_allclose(
        ...
        [-0.5698615266, 0.8685826873, 1.7077315470],
        rtol=0,
        atol=1e-8,
    )
    np.testing.assert_allclose(
        fam.cdf(y, F, None, None, False)[[0, 19, 39]],
        [0.8922773135, 0.0002822583, 0.4429213918],
        rtol=0,
        atol=1e-8,
    )
    np.testing.assert_allclose(
        fam.cdf(y, F, None, None, True)[[0, 19, 39]],
        [-0.1139783051, -8.1726878430, -0.8143629698],
        rtol=0,
        atol=1e-8,
    )
...
def test_mvn_ll_matches_mgcv_oracle(m, seed):
    ...
    np.testing.assert_allclose(r1["lb"], ref["lb"], rtol=0, atol=1e-8)
    np.testing.assert_allclose(
        float(np.sum(r1["lbb"])), ref["lbb_sum"], rtol=0, atol=1e-7
    )
    np.testing.assert_allclose(
    ...
    np.testing.assert_allclose(
        float(np.sqrt(np.sum(r3["d1H"][1] ** 2))), ref["d1H1_fro"], rtol=0, atol=1e-7
    )
    np.testing.assert_allclose(
        float(np.sum(r3["d1H"][0])), ref["d1H0_sum"], rtol=0, atol=1e-6
    )
    np.testing.assert_allclose(
...
def test_mvn_components_and_validation():
    ...
    np.testing.assert_allclose(
        np.diag(R), [np.exp(0.05), np.exp(-0.52)], rtol=0, atol=1e-12
    )
    ...
    rsd = (y - fitted) @ R.T
    np.testing.assert_allclose(
        pp["deviance"], float(np.sum(rsd**2)), rtol=0, atol=1e-12
    )
    rsd0 = (y - y.mean(axis=0)) @ R.T
    np.testing.assert_allclose(
        pp["null_deviance"], float(np.sum(rsd0**2)), rtol=0, atol=1e-12
    )
    np.testing.assert_allclose(
        fam.residuals(y, fitted, type="deviance"), rsd, rtol=0, atol=1e-12
    )
    np.testing.assert_allclose(
    ...
    start = fam.initialize_coef(Y, X, lpi, E=E, offset=[None, None], use_unscaled=True)
...
def test_general_family_weighted_ll_duplication_identity(name):
    ...
    for k in ("l", "lb", "lbb"):
        np.testing.assert_array_equal(np.asarray(r_none[k]), np.asarray(r_ones[k]))
...
def test_gaulss_weighted_ll_matches_fd():
    ...
    for k in range(p):
        ...
        cm[k] -= h
        fd_lb[k] = (
            fam.ll(y, X, cp, w, lpi=lpi)["l"] - fam.ll(y, X, cm, w, lpi=lpi)["l"]
        ) / (2 * h)
        fd_lbb[:, k] = (
            fam.ll(y, X, cp, w, lpi=lpi, deriv=1)["lb"]
            - fam.ll(y, X, cm, w, lpi=lpi, deriv=1)["lb"]
        ) / (2 * h)
    np.testing.assert_allclose(r1["lb"], fd_lb, rtol=0, atol=1e-6)
    np.testing.assert_allclose(r1["lbb"], fd_lbb, rtol=0, atol=1e-6)
    ...
    R = cholesky(D[:, None] * Hp * D[None, :], lower=False)
    r4 = fam.ll(
        y, X, coef, w, lpi=lpi, deriv=4, d1b=d1b, d2b=d2b, fh=(R, np.arange(p)), D=D
    )
...
def test_betar_Dd_matches_fd():
    ...
    h = 1e-6
    fd_mu = (fam.dev_resids(y, mu + h, wt) - fam.dev_resids(y, mu - h, wt)) / (2 * h)
    fd_mu2 = (
        fam.dev_resids(y, mu + h, wt)
        - 2 * fam.dev_resids(y, mu, wt)
        + fam.dev_resids(y, mu - h, wt)
    ) / h**2
    fd_th = (
        fam.dev_resids(y, mu, wt, theta=th + h)
        - fam.dev_resids(y, mu, wt, theta=th - h)
    ) / (2 * h)
...
def test_ocat_components_match_mgcv():
    ...
    np.testing.assert_allclose(
        ...
        [
            0.11511358970464147,
            -0.056058657808741363,
            -0.0035821305595196231,
            -0.13054122133675861,
            -0.13895868706639994,
            -0.0837958137273902,
            0.18171549534589687,
    ...
    np.testing.assert_allclose(
        ...
        np.array(
            [
                [0, 0],
                [-2.0018700821058992, 0],
                [-0.0085802873275185494, -1.8345684322093283],
                [1.2847448633611565, 2.5871584470023166],
                [-2.0755253980313082, 0],
                [-0.20154634609759842, -2.0400833323027197],
                [0, 0],
                [0.93950892609768777, 1.8919386436218986],
            ]
        ),
        rtol=0,
        atol=1e-9,
    ...
    np.testing.assert_allclose(
        ...
        np.array(
            [
                ...
                [0, 0],
                [-0.64163658792646583, -1.2920974161490391],
            ]
        ),
        rtol=0,
        atol=1e-9,
    ...
    np.testing.assert_allclose(
        ...
        np.array(
            [
                ...
                [0, 0],
                [0.23477341634231763, 0.47277560280143555],
            ]
        ),
        rtol=0,
        atol=1e-9,
    ...
    np.testing.assert_allclose(
        ...
        np.array(
            [
                [0, 0],
                [0.17468144108232198, 0],
                [0.19998018354312383, 0.206340975445134],
                [0.34459113220558429, 0.69392132544931229],
                [0.18489616284655114, 0],
                [0.19439168956731451, 0.30757115418809855],
                [0, 0],
                [0.19196367892522651, 0.38656737817166814],
            ]
        ),
        rtol=0,
        atol=1e-9,
    ...
    np.testing.assert_allclose(
        pre["Theta"], [-0.070896606721916375, 0.052931367693468845], rtol=0, atol=1e-9
    )
...
def test_ocat_Dd_matches_fd():
    ...
    h = 1e-6
    fd_mu = (
        fam.dev_resids(y0, mu + h, wt, theta=th)
        - fam.dev_resids(y0, mu - h, wt, theta=th)
    ) / (2 * h)
    h2 = 1e-4  # second difference: coarser step keeps cancellation in check
    fd_mu2 = (
        fam.dev_resids(y0, mu + h2, wt, theta=th)
    ...
    for k in range(2):
        ...
        thm[k] -= h
        fd_thk = (
            fam.dev_resids(y0, mu, wt, theta=thp)
            - fam.dev_resids(y0, mu, wt, theta=thm)
        ) / (2 * h)
...
def test_ziP_components_match_mgcv():
        ...
        np.column_stack(
            [
                [
                    ...
                    -0.37026076261421353,
                    3.4944167768696115,
                    -0.050742490399748402,
                    1.0654560531477617,
                    -0.0026880592208130411,
                    -1.7966052442293883,
                ],
                [
                    -0.535603765182178,
    ...
    np.testing.assert_allclose(
        fam.aic(y, mu, 0, wt, 0, theta=th), 20.865561995417881, rtol=0, atol=1e-9
    )
    np.testing.assert_allclose(
...
def test_ziP_b_nonzero_components_match_mgcv():
        ...
        np.column_stack(
            [
                [
                    ...
                    -0.21444505775023529,
                    3.6008376112485259,
                    -0.0071065252647155115,
                    1.0034087229472164,
                    -2.8606088762550079e-05,
                    -1.6035680088371769,
                ],
                [
                    -0.46099843289559428,
                    ...
                    -0.28284810248746428,
                    0.59367775620024843,
                    -0.014060015237645187,
                    -0.33086826094582555,
                    -7.0745200521528338e-05,
                    -1.0575346740736420,
                ],
            ]
        ),
        ...
        np.column_stack(
            [
                [
                    ...
                    0.09254172245864013,
                    1.95536392161329253,
                    0.00071232090990356114,
                    3.17534311039586292,
                ],
                [
                    0.0236394138481182958,
                    -0.8327005217278636851,
                    1.0955456626414211,
                    7.09369003345143323,
                    0.1713739281290409533,
                    1.0095712868244369,
                    0.0017144644865693542,
                    -0.5497343940317986899,
                ],
            ]
        ),
        rtol=0,
    ...
    np.testing.assert_allclose(
        D["Dth2"][:, 2],
        [
            -0.0809694718583982564,
            -0.2842163189121490463,
            0.6979680159088302860,
            0.6915586706591358990,
            0.1718251907663267686,
            -0.2217663530216369416,
            0.0021649004419334612,
            -0.3488461525583514966,
        ],
        rtol=0,
        atol=1e-9,
    )
    np.testing.assert_allclose(
        D["Dmuth2"][:, 2],
        [
            -0.755905910445411555,
            0.560366653477998300,
            0.878061772576305133,
            9.242050942630795518,
            -1.524462157900505987,
            0.131161416228383843,
            -0.046277012289079662,
            3.232019812313918106,
        ],
        rtol=0,
        atol=1e-8,
    )
    np.testing.assert_allclose(
        ...
        [
            0.40161458642977266,
            10.95259405800349484,
            -15.51518510456715916,
            52.97034126309804947,
            8.03760319065614226,
            6.80845044097395746,
            0.87271760716416480,
            19.53488770370943328,
        ],
        rtol=0,
        atol=1e-8,
    )
...
def test_ziP_Dd_matches_fd():
    ...
    for k in range(2):
        ...
        thm[k] -= h
        fd_thk = (
            fam.dev_resids(y, mu, wt, theta=thp) - fam.dev_resids(y, mu, wt, theta=thm)
        ) / (2 * h)
        np.testing.assert_allclose(D["Dth"][:, k], wt * fd_thk, rtol=1e-5, atol=1e-5)
...
def test_cnorm_components_match_mgcv():
    ...
    np.testing.assert_allclose(
        D["Dmu3"],
        [0, -0.000197480812203978, 0, 0.60605462153934, -0.21623962112153],
        rtol=0,
        atol=1e-11,
    )
    np.testing.assert_allclose(
    ...
    np.testing.assert_allclose(
        D["Dmu4"],
        [0, 0.00394885852463045, 0, -0.232125048982469, -0.0565948284912688],
        rtol=0,
        atol=1e-10,
    )
    np.testing.assert_allclose(
    ...
    np.testing.assert_allclose(
        D["Dmu3th"],
        [0, 0.00154423954643431, 0, -1.91101388421101, 0.682675760459353],
        rtol=0,
        atol=1e-10,
    )
    ...
    assert float(np.sum(np.abs(le["LSTH1"]))) == 0.0
    np.testing.assert_allclose(
        fam.get_theta(trans=True), [np.exp(0.3)], rtol=0, atol=1e-12
    )
...
def test_cnorm_Dd_matches_fd():
    ...
    fd_mu2 = (m2ll(mu + h, th0) - 2 * m2ll(mu, th0) + m2ll(mu - h, th0)) / h**2
...
def test_cnorm_construction_and_validation():
    ...
    np.testing.assert_array_equal(
        cnorm(link="log").initialize(np.array([0.3, 2.0]), np.ones(2)), [1.0, 2.0]
    )
    np.testing.assert_array_equal(
...
def test_cpois_components_match_mgcv():
    ...
    np.testing.assert_allclose(
        dr,
        [
            0.093929340763727609,
            0.32427618396314406,
            0.060124358816485542,
            0.87567736610300229,
            1.8,
            0.046856172595730383,
        ],
        rtol=1e-14,
        atol=0,
    )
...
def test_cpois_construction_and_validation():
    ...
    y = np.array([0.5, 2.0])
    np.testing.assert_array_equal(cpois(link="identity").initialize(y, np.ones(2)), y)
    np.testing.assert_array_equal(
    ...
    np.testing.assert_allclose(dev, 2.0 * (ylogy - (y - mu)), rtol=1e-12, atol=1e-12)
...
def test_clog_components_match_mgcv():
    ...
    np.testing.assert_allclose(
        ...
        [
            0.13297872286444656,
            0.00064770448452167173,
            0.024645863841215476,
            1.010811660242168,
            0.9907951404171742,
            3847863142179033.5,
        ],
    ...
    exp = {
        ...
        "Dmu2": [
            ...
            0.52540422036177381,
            0.261286207431696,
            1.9497706066361287e-15,
        ],
        "Dth": [
            -0.26304907488585949,
            -0.0012216848009996006,
            -0.049190740352105466,
            0.33252312875773121,
            0.34729978238482129,
            -68.999999999999886,
        ],
        ...
        "Dmu3": [
            0.096484175770105721,
            0.0079878924099485715,
            0.044443027739635224,
            0.11368032043932709,
            -0.04232451164291777,
            -1.4444255915456878e-15,
        ],
        "Dmu2th": [
            -0.95947739425274758,
            -0.97691640500401522,
        ...
        "Dmu4": [
            ...
            -0.14328723208820304,
            -0.25145288474698707,
            -0.061414535592725583,
            1.0700567966360104e-15,
        ],
        "Dth2": [
            0.5146680726223557,
            0.0021596824071976606,
            0.097978855366470788,
            -0.24845845349984741,
            -0.25323674770941074,
            69.000000000004107,
        ],
        ...
        "Dmu2th2": [
            ...
            1.8340237790089229,
            0.89606206198464955,
            1.9921781173304531e-12,
        ],
        "Dmu3th": [
            -0.36905421087016155,
            -0.019960730575645763,
            -0.17631525284536659,
            -0.44162211521677608,
            0.16382225628438868,
            -4.5499406133689004e-14,
        ],
    }
    ...
    np.testing.assert_allclose(
        fam.aic(y, mu, 0.0, wt, 0), 15.910610320785761, rtol=1e-14
    )
    le = fam.ls_extended(y, wt)
    np.testing.assert_allclose(le["ls"], -6.5018787506728541, rtol=1e-14)
    np.testing.assert_allclose(le["lsth1"], [-3.9623749785560696], rtol=1e-14)
    np.testing.assert_allclose(le["lsth2"], [[-0.073257888763456291]], rtol=1e-13)
    np.testing.assert_allclose(
        le["LSTH1"].ravel(),
        [-1.0, -0.96237497855606957, -1.0, 0.0, 0.0, -1.0],
        rtol=1e-14,
    )
...
def test_clog_construction_and_validation():
    ...
    np.testing.assert_array_equal(_clog_log1pexp(np.array([35.0])), np.exp(35.0))
    np.testing.assert_allclose(
...
def test_bcg_components_match_mgcv():
    ...
    np.testing.assert_allclose(
        ...
        [
            0.0,
            0.062798377990909593,
            -1.4679056433719779,
            0.75371975840918415,
            -0.89905663520180124,
            -0.065243109622599604,
        ],
        rtol=1e-12,
        atol=0,
    )
    np.testing.assert_allclose(
        ...
        [
            3.3575912323843515,
            0.67538398720085868,
            0.0035531314471942497,
            0.25359899816612091,
            0.0,
            547.23090548819118,
            -1.8314673811650801,
            -1.0091661750898306,
            0.082145465281791286,
            0.2085312220130115,
            0.0,
            151.63024907263957,
            0.62977258433481509,
    ...
    np.testing.assert_allclose(
        ...
        [
            -2.4838128187057316,
            -0.22899659577570419,
            0.0026906387185074328,
            -0.45230348721151326,
            0.0,
            124.00985546440461,
            6.5499860818848692,
            1.6161446669546113,
            0.081279279488592815,
            0.20996203307867312,
            0.0,
            49.882657942915102,
            -4.5045865458129013,
    ...
    np.testing.assert_allclose(
        ...
        [
            0.0,
            -0.50356411446106319,
            0.00036808288773070713,
            0.44127999364757825,
            0.0,
            0.23859860125821797,
            0.0,
            1.5768937572121571,
            0.010454051122515517,
            -1.5529373478733481,
            0.0,
            0.44898623742756172,
            16.11002165976381,
    ...
    np.testing.assert_allclose(
        ...
        [
            0.0,
            -0.090272421757199517,
            0.0079221559762974181,
            -0.28988907473869796,
            0.0,
            0.40776943518903863,
            0.0,
...
def test_bcg_construction_and_validation():
    ...
    f = bcg(theta=(2.0, 0.5))
    assert f.n_theta == 0
    np.testing.assert_allclose(f.get_theta(), [2.0, np.log(2.0), np.log(0.5)])
    np.testing.assert_allclose(f.get_theta(True), [2.0, 2.0, np.log(0.5)])
    f = bcg(theta=(1.3, -0.5))
    assert f.n_theta == 2
...
def test_gfam_components_match_mgcv():
    ...
    exp = {
        ...
        "Dth": [
            0.0,
            0.0,
            -0.011494749284079071,
            0.007359766645563358,
            -0.004646648785357787,
            0.0,
            0.0,
            0.0,
            0.0,
    ...
    np.testing.assert_allclose(
        ...
        [
            0.0,
            0.0,
            0.002422743278417013,
            0.0010193790896375246,
            0.0005357661343759946,
            0.0,
            0.0,
            0.0,
            0.0,
...
def test_softplus_poisson_glm_matches_mgcv():
    ...
    np.testing.assert_allclose(
        ...
        [0.595080256, 1.21651587, -0.704083015],
        atol=1e-6,
    )
...
def test_qnbinom_pnbinom_mu_match_r():
    ...
    ps = [1e-4, 0.001, 0.01, 0.1, 0.3, 0.5, 0.77, 0.9, 0.99, 0.9999]
    grids = {
        (2.5, 7.3): [0, 0, 0, 2, 4, 6, 10, 14, 24, 42],
        (2.0, 0.03): [0, 0, 0, 0, 0, 0, 0, 0, 1, 2],
        (37.5, 4200.0): [2103, 2386, 2764, 3345, 3813, 4163, 4690, 5103, 5966, 7254],
        (0.07, 123456.7): [0, 0, 0, 0, 0, 52, 25362, 266391, 2326395, 8729299],
    }
    ...
    np.testing.assert_array_equal(
        ...
        [
            0.032868874445290082,
            0.094078768182692535,
            0.26302499000982504,
            0.59957811374520131,
            0.9746158998054657,
            0.0,
        ...
        for size, mu in grids:
            for lt in (True, False):
                for lg in (True, False):
                    ...
                    got_rs = rs_q(p_in, np.full(10, size), np.full(10, mu), lt, lg)
                    got_py = [nm.qnbinom_mu(p, size, mu, lt, lg) for p in p_in]
...
def _census_frame_42():
    ...
    y[:10] = 0.0
    wt = np.where(rng.uniform(size=n) < 0.3, rng.integers(1, 5, n).astype(float), 1.0)
...
def test_tw_nb_theta_chain_census_matches_mgcv():
    ...
    fam.set_theta(0.9)
    assert float(fam.variance(mu)[0]) == pytest.approx(4.8003315958745265, rel=1e-13)
    dd = fam.Dd(y, mu, 0.9, wt, level=2)
    assert float(dd["Dth"][10]) == pytest.approx(-0.40292464474908973, rel=1e-13)
    assert float(dd["Dth2"][0]) == pytest.approx(18.27147175320773, rel=1e-13)
    assert float(dd["Dmuth2"][1]) == pytest.approx(-0.13234424141561646, rel=1e-13)
    assert float(dd["Dmu2th2"][1]) == pytest.approx(0.3740245563340534, rel=1e-13)
    dd_neg = fam.Dd(y, mu, -0.7, wt, level=2)
    assert float(dd_neg["Dth2"][10]) == pytest.approx(-0.026858465349980126, rel=1e-13)

    fam_nb = nb()
    th = float(np.log(1.7))
    ddn = fam_nb.Dd(ynb, mu, th, wt, level=2)
    assert float(ddn["Dmu3"][5]) == pytest.approx(1.172357101442247, rel=1e-13)
    assert float(ddn["Dmu4"][3]) == pytest.approx(-0.028757174064230596, rel=1e-13)
    assert float(ddn["Dmu3th"][5]) == pytest.approx(-2.1553653475720496, rel=1e-13)
    ls3 = fam_nb.ls_extended(ynb, wt, theta=th, scale=1.0)
    assert float(ls3["LSTH1"][11, 0]) == pytest.approx(0.3940667463254954, rel=1e-13)
