...
def test_nlopt_bobyqa_quadratic_matches_nloptr():
    ...
    """
    def fq(x):
        return (3 * (x[0] - 0.7) ** 2 + (x[1] + 0.3) ** 2
                + 5 * (x[2] - 0.9) ** 2 + 0.2 * x[0] * x[1])

    r = _nlopt_ln_bobyqa(fq, np.array([1.0, 0.0, 1.0]),
                         np.array([0.0, -np.inf, 0.0]),
                         np.array([np.inf, np.inf, np.inf]))
    # nloptr reference (NLOPT_LN_BOBYQA) — byte-for-byte.
    expected_sol = np.array([0.712374762653482, -0.371237567717368,
                             0.899999814113911])
...
def test_nlopt_default_step_matches_nlopt_heuristic():
    ...
    dx = _nlopt_default_step(np.array([1.0, 0.0, 1.0]),
                             np.array([0.0, -np.inf, 0.0]),
                             np.array([np.inf, np.inf, np.inf]))
...
def test_nlopt_bobyqa_respects_maxeval():
    ...
    r = _nlopt_ln_bobyqa(fq, np.array([1.0, 1.0]),
                         np.array([-np.inf, -np.inf]),
                         np.array([np.inf, np.inf]), maxeval=8)
    assert r.nfev <= 8
