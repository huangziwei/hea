...
def _build_cases():
    ...
    add(
        "rnorm_scaled", "rnorm(4000, 2.5, 0.5)", [], lambda mt: mt.rnorm(4000, 2.5, 0.5)
    )
    add(
        "rexp",
        "rexp(4000)",
    ...
    nbm = np.array(
        [m for _ in [0.5, 1.0, 2.0, 5.0, 10.0, 50.0] for m in [0.5, 2.0, 10.0, 50.0]]
        * 30,
        float,
    )
    add(
        "rnbinom",
        "rnbinom(length(p0), size=p0, mu=p1)",
        [nbz, nbm],
    ...
    ncdf = np.array(
        [d for d in [1.0, 2.0, 5.0, 10.0] for _ in [0.5, 2.0, 8.0]] * 80, float
    )
    ncp = np.array(
        [c for _ in [1.0, 2.0, 5.0, 10.0] for c in [0.5, 2.0, 8.0]] * 80, float
    )
    add(
        "rchisq_nc",
        "rchisq(length(p0), p0, ncp=p1)",
        [ncdf, ncp],
    ...
    f1 = np.array(
        [a for a in [1.0, 5.0, 30.0, 100.0] for _ in [2.0, 10.0, 50.0]] * 60, float
    )
    f2 = np.array(
        [b for _ in [1.0, 5.0, 30.0, 100.0] for b in [2.0, 10.0, 50.0]] * 60, float
    )
    add(
        "rf",
        "rf(length(p0), p0, p1)",
    ...
    bb = np.array(
        [b for _ in [0.3, 0.5, 1.0, 2.0, 5.0] for b in [0.5, 1.0, 3.0]] * 60, float
    )
    add(
        "rbeta",
        "rbeta(length(p0), p0, p1)",
        [ba, bb],
    ...
    add(
        "sample_rep",
        "sample.int(60, 1000, replace=TRUE)",
        [],
    ...
    add(
        "sample_prob_norep",
        "sample.int(500, 100, prob=p0)",
        [w],
        lambda mt, p=w: mt.sample_prob(p, 100, False) + 1,
    )
    add(
        "sample_prob_walker",
        "sample.int(500, 800, replace=TRUE, prob=p0)",
        [w],
    ...
    add("exp_rand_n", "rexp(3000)", [], lambda mt: mt.exp_rand_n(3000))
